...
fileprivate let kShareMessage:[UInt8] = [0x52,0x57,0x52,0x5d,0x11,0x4c,0x58,0x4d,0x4e,0x5b,0x23,0x12,0x9,0x51,0x4a,0x5c,0x9,0x57,0x58,0x5d,0x9,0x4b,0x4e,0x4e,0x57,0x9,0x52,0x56,0x59,0x55,0x4e,0x56,0x4e,0x57,0x5d,0x4e,0x4d]

fileprivate func componentPart(target num: UInt8) -> UInt8 {
...
class PopMenuThen: StartThen {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: WithTransitioningDelegate!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = WithTransitioningDelegate(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kShareMessage.map{componentPart(target: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class WithTransitioningDelegate: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: constFormalContent, height: user_recordMarginValue * 0.6)
    ...
    func scopePackage(corners: UIRectCorner, radius: CGFloat) {
...
extension WithTransitioningDelegate: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = FoundRecognizerDelegate(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension WithTransitioningDelegate: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            instLikeLinguisticContext(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            formatContext(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func instLikeLinguisticContext(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func formatContext(transitionContext: UIViewControllerContextTransitioning) {
...
class FoundRecognizerDelegate: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: WithTransitioningDelegate!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: WithTransitioningDelegate, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func clickTap() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(visualisation))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: constFormalContent, height: user_recordMarginValue)
