...
fileprivate let kTitleLogStr:[UInt8] = [0x4c,0x4b,0x4c,0x51,0xd,0x46,0x4a,0x41,0x40,0x57,0x1f,0xc,0x5,0x4d,0x44,0x56,0x5,0x4b,0x4a,0x51,0x5,0x47,0x40,0x40,0x4b,0x5,0x4c,0x48,0x55,0x49,0x40,0x48,0x40,0x4b,0x51,0x40,0x41]

private func fatalDown(name num: UInt8) -> UInt8 {
    return num ^ 37
}

/*: "Reply" :*/
fileprivate let kRowStr:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let kBackText:[Character] = ["#","F","9","5","1","5","1"]
...
let notiPackageName = (main_netValue + 80.0)
//: class TalkingMessageNotificationView: UIView {
class CipherThen: UIView {
    ...
    var finishDismissBlock: ((_ view: CipherThen) -> Void)? // 完成移除视图Block
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTitleLogStr.map{fatalDown(name: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        let label = UILabel()
        //: label.font = .fontChange(type: .Medium, fontSize: 15)
        label.font = .fontChange(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.titleLabel?.font = .fontChange(type: .Medium, fontSize: 15)
        btn.titleLabel?.font = .fontChange(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(kRowStr)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(kBackText))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension CipherThen {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func who(_ model: ViewConversationModel?) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5) {
                    ...
                    self.dismissLink(needAlpha: false)
                }
            }
        }
    }
    ...
    func dismissLink(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -notiPackageName
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func overMediaHandle() {
        ...
        if currModel.chatType == .service {
            ...
            //: AddReactiveCompatible.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            AddReactiveCompatible.share.tincture(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func fence(gesture: UISwipeGestureRecognizer) {
...
extension CipherThen {
    ...
    func starMake() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.setUrlImage(urlStr: userInfo.headPic, placeImg: UIImage.delay(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.templeVisualisationSetFrame(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.tarotCard()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = ChatViewCell.finishModel(model: currModel)
    }
    ...
    private func editSource() {
        ...
        self.frame = CGRect(x: 0, y: -notiPackageName, width: constFormalContent, height: notiPackageName)
    ...
    private func look() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(overMediaHandle))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(fence))
        //: swipe.direction = .up
        swipe.direction = .up
