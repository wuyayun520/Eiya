...
fileprivate let k_tapStr:[Character] = ["\u{8bbe}","\u{7f6e}","普","通","\u{72b6}","\u{6001}","的","\u{6587}"]
fileprivate let kAddStr:String = "\u{5b57}颜\u{8272}时 请"
fileprivate let k_whiteViewId:String = "manager gift turn key blind使用RG"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let k_currentUrl:[Character] = ["设","置","选","中","状"]
fileprivate let k_errorColorData:String = "态的文"
fileprivate let kBuildSucceedValue:String = "字\u{989c}色时 请"
fileprivate let k_removeMsg:String = "间的颜data值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let kCenterMsg:[UInt8] = [0xd4,0xd9,0xd4,0xdf,0x93,0xce,0xda,0xcf,0xd0,0xdd,0xa5,0x94,0x8b,0xd3,0xcc,0xde,0x8b,0xd9,0xda,0xdf,0x8b,0xcd,0xd0,0xd0,0xd9,0x8b,0xd4,0xd8,0xdb,0xd7,0xd0,0xd8,0xd0,0xd9,0xdf,0xd0,0xcf]

fileprivate func blueprintWill(from num: UInt8) -> UInt8 {
...
/*: "设置的下标不合法!!" :*/
fileprivate let k_mediumBottomStrengthName:String = "secure"
fileprivate let k_buttonViewPath:[Character] = ["置","的","下","标","\u{4e0d}","\u{5408}","法","!","!"]

/*: "Bags" :*/
fileprivate let kPathData:String = "indicator if hideBags"
...
open class CoordinateReactiveCompatible: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: PitchSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.excavate(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(k_tapStr) + kAddStr + String(k_whiteViewId.suffix(4)) + "B\u{7a7a}间的颜色值"))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.excavate(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(k_currentUrl) + k_errorColorData.capitalized + kBuildSucceedValue + "使用RGB空" + k_removeMsg.replacingOccurrences(of: "data", with: "色")))
        }
    ...
    fileprivate func excavate(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: PitchSegmentStyle, titles: [String]) {
    ...
    @objc func infoClick(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? ExtentThen else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kCenterMsg.map{blueprintWill(from: $0)}, encoding: .utf8)!)
    }
}
...
public extension CoordinateReactiveCompatible {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func scaleOfMeasurement(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (k_mediumBottomStrengthName.replacingOccurrences(of: "secure", with: "设") + String(k_buttonViewPath)))
    ...
    func names(_ titles: [String]) {
        ...
        scaleOfMeasurement(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension CoordinateReactiveCompatible {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func underContents() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = ExtentThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.infoClick(_:)))
            ...
            if title == (String(kPathData.suffix(4))).localized {
    ...
    fileprivate func electronicName() {
        ...
        if let firstLabel = labelsArray[0] as? ExtentThen {
    ...
    func doColor(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: (String(kPathData.suffix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            AppNameGiftManager.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! ExtentThen
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! ExtentThen
        // 让选中标签居中显示
    ...
    public func isSex(_ currentIndex: Int) {
