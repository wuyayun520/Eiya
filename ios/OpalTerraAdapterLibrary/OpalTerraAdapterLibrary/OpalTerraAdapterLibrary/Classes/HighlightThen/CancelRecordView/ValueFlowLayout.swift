...
class ValueFlowLayout: UICollectionViewFlowLayout {

    //: var sectionArr = [GiftDataType]()  
    var sectionArr = [AddSubscriptType]()  // collection有多少个分区
    //: var hotPage = 0                    
    var hotPage = 0                    // hot页数
    ...
    private var GiftItemHeight  = Int(constViewText/2) // 每个礼物的高度
    ...
    override func prepare() {
        ...
        layoutContentSize = CGSize(width: Int(constFormalContent-10) * totalPage, height: 2*GiftItemHeight)
    }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if indexPath.section == AddSubscriptType.Vip.rawValue {
            //: page += hotPage
            page += hotPage
        //: } else if indexPath.section == GiftDataType.Bags.rawValue {
        } else if indexPath.section == AddSubscriptType.Bags.rawValue {
        ...
        let itemWidth = Int(constFormalContent-10) / GiftItemColumn
        //: let x = page * Int(ScreenWidth-10) + pageCol * itemWidth
        let x = page * Int(constFormalContent-10) + pageCol * itemWidth
