...
/*: "Speaker On" :*/
fileprivate let kArcData:String = "Speaki size"

/*: "Speaker Off" :*/
fileprivate let kManagerSubmitContent:[Character] = ["S","p","e","a","k","e","r"," ","O","f","f"]

/*: "s- :*/
fileprivate let kListAddStr:[Character] = ["s","-"]

/*: - :*/
fileprivate let kPriceCornerCollectionContent:[Character] = ["-"]

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let k_botData:String = "TRTrequest"
fileprivate let kMakeAtFormat:String = "销毁. "
fileprivate let k_toGiftValue:String = "self listlogId："

/*: ." :*/
fileprivate let kTitleData:[Character] = ["."]

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let k_makeTitle:[Character] = ["T","R","T","C"," ","音","频","通"]
fileprivate let kModelId:String = "话\u{9519}误. "
fileprivate let k_infoSizeFormat:String = "back manager if effectode:"

/*: , errMsg: :*/
fileprivate let kToId:String = "text self moment var speak, er"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let k_countFormat:[Character] = ["T","R","T","C"," ","音","频","\u{901a}","话","S","D","K","收","到","警","告","."," ","w","a","r","n","i","n","g","C","o","d","e"]
fileprivate let k_itemTitle:String = ":"

/*: , warningMsg: :*/
fileprivate let k_listMessage:String = ", wavoice mess else section"
fileprivate let kDeleteEnableData:String = "rviewn"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let kMakeFormat:[Character] = ["T","R","T","C"," ","音","频","通","话","\u{8fdb}","\u{5165}","房","间","."," ","r","e","s","u","l"]
fileprivate let k_readId:[Character] = ["t",":"]

/*: , logId： :*/
fileprivate let k_equalName:[Character] = [","," ","l","o","g","I","d","\u{ff1a}"]

/*:  加入了房间" :*/
fileprivate let k_appId:[Character] = [" ","加","\u{5165}","了","房","间"]

/*:  离开了房间" :*/
fileprivate let k_popPath:String = " 离cell了房间"
...
class PopCloudDelegate: NSObject {
    ...
    private var config = ConfigFileThen() // 音频渲染配置类
    //: private var logId = 0
...
extension PopCloudDelegate {
    ...
    func gazette(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !showSearch()

        //: self.config.isSpace = true
        self.config.isSpace = true
        //: self.logId = roomId
        self.logId = roomId
        //: enterRoom()
    ...
    func slowingAction(isMute: Bool) {
    ...
    func follow(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard showSearch() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(kArcData.prefix(5)) + "er On").localized : (String(kManagerSubmitContent)).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        masterKey(isHandsfree: isHandsfree)
    }
}
...
extension PopCloudDelegate {
    ...
    func notificationText() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(audioAppNotifPublicTransportBrake(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(popularNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(inputSignal(notif:)),
    ...
    @objc private func audioAppNotifPublicTransportBrake(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func popularNotif(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func inputSignal(notif: Notification) {
        ...
        DispatchQueue.main.async {
            ...
            if interruptStatus == AVAudioSession.InterruptionType.began {
                ...
                self.audioAppNotifPublicTransportBrake(notif: nil)
    ...
    @objc func supra(notif: Notification) {
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(kArcData.prefix(5)) + "er On").localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            masterKey(isHandsfree: self.config.isHandsfree)
...
extension PopCloudDelegate {
    ...
    private func walkIn() {
        ...
        params.sdkAppId = app_limitMaleText
        //: params.roomId = UInt32(logId)
        params.roomId = UInt32(logId)
        //: params.userId = DistanceAppManager.share.loginUserMode.userID
        params.userId = DistanceAppManager.share.loginUserMode.userID
        //: params.userSig = DistanceAppManager.share.appUserConfigMode.uSigTrtc
        params.userSig = DistanceAppManager.share.appUserConfigMode.uSigTrtc
    ...
    private func masterKey(isHandsfree: Bool) {
    ...
    private func iconTableView() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        UploadDisplayThen.makeSelected(msg: (k_botData.replacingOccurrences(of: "request", with: "C") + " 音\u{9891}通话" + kMakeAtFormat.capitalized + String(k_toGiftValue.suffix(6))) + "\(logId).")
        //: guard self.config.isSpace else { return }
        guard self.config.isSpace else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            masterKey(isHandsfree: true)
        }
...
extension PopCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        UploadDisplayThen.makeSelected(msg: (String(k_makeTitle) + kModelId + "errC" + String(k_infoSizeFormat.suffix(4))) + "\(errCode)" + (String(kToId.suffix(4)) + "rMsg:") + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.arcInfo(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        UploadDisplayThen.makeSelected(msg: (String(k_countFormat) + k_itemTitle.capitalized) + "\(warningCode)" + (String(k_listMessage.prefix(4)) + kDeleteEnableData.replacingOccurrences(of: "view", with: "ni") + "gMsg:") + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        UploadDisplayThen.makeSelected(msg: (String(kMakeFormat) + String(k_readId)) + "\(result)" + (String(k_equalName)) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            //: self.config.disconnected = true
            self.config.disconnected = true
            //: TalkingSocketManager.shared.endTalkingPing()
            ListSocketManager.shared.someColor()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(k_appId)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (k_popPath.replacingOccurrences(of: "cell", with: "开")))
    }
}
