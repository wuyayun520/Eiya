...
/*: "call_bgm" :*/
fileprivate let k_shareStr:String = "call_bgsize count stage"
fileprivate let kEqualTitleMsg:String = "top"

/*: "call_response_bgm" :*/
fileprivate let k_succeedRawValue:String = "info cell user textcall_"
fileprivate let k_toStr:String = "rewhitep"
...
enum HighlightExpressionConvertible: Int {
    //: case Call
    case Call /// 音视频拨打音乐
    //: case Response
...
class ModePlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = HighlightExpressionConvertible.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func medicamentCheck(type: HighlightExpressionConvertible) -> ModePlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = ModePlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.someLocal()
...
extension ModePlayer {
    ...
    private func selectNotify(notify: Notification) {
    ...
    private func paper(notify: Notification) {
    ...
    private func headInMarshal() -> Bool {
    ...
    private func someLocal() {
        //: var fileName = "call_bgm"
        var fileName = (String(k_shareStr.prefix(7)) + kEqualTitleMsg.replacingOccurrences(of: "top", with: "m"))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(k_succeedRawValue.suffix(5)) + k_toStr.replacingOccurrences(of: "white", with: "s") + "onse_bgm")
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = VideoEffectTool.default.titleType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension ModePlayer: TXVodPlayListener {
