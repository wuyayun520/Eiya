...
fileprivate let kTextValue:[UInt8] = [0x14,0x13,0x14,0x9,0x55,0x1e,0x12,0x19,0x18,0xf,0x47,0x54,0x5d,0x15,0x1c,0xe,0x5d,0x13,0x12,0x9,0x5d,0x1f,0x18,0x18,0x13,0x5d,0x14,0x10,0xd,0x11,0x18,0x10,0x18,0x13,0x9,0x18,0x19]

private func appError(make num: UInt8) -> UInt8 {
    return num ^ 125
}

/*: "#FFD13A" :*/
fileprivate let kFilterColorMessage:String = "#Fcontent equal frame var"
fileprivate let k_detailStr:String = "FD13view"

/*: "gift_gb_umnad_1" :*/
fileprivate let kTouchNameFormat:String = "kit view valuegift_g"

/*: "icon_circle_pic_default" :*/
fileprivate let kListStr:[Character] = ["i","c","o","n","_","c","i","r","c","l","e","_","p","i","c"]
fileprivate let k_giftValue:[Character] = ["_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let kKitName:String = "Openchannel self"
fileprivate let k_imageId:String = "ky blself status format data"

/*: "Sent " :*/
fileprivate let kScaleValue:String = "if ifSent "

/*: "PingFangSC-Semibold" :*/
fileprivate let kLeadingId:String = "left stroke viewPing"
fileprivate let kTouchFormalKey:String = "-Semivar if"
fileprivate let kAddPath:String = "bmainld"

/*: "%d" :*/
fileprivate let kCellPath:String = "point direction white%d"

/*: "Bombard" :*/
fileprivate let k_labelKey:String = "Bombardshow content make on"
...
class SharedReactiveCompatible: UIView {
    ...
    var giftMsgModel = TitleManagerMeasurable()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [PublishHandyJSON()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTextValue.map{appError(make: $0)}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(kFilterColorMessage.prefix(2)) + k_detailStr.replacingOccurrences(of: "view", with: "A")))
...
extension SharedReactiveCompatible {
    ...
    func settleDataMTheory(model: TitleManagerMeasurable) {
        //: self.giftMsgModel = model
        self.giftMsgModel = model
        //: let fromNickname = giftMsgModel.fromNickname
        let fromNickname = giftMsgModel.fromNickname
        //: self.bgImage.image = UIImage.submitGift(name: "gift_gb_umnad_1")
        self.bgImage.image = UIImage.submitGift(name: (String(kTouchNameFormat.suffix(6)) + "b_umnad_1"))

        //: let placeholder = UIImage.submitGift(name: "icon_circle_pic_default")
        let placeholder = UIImage.submitGift(name: (String(kListStr) + String(k_giftValue)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == BugOutMirrorPath.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(kKitName.prefix(4)) + " luc" + String(k_imageId.prefix(5)) + "ind-box").localized
            //: }else {
        } else {
        ...
        self.giftName.text = (String(kScaleValue.suffix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(kLeadingId.suffix(4)) + "FangSC" + String(kTouchFormalKey.prefix(5)) + kAddPath.replacingOccurrences(of: "main", with: "o")), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = full(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(kLeadingId.suffix(4)) + "FangSC" + String(kTouchFormalKey.prefix(5)) + kAddPath.replacingOccurrences(of: "main", with: "o")), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = full(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.publicTransport(texts: strs, colors: colors, fonts: fonts)
        ...
        nobble(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func nobble(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(kLeadingId.suffix(4)) + "FangSC" + String(kTouchFormalKey.prefix(5)) + kAddPath.replacingOccurrences(of: "main", with: "o")), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = full(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(kLeadingId.suffix(4)) + "FangSC" + String(kTouchFormalKey.prefix(5)) + kAddPath.replacingOccurrences(of: "main", with: "o")), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = full(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.behindVoice(150, 77, 20, 1), UIColor.white, UIColor.behindVoice(150, 77, 20, 1)]
        ...
        attText = attText.publicTransport(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.forewordRobot(attributedText: attText)
        ...
        countLb.atChaseFrame(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func bridePriceModel(giftMsgModel: TitleManagerMeasurable) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(busy), object: nil)
        ...
        self.perform(#selector(busy), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func workIn(model _: TitleManagerMeasurable, animate _: Bool) {
    ...
    func animationLogChase(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
    ...
    @objc func busy() {
        ...
        giftMsgModel = TitleManagerMeasurable()
...
extension SharedReactiveCompatible {
    ...
    func full(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension SharedReactiveCompatible {
    ...
    private func groupSubviews() {
        ...
        font = full(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.publicTransport(texts: [(String(k_labelKey.prefix(7))).localized], colors: [UIColor.behindVoice(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.atChaseFrame(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.forewordRobot(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.atChaseFrame(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
