...
fileprivate let kAppData:[UInt8] = [0xb2,0xb5,0xb2,0xaf,0xf3,0xb8,0xb4,0xbf,0xbe,0xa9,0xe1,0xf2,0xfb,0xb3,0xba,0xa8,0xfb,0xb5,0xb4,0xaf,0xfb,0xb9,0xbe,0xbe,0xb5,0xfb,0xb2,0xb6,0xab,0xb7,0xbe,0xb6,0xbe,0xb5,0xaf,0xbe,0xbf]

private func doingRange(gesture num: UInt8) -> UInt8 {
    return num ^ 219
}

/*: "%02ld:%02ld:%02ld" :*/
fileprivate let k_intimateText:String = "push show to false self%02l"
fileprivate let kInputFormat:String = "self model var kit2ld:%02ld"

/*: "icon_mini_voice_end" :*/
fileprivate let kUserValue:[UInt8] = [0xb5,0xaf,0xbb,0xba,0xab,0xb9,0xb5,0xba,0xb5,0xab,0xc2,0xbb,0xb5,0xaf,0xb1,0xab,0xb1,0xba,0xb0]

fileprivate func statusFirst(re num: UInt8) -> UInt8 {
...
fileprivate let kAddStr:String = "as makeicon_m"
fileprivate let kPlayerId:String = "message frame main self sectionvoice_"
fileprivate let k_tableUrl:String = "ispaceg"

/*: "#F95151" :*/
fileprivate let k_phoneName:String = "if self#F95151"

/*: "#34C759" :*/
fileprivate let kDirectionTitle:[Character] = ["#","3","4","C","7"]
fileprivate let kStatKey:String = "5ready"

/*: "End of call" :*/
fileprivate let k_sourceName:String = "more announcement content indexEn"
fileprivate let k_atName:String = "view heightd of"
...
class ListMiniView: ObjectMiniView {
    // 小窗口通话状态
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kAppData.map{doingRange(gesture: $0)}, encoding: .utf8)!)
    }
...
extension ListMiniView {
    /// 构建音频小窗口
    //: static func buildVoiceMiniView() -> TalkingVoiceMiniView {
    static func activityView() -> ListMiniView {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = ListMiniView(frame: .zero)
    ...
    func subsequently(_ time: Double) {
...
extension ListMiniView {
    ...
    private func quantityimateConstraint() {
        ...
        vocalisationOpen(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func vocalisationOpen(status: EqualWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: kUserValue.map{statusFirst(re: $0)}, encoding: .utf8)! : (String(kAddStr.suffix(6)) + "ini_" + String(kPlayerId.suffix(6)) + "call" + k_tableUrl.replacingOccurrences(of: "space", with: "n"))
        //: self.statusIcon.image = UIImage.submitGift(name: iconStr)
        self.statusIcon.image = UIImage.submitGift(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(k_phoneName.suffix(7)))) : UIColor(hex: (String(kDirectionTitle) + kStatKey.replacingOccurrences(of: "ready", with: "9")))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(k_sourceName.suffix(2)) + String(k_atName.suffix(4)) + " call").localized
        }
    }
}
