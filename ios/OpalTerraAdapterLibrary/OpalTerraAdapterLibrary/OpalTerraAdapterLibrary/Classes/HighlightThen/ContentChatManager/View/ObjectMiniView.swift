...
fileprivate let k_warningFormat:[UInt8] = [0xb5,0xba,0xb5,0xc0,0x74,0xaf,0xbb,0xb0,0xb1,0xbe,0x86,0x75,0x6c,0xb4,0xad,0xbf,0x6c,0xba,0xbb,0xc0,0x6c,0xae,0xb1,0xb1,0xba,0x6c,0xb5,0xb9,0xbc,0xb8,0xb1,0xb9,0xb1,0xba,0xc0,0xb1,0xb0]

fileprivate func areaActual(seek num: UInt8) -> UInt8 {
...
class ObjectMiniView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_warningFormat.map{areaActual(seek: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        ListSocketManager.shared.containerView?.isHidden = false
    }

    /// 移除视图
    //: func dismissWhenCallEnd() {
    func spiralBlindfoldLacrimalApparatusWithoutAdjourn() {
        //: removeAllGestures()
        tantamount()
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= constFormalContent * 0.5) ? -self.width : constFormalContent + self.width
            ...
            ListSocketManager.shared.containerView?.isHidden = true
        }
    }
}
...
extension ObjectMiniView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func quitPop() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.sumimate(sender:)))
    ...
    func subGestures() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.forage(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.quitPop()
    ...
    @objc func forage(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(constFormalContent - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(user_recordMarginValue - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = constFormalContent - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = constFormalContent
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= constFormalContent * 0.5 {
            ...
            ListSocketManager.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func sumimate(sender _: UIPanGestureRecognizer) {
