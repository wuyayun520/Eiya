...
fileprivate let kUserMaxKey:[UInt8] = [0xf1,0xf6,0xf1,0xec,0xb0,0xfb,0xf7,0xfc,0xfd,0xea,0xa2,0xb1,0xb8,0xf0,0xf9,0xeb,0xb8,0xf6,0xf7,0xec,0xb8,0xfa,0xfd,0xfd,0xf6,0xb8,0xf1,0xf5,0xe8,0xf4,0xfd,0xf5,0xfd,0xf6,0xec,0xfd,0xfc]

private func molarityShare(info num: UInt8) -> UInt8 {
    return num ^ 152
}

/*: "#171717" :*/
fileprivate let kFailData:[Character] = ["#","1","7","1","7","1","7"]

/*: "icon_cameraoff" :*/
fileprivate let kInputFormat:[Character] = ["i","c","o","n","_","c","a","m"]
fileprivate let kCounteractionFormat:String = "cardraoff"
...
class CoverWindowSashView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.behindVoice(51, 51, 51, 1)
        ...
        shank()
        //: bindInteraction()
        digitiserCounteraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kUserMaxKey.map{molarityShare(info: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (String(kFailData)))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        //: v.contentMode = .scaleAspectFill
        v.contentMode = .scaleAspectFill
        //: v.image = UIImage.submitGift(name: "icon_cameraoff")
        v.image = UIImage.submitGift(name: (String(kInputFormat) + kCounteractionFormat.replacingOccurrences(of: "card", with: "e")))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension CoverWindowSashView {
    /// 刷新视频通话背景
    /// - Parameters:
    ///   - urlStr: 背景图片
    ///   - isOpenCamera: 摄像头状态
    //: func refreshBgView(urlStr: String?, isOpenCamera: VideoCameraStatus) {
    func managerCamera(urlStr: String?, isOpenCamera: WithCameraStatus) {
...
extension CoverWindowSashView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func objectTo(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, constFormalContent - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, user_recordMarginValue - self.frame.size.height * 0.5)
...
extension CoverWindowSashView {
    ...
    private func digitiserCounteraction() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(conference))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(objectTo(pan:)))
        //: pan.delegate = self
        pan.delegate = self
