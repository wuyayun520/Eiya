...
fileprivate let k_touchName:[UInt8] = [0x3c,0x3b,0x3c,0x21,0x7d,0x36,0x3a,0x31,0x30,0x27,0x6f,0x7c,0x75,0x3d,0x34,0x26,0x75,0x3b,0x3a,0x21,0x75,0x37,0x30,0x30,0x3b,0x75,0x3c,0x38,0x25,0x39,0x30,0x38,0x30,0x3b,0x21,0x30,0x31]

private func courseOfStudy(cell num: UInt8) -> UInt8 {
    return num ^ 85
}
...
@objc protocol LimitObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func sizeEmpty()

    //: @objc optional func endScroll()
    @objc optional func commonGroupView()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class DanmuThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(quickFor))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_touchName.map{courseOfStudy(cell: $0)}, encoding: .utf8)!)
    }
...
extension DanmuThen {
    ...
    func detailTimer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(blue), userInfo: nil, repeats: true)
    }
...
extension DanmuThen {
    ...
    func mews(msgModel: StartMeasurable?) {
    ...
    func green() {
        ...
        if AllHeight > main_useData {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < main_useData {
                ...
                self.msgTableView.height = main_useData
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = main_useData - AllHeight
        ...
        resumeDoing(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func resumeDoing(animated: Bool) {
    ...
    func carteDuJourPending(inPending: Bool) {
...
extension DanmuThen: UITableViewDelegate, UITableViewDataSource {
    ...
    func faultAt(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.commonGroupView?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(appPartyValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = PublishBassCell.singleFile(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension DanmuThen {
    ...
    func subImage() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: appPositionText, height: main_useData)
    }
}
