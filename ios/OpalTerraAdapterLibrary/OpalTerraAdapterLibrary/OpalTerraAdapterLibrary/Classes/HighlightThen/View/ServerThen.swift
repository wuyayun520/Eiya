...
fileprivate let k_textKey:[Character] = ["N","e","e","d"," ","a"," ","g","i","f","t"," ","t","o"," ","u","n","l","o","c"]
fileprivate let kBrightUrl:String = "under"

/*: "init(coder:) has not been implemented" :*/
fileprivate let kGiftPath:[UInt8] = [0x30,0x37,0x30,0x2d,0x71,0x3a,0x36,0x3d,0x3c,0x2b,0x63,0x70,0x79,0x31,0x38,0x2a,0x79,0x37,0x36,0x2d,0x79,0x3b,0x3c,0x3c,0x37,0x79,0x30,0x34,0x29,0x35,0x3c,0x34,0x3c,0x37,0x2d,0x3c,0x3d]

private func withImage(make num: UInt8) -> UInt8 {
    return num ^ 89
}

/*: "intimatephoto_btn_no" :*/
fileprivate let k_translateTitle:[Character] = ["i","n","t","i","m","a","t","e","p","h"]
fileprivate let k_fillPath:String = "data makeoto_"
fileprivate let k_modelName:String = "for eventbtn_no"

/*: "Burn After Reading" :*/
fileprivate let k_toTitle:String = "with toBurn "
fileprivate let k_failPath:[Character] = ["e","a","d","i","n","g"]

/*: "Use Photo" :*/
fileprivate let kDuringMsg:[Character] = ["U","s","e"," ","P","h","o","t"]
fileprivate let k_afterPath:String = "near"

/*: "Use Video" :*/
fileprivate let k_cellActionContent:String = "u"
fileprivate let k_intervalMessage:String = "equal area type screense Video"

/*: "intimatephoto_btn_chose" :*/
fileprivate let k_viewMessage:[Character] = ["i","n","t","i","m","a","t","e","p","h","o","t","o","_","b","t","n","_","c","h","o","s","e"]

/*: "%@ (%@x1  %@ Gold coins)" :*/
fileprivate let kTapMidValue:String = "%@photo(%"
fileprivate let kViewPath:String = "make frame return model false%@ Go"
fileprivate let k_zoneUrl:String = ")"

/*: "#7E6AFF" :*/
fileprivate let k_colorKey:String = "#7E6AFFcell make cell transform"
...
class ServerThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kGiftPath.map{withImage(make: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var burnAfterReadingImgV: UIImageView = {
        //: let img = UIImageView(image: UIImage.submitGift(name: "intimatephoto_btn_no"))
        let img = UIImageView(image: UIImage.submitGift(name: (String(k_translateTitle) + String(k_fillPath.suffix(4)) + String(k_modelName.suffix(6)))))
    ...
    private lazy var burnAfterReadingLab: UILabel = {
        ...
        lab.text = (String(k_toTitle.suffix(5)) + "After R" + String(k_failPath)).localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var burnAfterReadingBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(lipTo), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var needGiftBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(labelMake), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var useBtn: UIButton = {
        ...
        btn.setTitle(self.type == .intimatePhoto ? (String(kDuringMsg) + k_afterPath.replacingOccurrences(of: "near", with: "o")).localized : (k_cellActionContent.uppercased() + String(k_intervalMessage.suffix(8))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(outsideImage), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ServerThen {
    ...
    @objc private func outsideImage() {
    ...
    @objc private func lipTo() {
        ...
        let imageName = burnAfterReadingBtn.isSelected ? (String(k_viewMessage)) : (String(k_translateTitle) + String(k_fillPath.suffix(4)) + String(k_modelName.suffix(6)))
        //: burnAfterReadingImgV.image = UIImage.submitGift(name: imageName)
        burnAfterReadingImgV.image = UIImage.submitGift(name: imageName)
    }
}
...
extension ServerThen {
    ...
    @objc private func labelMake() {
        //: guard needGiftBtn.isSelected == false else {
        guard needGiftBtn.isSelected == false else { // 还原状态
            //: needGiftImgeV.image = UIImage.submitGift(name: "intimatephoto_btn_no")
            needGiftImgeV.image = UIImage.submitGift(name: (String(k_translateTitle) + String(k_fillPath.suffix(4)) + String(k_modelName.suffix(6))))
        ...
        let type = (self.type == .intimatePhoto ? FrameCollection.intimatePhoto : FrameCollection.intimateVideo)
        //: TalkingChatGiftManager.share.func__sendGiftEvent(type: type) {
        AppNameGiftManager.share.searchCompletion(type: type) {
            //: self.func__showGiftChoiceView()
            self.queryFinancialAid()
        }
    }

    /// 展示礼物视图
    //: private func func__showGiftChoiceView() {
    private func queryFinancialAid() {
        ...
        giftView.okActionBlock = { [weak self] giftModel in
            ...
            self.needGiftBtn.isSelected = true
            //: self.needGiftImgeV.image = UIImage.submitGift(name: "intimatephoto_btn_chose")
            self.needGiftImgeV.image = UIImage.submitGift(name: (String(k_viewMessage)))
            ...
            let giftAttrStr = NSMutableAttributedString(string: (kTapMidValue.replacingOccurrences(of: "photo", with: " ") + "@x1  " + String(kViewPath.suffix(5)) + "ld coins" + k_zoneUrl.capitalized).edgeDelay(self.normalGiftText, giftModel.name, giftModel.price))
            //: let attributes = [.foregroundColor: UIColor(hex: "#7E6AFF")!, .font: UIFont.fontChange(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
            let attributes = [.foregroundColor: UIColor(hex: (String(k_colorKey.prefix(7))))!, .font: UIFont.fontChange(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
...
extension ServerThen {
    ...
    private func tier() {
        ...
        whiteBgV.addSubview(burnAfterReadingBtn)
        //: if DistanceAppManager.share.loginUserMode.status != 1 &&
        if DistanceAppManager.share.loginUserMode.status != 1,
    ...
    private func sameNameConstraint() {
        ...
        useBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-20 - appButtonData)
