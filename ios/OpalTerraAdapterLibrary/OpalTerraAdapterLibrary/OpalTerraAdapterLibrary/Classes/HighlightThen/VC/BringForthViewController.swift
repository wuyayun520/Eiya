...
fileprivate let kActualVideoText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_photo_giftbg" :*/
fileprivate let k_tempFormat:String = "ibubble"
fileprivate let kMakePlayerMessage:[Character] = ["o","n","_","p","h","o","t","o","_","g","i","f","t","b","g"]

/*: "Send" :*/
fileprivate let k_cookieLastKey:String = "Sendview in public"

/*: "nav_back_black_nor" :*/
fileprivate let kErrorViewFormat:String = "nav_bcookie tap make text view"
fileprivate let kInfoFormat:String = "actop"
fileprivate let kGiftPath:String = "_blakit block"

/*: "toUid" :*/
fileprivate let kModelName:String = "toUidmake container time"

/*: "giftId" :*/
fileprivate let kShareLayerMsg:String = "giftIdheight lab line"

/*: "giftNum" :*/
fileprivate let kBlankNameId:String = "giftNudata content formal model"
fileprivate let kObserverName:String = "load"

/*: "unlockMsgId" :*/
fileprivate let kViewMessage:String = "unarc"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let kHeadFormat:[UInt8] = [0x8f,0xa4,0xa4,0xa5,0xe1,0xa0,0xe1,0xe4,0x81,0xe1,0xe9,0xe4,0x81,0xe1,0x86,0xae,0xad,0xa5,0xe1,0xa2,0xae,0xa8,0xaf,0xb2,0xe8,0xe1,0xb5,0xae,0xe1,0xb4,0xaf,0xad,0xae,0xa2,0xaa]

private func labelCenterAll(index num: UInt8) -> UInt8 {
    return num ^ 193
}

/*: "icon_photo_lock" :*/
fileprivate let kListId:String = "applicationon"
fileprivate let k_effectFailureMessage:[Character] = ["t","o","_","l","o","c","k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let kNameValue:[UInt8] = [0x73,0x35,0x31,0x20,0x77,0x65,0x69,0x76,0x20,0x6f,0x74,0x20,0x6e,0x65,0x65,0x72,0x63,0x73,0x20,0x65,0x68,0x74,0x20,0x6e,0x6f,0x20,0x6b,0x63,0x69,0x6c,0x43]

/*: "icon_photo_burn" :*/
fileprivate let kColorMessage:String = "case leading noicon_"
fileprivate let k_netManagerStr:String = "lovelyoto"

/*: "Already burned" :*/
fileprivate let kFromUrl:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r"]
fileprivate let k_addKey:[Character] = ["n","e","d"]
...
// __CLOSE_PRINT__
//
//  BringForthViewController.swift
//  AbroadTalking
//
...
typealias IntimatePhotoBlock = (_ model: TimeExposureModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class BringForthViewController: StartThen {
    ...
    //: private var imageTotalData = ChatCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = ChatCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = TimeExposureModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: ChatCellData, toUid: String) {
    init(imageCellData: ChatCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(DistanceAppManager.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(DistanceAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kActualVideoText.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.submitGift(name: (k_tempFormat.replacingOccurrences(of: "bubble", with: "c") + String(kMakePlayerMessage)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(k_cookieLastKey.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(forwardFor), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.submitGift(name: (String(kErrorViewFormat.prefix(5)) + kInfoFormat.replacingOccurrences(of: "top", with: "k") + String(kGiftPath.prefix(4)) + "ck_nor")).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(viewFor), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension BringForthViewController {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func atFreebee(giftId: String, giftNum: String) {
        ...
        params[(String(kModelName.prefix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(kShareLayerMsg.prefix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(kBlankNameId.prefix(6)) + kObserverName.replacingOccurrences(of: "load", with: "m"))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(kViewMessage.replacingOccurrences(of: "arc", with: "lo") + "ckMsgId")] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension BringForthViewController {
    ...
    @objc private func forwardFor() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        atFreebee(giftId: photoModel.giftId, giftNum: "1")
    }
    ...
    @objc private func mention() {
        ...
        self.slideError(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.pose(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension BringForthViewController {
    ...
    private func viewThen() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: kHeadFormat.map{labelCenterAll(index: $0)}, encoding: .utf8)!.edgeDelay(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.submitGift(name: (kListId.replacingOccurrences(of: "application", with: "ic") + "_pho" + String(k_effectFailureMessage)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: kNameValue.reversed(), encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(mention))
                ...
                self.intimateStatusImgV.image = UIImage.submitGift(name: (String(kColorMessage.suffix(5)) + k_netManagerStr.replacingOccurrences(of: "lovely", with: "ph") + "_burn"))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(kFromUrl) + String(k_addKey)).localized
            }
        ...
        self.slideError(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func slideError(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
