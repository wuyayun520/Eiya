...
/*: "uid" :*/
fileprivate let k_photoContent:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let k_textName:String = "ONLIN"

/*: "isNewUser" :*/
fileprivate let kBlockValue:String = "isNewlive array view false"

/*: "isTPAuth" :*/
fileprivate let k_withMsg:String = "isTthe for following in layer"
fileprivate let k_lifeName:String = "let sharePAuth"

/*: "totalIntimate" :*/
fileprivate let k_modelChangeId:String = "totalItap price strength"
fileprivate let kEqualUrl:String = "TE"

/*: "userStatus" :*/
fileprivate let k_toId:String = "white examine count returnuserSt"
fileprivate let k_stateId:[Character] = ["a","t","u","s"]

/*: "intimate" :*/
fileprivate let kCurrentId:[Character] = ["i","n","t","i","m","a"]
fileprivate let kKitText:String = "tmanager"
...
class CartListManager {
    ...
    private var moreMsgModel: ViewConversationModel = //: return ViewConversationModel.getPlaceholderModel(chatType: .moreMsg)
        .toForm(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func anyEqual() {
        //: self.officialArr = ResumeV2Listener.shared.getOfficialList()
        self.officialArr = ResumeV2Listener.shared.statusEqual()
        //: self.topArr = ResumeV2Listener.shared.topConvList
        self.topArr = ResumeV2Listener.shared.topConvList
        //: self.normalArr = ResumeV2Listener.shared.norConvList
        self.normalArr = ResumeV2Listener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        anTableData()
    ...
    func fromToCompletion(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: ResumeV2Listener.shared.getChatRoomData { succeed in
        ResumeV2Listener.shared.modelView { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = ResumeV2Listener.shared.chatRoomList
                self.chatRoomArr = ResumeV2Listener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func voicePath(indexPath: IndexPath) -> ViewConversationModel? {
    ...
    func loadFor(model: ViewConversationModel?) -> ViewConversationModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: HumankindThen? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: HumankindThen? = AppNameThen.imageBar(targetId: targetId)
    ...
    func upwardlyFinish(conversationList: [ViewConversationModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        AppNameThen.secretFrom(with: tempArr) { _, error in
    ...
    func sharedEqual(conversationModel: ViewConversationModel) {
    ...
    func bitCompletion(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: ResumeV2Listener.shared.func__getConversationList { arr, leftMore in
        ResumeV2Listener.shared.sharedImage { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
...
extension CartListManager {
    /// 初始化数据
    //: func req_moreMsgInitData() {
    func anTableData() {
        //: self.moreMsgArr = ResumeV2Listener.shared.moreMsgList
        self.moreMsgArr = ResumeV2Listener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func someResponse(indexPath: IndexPath) -> ViewConversationModel? {
    ...
    func nameModel() -> ViewConversationModel {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        anTableData()
        ...
        moreMsgModel.unreadCount = moreToEarnedRun()
...
extension CartListManager {
    ...
    func titleInfo(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        AppNameThen.getBy(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(k_photoContent))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(k_textName.lowercased() + "eStatus")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(kBlockValue.prefix(5)) + "User")].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(k_withMsg.prefix(3)) + String(k_lifeName.suffix(5)))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(k_modelChangeId.prefix(6)) + "ntima" + kEqualUrl.lowercased())].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(k_toId.suffix(6)) + String(k_stateId))].int ?? 1
                    ...
                    self.inputChange(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func upInfo(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(k_photoContent))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(k_textName.lowercased() + "eStatus")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(kBlockValue.prefix(5)) + "User")] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(k_toId.suffix(6)) + String(k_stateId))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(k_toId.suffix(6)) + String(k_stateId))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension CartListManager {
    ...
    func pastTitleInstance() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in ResumeV2Listener.shared.norConvList {
        for model in ResumeV2Listener.shared.norConvList {
            //: if model.totalIntimate >= DistanceAppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= DistanceAppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.loadFor(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in ResumeV2Listener.shared.topConvList {
        for model in ResumeV2Listener.shared.topConvList {
            //: if model.totalIntimate >= DistanceAppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= DistanceAppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func onTop(convModel: ViewConversationModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func positionCountimate(indexPath: IndexPath) -> ViewConversationModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func inputChange(uid: String, intimate: Int) {
        ...
        AppNameThen.equalBlock(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func sleepingTablet(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(k_photoContent))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(String(kCurrentId) + kKitText.replacingOccurrences(of: "manager", with: "e"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
