...
/*: "gifts" :*/
fileprivate let kBottomLiveKey:String = "GIFTS"

/*: "vipGifts" :*/
fileprivate let kValuePath:[Character] = ["v","i","p","G","i"]
fileprivate let k_topPath:[Character] = ["f","t","s"]

/*: "packGifts" :*/
fileprivate let kUserPath:[Character] = ["p","a","c"]
fileprivate let kRestrictionUrl:String = "kGiftsself equal fatal"

/*: "giftSendItem" :*/
fileprivate let k_bottomErrorId:String = "giftSimage attribute"
fileprivate let k_byFileMsg:String = "terra textendItem"
...
class AppNameGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [AddSubscriptType: [Any]]]()
    // 数据锁
    ...
    private var giftType = FrameCollection.normal

    //: static let share = TalkingChatGiftManager()
    static let share = AppNameGiftManager()
...
extension AppNameGiftManager {
    ...
    func searchCompletion(type: FrameCollection = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        AppNameGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[AddSubscriptType.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            AppNameGiftManager.share.addCompletion(completion: { isSucceed in
            ...
            AppNameGiftManager.share.actionCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            AppNameGiftManager.share.startCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            AppNameGiftManager.share.fullUp(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func fullUp(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = AppNameGiftManager.share.giftType
        //: PlayerRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        PlayerRequestTool.typeWill(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [AddSubscriptType: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(kBottomLiveKey.lowercased())] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<RangeMeasurable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[AddSubscriptType.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(kValuePath) + String(k_topPath))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<RangeMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[AddSubscriptType.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(kUserPath) + String(kRestrictionUrl.prefix(6)))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<RangeMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[AddSubscriptType.Bags] = bagsArr
                }
                ...
                var firstModel = PublishHandyJSON()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(k_bottomErrorId.prefix(5)) + String(k_byFileMsg.suffix(7)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<PublishHandyJSON>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func addCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = AppNameGiftManager.share.giftType
        //: PlayerRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        PlayerRequestTool.slide(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func messageMake(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: PlayerRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        PlayerRequestTool.cameraCommit(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<RunOutMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func actionCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = FrameCollection.live
        //: PlayerRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        PlayerRequestTool.afterWith(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func startCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = FrameCollection.party
        //: PlayerRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        PlayerRequestTool.viewShow(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func per(perPageCount: Int, dataType: AddSubscriptType) -> Int {
    ...
    func sharedType(dataType: AddSubscriptType) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func aggregationPush(dataType: AddSubscriptType, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension AppNameGiftManager {
    ...
    private func cryFor() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(actThroughSheaf),
    ...
    @objc private func actThroughSheaf() {
        //: gift_clearMemoryCache(showRed: true)
        pinData(showRed: true)
    }
}
...
extension AppNameGiftManager {
    ...
    func pinData(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        AppNameGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
