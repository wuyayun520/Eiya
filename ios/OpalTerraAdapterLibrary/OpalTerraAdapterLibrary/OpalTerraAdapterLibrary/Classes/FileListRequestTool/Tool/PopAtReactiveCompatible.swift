...
fileprivate let kTargetStr:[UInt8] = [0x6c,0x6c,0x61,0x63,0x20,0x61,0x20,0x67,0x6e,0x69,0x72,0x75,0x64,0x20,0x65,0x6c,0x62,0x61,0x6c,0x69,0x61,0x76,0x61,0x20,0x74,0x6f,0x6e,0x20,0x73,0x69,0x20,0x65,0x72,0x75,0x74,0x61,0x65,0x66,0x20,0x73,0x69,0x68,0x54]

/*: "wav" :*/
fileprivate let k_modelText:String = "wnumberv"
...
protocol PitchDoingThen: NSObject {
    /// 录音时间
...
enum EquationViewSubscriptType: Int {
    //: case PlayerFinish = 0
...
class PopAtReactiveCompatible: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: PitchDoingThen?
    ...
    static let shared = PopAtReactiveCompatible()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(notificationKey(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func get(path: NSString) -> String {
        //: if !FileManager.default.fileExists(atPath: Talking_Chat_record_SandboxPath) {
        if !FileManager.default.fileExists(atPath: user_methodData) {
            //: do {
            do {
                ...
                try FileManager.default.createDirectory(atPath: user_methodData, withIntermediateDirectories: true, attributes: nil)
                //: }catch {
            } catch {}
        }
        ...
        let timestr = NSDate.timeIntervalSinceReferenceDate
        //: let uid = DistanceAppManager.share.loginUserMode.userID
        let uid = DistanceAppManager.share.loginUserMode.userID
        //: if uid.count == 0 { return "" }
        if uid.count == 0 { return "" }

        //: return Talking_Chat_record_SandboxPath + "_" + uid + "_" + String(Int(timestr)) + "." + (path as String)
        return user_methodData + "_" + uid + "_" + String(Int(timestr)) + "." + (path as String)
    }
}

//: extension TalkingAudioTool: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
extension PopAtReactiveCompatible: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func notificationKey(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.arcInfo(showMsg: String(bytes: kTargetStr.reversed(), encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.thanPath(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.listStatus(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.listStatus(status: .PlayerFinish)
    }
}

//: extension TalkingAudioTool {
extension PopAtReactiveCompatible {
    //: func BeginRecord() {
    func teamGetMovingRecord() {
        //: deleteFile()
        sendFile()
        ...
        let path = get(path: (k_modelText.replacingOccurrences(of: "number", with: "a")) as NSString)
    ...
    func afterFinishLab() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(parameter(time:)), userInfo: nil, repeats: true)
    ...
    @objc func parameter(time _: Timer) {
        //: recorder?.updateMeters()
        recorder?.updateMeters()
        ...
        self.delegate?.pull(currentTime: interval)
    }
    ...
    func zoneEqual() {
        ...
        self.delegate?.actual(currentTime: currentTime)
    }
    ...
    func actionTimer() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(openBlock(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
    ...
    @objc func openBlock(time _: Timer) {
        //: if self.player != nil {
        if self.player != nil {
