...
fileprivate let kValueId:[UInt8] = [0x9a,0x9d,0x9a,0x87,0xdb,0x90,0x9c,0x97,0x96,0x81,0xc9,0xda,0xd3,0x9b,0x92,0x80,0xd3,0x9d,0x9c,0x87,0xd3,0x91,0x96,0x96,0x9d,0xd3,0x9a,0x9e,0x83,0x9f,0x96,0x9e,0x96,0x9d,0x87,0x96,0x97]

/*: "Add more details of yourself" :*/
fileprivate let k_postStr:String = "data total return class rowAdd "
fileprivate let kMakeUrl:String = "time bottom type model deta"
fileprivate let kButtonContent:String = "of yself table stream"
fileprivate let k_pathViewName:String = "elscreen"

/*: "#999999" :*/
fileprivate let k_cosKey:[Character] = ["#","9","9","9","9","9"]
fileprivate let k_mStr:[Character] = ["9"]

/*: "0/500" :*/
fileprivate let k_cellName:String = "0/500"

/*: "Tip:Don't post content that induces others to send gifts or other money-relatsed content." :*/
fileprivate let kArrayContent:[UInt8] = [0xad,0x90,0x89,0xc3,0xbd,0x96,0x97,0xde,0x8d,0xd9,0x89,0x96,0x8a,0x8d,0xd9,0x9a,0x96,0x97,0x8d,0x9c,0x97,0x8d,0xd9,0x8d,0x91,0x98,0x8d,0xd9,0x90,0x97,0x9d,0x8c,0x9a,0x9c,0x8a,0xd9,0x96,0x8d,0x91,0x9c,0x8b,0x8a,0xd9,0x8d,0x96,0xd9,0x8a,0x9c,0x97,0x9d,0xd9,0x9e,0x90,0x9f,0x8d,0x8a,0xd9,0x96,0x8b,0xd9,0x96,0x8d,0x91,0x9c,0x8b,0xd9,0x94,0x96,0x97,0x9c,0x80,0xd4,0x8b,0x9c,0x95,0x98,0x8d,0x8a,0x9c,0x9d,0xd9,0x9a,0x96,0x97,0x8d,0x9c,0x97,0x8d,0xd7]

private func makeAdd(user num: UInt8) -> UInt8 {
    return num ^ 249
}

/*: / :*/
fileprivate let kBagData:[Character] = ["/"]

/*: "\n" :*/
fileprivate let k_modelMessage:String = "\n"
...
class StreamCoverViewCell: UITableViewCell {
    //: let limitCount = 500
    let limitCount = 500
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(notificationBy(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kValueId.map{$0^243}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(k_postStr.suffix(4)) + "more" + String(kMakeUrl.suffix(5)) + "ils " + String(kButtonContent.prefix(4)) + "ours" + k_pathViewName.replacingOccurrences(of: "screen", with: "f")).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (String(k_cosKey) + String(k_mStr)))
        //: LB.font = UIFont.fontChange(type: .Regular, fontSize: 14)
        LB.font = UIFont.fontChange(type: .Regular, fontSize: 14)
        //: LB.text = "0/500"
        LB.text = (k_cellName.capitalized)
    ...
    lazy var textView: BreakUpTextView = {
        ...
        //: let sign = DistanceAppManager.share.loginUserMode.signature
        let sign = DistanceAppManager.share.loginUserMode.signature
        //: textView.text = sign
        textView.text = sign
        //: textView.placeholder = "Tip:Don't post content that induces others to send gifts or other money-relatsed content.".localized
        textView.placeholder = String(bytes: kArrayContent.map{makeAdd(user: $0)}, encoding: .utf8)!.localized
        //: textView.placeholderColor = UIColor.appValueDetailColor()
        textView.placeholderColor = UIColor.childMenu()
...
extension StreamCoverViewCell {
    ...
    @objc func notificationBy(notification: NSNotification) {
...
extension StreamCoverViewCell: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > limitCount {
            ...
            if textNum > limitCount && limitCount > 0 {
                ...
                textView.text = someFactor(index: limitCount, text: textContent)
            }
        }
        //: numberLB.text =  "\(textView.text.count)/\(limitCount)"
        numberLB.text = "\(textView.text.count)/\(limitCount)"
    ...
    private func someFactor(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
