...
fileprivate let k_filterFormat:[UInt8] = [0x7e,0x83,0x7e,0x89,0x3d,0x78,0x84,0x79,0x7a,0x87,0x4f,0x3e,0x35,0x7d,0x76,0x88,0x35,0x83,0x84,0x89,0x35,0x77,0x7a,0x7a,0x83,0x35,0x7e,0x82,0x85,0x81,0x7a,0x82,0x7a,0x83,0x89,0x7a,0x79]

fileprivate func popHome(succeed num: UInt8) -> UInt8 {
...
/*: "Add" :*/
fileprivate let k_actionAccountValue:[Character] = ["A","d","d"]

/*: "btn_me_edit" :*/
fileprivate let k_touchMessage:String = "layer labelbtn_"
fileprivate let k_indexAppName:String = "icollection"

/*: "Program" :*/
fileprivate let kMakeId:[Character] = ["P","r","o","g","r","a","m"]
...
// __CLOSE_PRINT__
//
//  MenuResumeReactiveCompatible.swift
//  AbroadTalking
//
...
class MenuResumeReactiveCompatible: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        numbererchangeMenuConstraint()
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: k_filterFormat.map{popHome(succeed: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(backToTalkClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLab: UILabel = {
        ...
        lb.text = (String(k_actionAccountValue)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var editImg: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.submitGift(name: (String(k_touchMessage.suffix(4)) + "me_ed" + k_indexAppName.replacingOccurrences(of: "collection", with: "t"))))
        //: imag.isUserInteractionEnabled = false
        imag.isUserInteractionEnabled = false
        //: return imag
        return imag
...
extension MenuResumeReactiveCompatible {
    //: func setViewData(title: String, icon: String) {
    func arrayAll(title: String, icon: String) {
        //: iconImg.image = UIImage.submitGift(name: icon)
        iconImg.image = UIImage.submitGift(name: icon)
        //: titleLab.text = title.localized
        titleLab.text = title.localized
        //: if title == "Program" {
        if title == (String(kMakeId)) {
    ...
    func readArrow() {
        ...
        backBtn.imageUser(width: constFormalContent - 30, height: 50, corners: [.topLeft, .topRight], cornerRadii: .init(width: 8, height: 8))
    }
}
...
extension MenuResumeReactiveCompatible {
    ...
    private func numbererchangeMenuConstraint() {
        //: backBtn.snp.remakeConstraints { make in
        backBtn.snp.remakeConstraints { make in
            //: make.leading.top.equalToSuperview().offset(15)
            make.leading.top.equalToSuperview().offset(15)
