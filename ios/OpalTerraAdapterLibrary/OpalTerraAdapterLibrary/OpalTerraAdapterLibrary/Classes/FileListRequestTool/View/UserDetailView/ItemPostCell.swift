...
fileprivate let kToShareMsg:[UInt8] = [0x7d,0x82,0x7d,0x88,0x3c,0x77,0x83,0x78,0x79,0x86,0x4e,0x3d,0x34,0x7c,0x75,0x87,0x34,0x82,0x83,0x88,0x34,0x76,0x79,0x79,0x82,0x34,0x7d,0x81,0x84,0x80,0x79,0x81,0x79,0x82,0x88,0x79,0x78]

fileprivate func centerData(leading num: UInt8) -> UInt8 {
...
/*: "More" :*/
fileprivate let k_rawMessage:String = "list data target height tabMore"

/*: "icon_details_arrow" :*/
fileprivate let k_imageManualKey:String = "icviewn"
fileprivate let k_modelMessage:[Character] = ["i","l","s","_","a","r","r","o","w"]

/*: " (%ld)" :*/
fileprivate let kToErrorUrl:String = "color hidden bottom (%ld)"

/*: "Post" :*/
fileprivate let k_rangeFormat:[Character] = ["P","o","s","t"]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let kResidenceMessage:String = "request time make namebtn_d"
fileprivate let k_labelFilePath:String = "let phone normalic_sto"
fileprivate let k_rawPathTopData:[Character] = ["p","_","n","o","r"]
...
class ItemPostCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kToShareMsg.map{centerData(leading: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .behindVoice(0, 0, 0, 0.4)
    ...
    lazy var moreButton: ResumeControl = .init().then {
        //: $0.titleLabel?.font = .fontChange(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .fontChange(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(k_rawMessage.suffix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (k_imageManualKey.replacingOccurrences(of: "view", with: "o") + "_deta" + String(k_modelMessage))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(failing), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ItemPostCell {
    //: private func bindInteraction() {
    private func sinceCounteraction() {}
}
...
extension ItemPostCell {
    ...
    func contentTag(userModel: GroupTransformable) {
        ...
        strA = [(String(k_rangeFormat)).localized, str]
        //: colorA = [UIColor.colorStatuteNameApp(), UIColor.appValueColor()]
        colorA = [UIColor.colorStatuteNameApp(), UIColor.scaleValueTo()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.tapShared(fontSize: 18), UIFont.tapShared(fontSize: 16)]
        ...
        attributedString = attributedString.publicTransport(texts: strA, colors: colorA, fonts: fontA)
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.rectangle())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    enable(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.rectangle())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    enable(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.rectangle())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    enable(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func failing() {
        ...
        let vc = SizeDataSource(uid: userId)
    ...
    func enable(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(kResidenceMessage.suffix(5)) + "ynam" + String(k_labelFilePath.suffix(6)) + String(k_rawPathTopData))))
...
extension ItemPostCell {
    ...
    private func key() {
        ...
        let height = (constFormalContent - actualWidth(w: 15 * 4)) / 3.0
