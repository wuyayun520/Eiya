...
fileprivate let k_fromUrl:[UInt8] = [0x5e,0x59,0x5e,0x43,0x1f,0x54,0x58,0x53,0x52,0x45,0xd,0x1e,0x17,0x5f,0x56,0x44,0x17,0x59,0x58,0x43,0x17,0x55,0x52,0x52,0x59,0x17,0x5e,0x5a,0x47,0x5b,0x52,0x5a,0x52,0x59,0x43,0x52,0x53]

private func norTo(length num: UInt8) -> UInt8 {
    return num ^ 55
}

/*: "Gifts received" :*/
fileprivate let k_imageValue:String = "next typeGifts "
fileprivate let k_userFormat:String = "reexpressived"
...
// __CLOSE_PRINT__
//
//  ChecklistReactiveCompatible.swift
//  AbroadTalking
//
...
class ChecklistReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_fromUrl.map{norTo(length: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLab: UILabel = {
        ...
        lb.text = (String(k_imageValue.suffix(6)) + k_userFormat.replacingOccurrences(of: "express", with: "ce")).localized
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(StartViewCell.self, forCellWithReuseIdentifier: StartViewCell.className())
        //: return collect
        return collect
        //: }()
    }()
...
extension ChecklistReactiveCompatible: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let model: CoverDoingHandyJSON = self.giftsDataArray[indexPath.row]
        //: cell.configGiftItemCellWithData(giftModel: model, isNewProfile)
        cell.makeView(giftModel: model, isNewProfile)
        //: return cell
        return cell
    }
...
extension ChecklistReactiveCompatible {
