...
/*: "#AAAAAA" :*/
fileprivate let kStyleValue:String = "#AAAAAAframe intimate as raw"

/*: " No more than 10 characters" :*/
fileprivate let k_trackMsg:[Character] = [" ","N","o"," ","m","o","r","e"," ","t","h","a"]
fileprivate let k_colorCenterId:[Character] = ["n"," ","1","0"," ","c","h","a","r","a","c","t","e","r","s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let kSizeLineTitle:[UInt8] = [0xe6,0xe1,0xe6,0xfb,0xa7,0xec,0xe0,0xeb,0xea,0xfd,0xb5,0xa6,0xaf,0xe7,0xee,0xfc,0xaf,0xe1,0xe0,0xfb,0xaf,0xed,0xea,0xea,0xe1,0xaf,0xe6,0xe2,0xff,0xe3,0xea,0xe2,0xea,0xe1,0xfb,0xea,0xeb]

private func loadFile(record num: UInt8) -> UInt8 {
    return num ^ 143
}
...
class WillViewDelegate: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: VideoAlertConfig
    ...
    private lazy var inputTView: SignalingScrollView = {
        //: let input = InputTextView()
        let input = SignalingScrollView()
        //: input.font = UIFont.fontChange(type: .Regular, fontSize: 16)
        input.font = UIFont.fontChange(type: .Regular, fontSize: 16)
        //: input.placeholderColor = .init(hex: "#AAAAAA") ?? UIColor.gray
        input.placeholderColor = .init(hex: (String(kStyleValue.prefix(7)))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(k_trackMsg) + String(k_colorCenterId)).localized
        ...
        //: input.delegate = self
        input.delegate = self
        //: input.backgroundColor = .postColor()
        input.backgroundColor = .postColor()
        //: input.textColor = UIColor.colorStatuteNameApp()
        input.textColor = UIColor.colorStatuteNameApp()
        //: let attributes = [NSAttributedString.Key.foregroundColor: UIColor.colorStatuteNameApp(), .font: UIFont.fontChange(type: .Regular, fontSize: 16)]
        let attributes = [NSAttributedString.Key.foregroundColor: UIColor.colorStatuteNameApp(), .font: UIFont.fontChange(type: .Regular, fontSize: 16)]
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.increase().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(on), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.makeEqual(colors: UIColor.georgiaHomeBoy(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.makeEqual(colors: UIColor.actionPhoto(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(fullPhaseOfTheMoon), for: .touchUpInside)
    ...
    init(title: String?,
         ...
         config: VideoAlertConfig?)
    {
        //: let model = config ?? ShowAlertConfig.init()
        let model = config ?? VideoAlertConfig()
        //: alertConfig = model
        alertConfig = model
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kSizeLineTitle.map{loadFile(record: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func thenModel(title: String, isTouch: Bool) {
        ...
        //: if !isTouch {
        if !isTouch {
            //: rightBtn.setBackgroundImage(UIImage.submitGift(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            rightBtn.setBackgroundImage(UIImage.submitGift(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.submitGift(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.submitGift(name: alertConfig.rightBackgroundImage).withTintColor(.increase())
...
extension WillViewDelegate: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func lea(_ textField: UITextField, shouldChangeCharactersIn _: NSRange, replacementString string: String) -> Bool {
    ...
    func everyScale(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = underArray(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func underArray(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
