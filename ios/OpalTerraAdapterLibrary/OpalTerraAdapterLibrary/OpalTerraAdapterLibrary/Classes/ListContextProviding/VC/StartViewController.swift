...
class StartViewController: StartThen {
    //: var type = LoginType.PhoneLogin
    var type = PalmClusterLiteral.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: GolfPlayerView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = GolfPlayerView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension StartViewController {
    ...
    private func withPwd(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        IngatheringPullReactiveCompatible.gift(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
    ...
    private func line() {
        ...
        IngatheringPullReactiveCompatible.along { [weak self] succeed, _, _ in
            //: EndProgressHUD.popEnable()
            EndProgressHUD.popEnable()
            //: guard let self = self else { return }
            guard let self = self else { return }
