...
fileprivate let k_infoLabErrorName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "8075F5" :*/
fileprivate let kRetValue:String = "8075F5camera self"

/*: "9A9B9C" :*/
fileprivate let kMakeId:String = "9"
fileprivate let k_listId:String = "A9whiteC"

/*: "kOpacityAnimation" :*/
fileprivate let k_detailTitle:[Character] = ["k","O","p","a","c"]
fileprivate let kPerspectiveFormat:String = "ityAnself view"
fileprivate let kReasonMsg:[Character] = ["i","m","a","t","i","o","n"]

/*: "opacity" :*/
fileprivate let kQuantityFormat:String = "opacgiftty"

/*: "forwards" :*/
fileprivate let k_playId:[Character] = ["f","o","r","w","a"]
fileprivate let kFaceKey:[Character] = ["r","d","s"]

/*: "easeIn" :*/
fileprivate let k_textValue:String = "table beauty makeeaseI"
fileprivate let kViewName:String = "index"

/*: "8C7AF8" :*/
fileprivate let kNorFormat:String = "for icon8C7AF8"
...
// __CLOSE_PRINT__
//
//  MenuEndReactiveCompatible.swift
//  AbroadTalking
//
...
class MenuEndReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_infoLabErrorName.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(remote(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension MenuEndReactiveCompatible {
    ...
    func clickAll() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(kRetValue.prefix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (kMakeId.capitalized + k_listId.replacingOccurrences(of: "white", with: "B9")))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func alongData(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(k_detailTitle) + String(kPerspectiveFormat.prefix(5)) + String(kReasonMsg)))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(eliteTarget(), forKey: (String(k_detailTitle) + String(kPerspectiveFormat.prefix(5)) + String(kReasonMsg)))
        }
    ...
    fileprivate func eliteTarget() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (kQuantityFormat.replacingOccurrences(of: "gift", with: "i")))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (String(k_playId) + String(kFaceKey)))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(k_textValue.suffix(5)) + kViewName.replacingOccurrences(of: "index", with: "n"))))
        //: return animation
        return animation
    }
}
...
extension MenuEndReactiveCompatible {
    ...
    private func create() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(kNorFormat.suffix(6))))?.cgColor
    ...
    private func expanseConstraint() {}

    // 添加事件
    //: private func bindInteraction() {
    private func behindBirth() {}
}
