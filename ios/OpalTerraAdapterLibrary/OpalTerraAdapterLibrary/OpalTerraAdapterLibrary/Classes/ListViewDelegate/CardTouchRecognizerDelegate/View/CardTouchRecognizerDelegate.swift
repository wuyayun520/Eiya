...
fileprivate let kWithUrl:[UInt8] = [0xad,0xb2,0xad,0xb8,0x6c,0xa7,0xb3,0xa8,0xa9,0xb6,0x7e,0x6d,0x64,0xac,0xa5,0xb7,0x64,0xb2,0xb3,0xb8,0x64,0xa6,0xa9,0xa9,0xb2,0x64,0xad,0xb1,0xb4,0xb0,0xa9,0xb1,0xa9,0xb2,0xb8,0xa9,0xa8]

fileprivate func statusModelManager(fatal num: UInt8) -> UInt8 {
...
// __CLOSE_PRINT__
//
//  CardTouchRecognizerDelegate.swift
//  AbroadTalking
//
...
enum SingleSwipeDirection: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol PullCellDelegete: NSObjectProtocol {
    ///
...
class CardTouchRecognizerDelegate: UIView {
    ...
    var cell_delegate: PullCellDelegete?

    //: var model: TalkingMatchResultModel?
    var model: ValueModelType?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kWithUrl.map{statusModelManager(fatal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(matrix(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension CardTouchRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    @objc func matrix(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if MakeLanguageManager.shared.direction == .rightToLeft {
                ...
                if movePoint.x > constFormalContent / 4 {
                    ...
                    self.cell_delegate?.equalBind(cell: self, forMovePoint: self.currentPoint)
    ...
    func roundSource(direction: SingleSwipeDirection) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
...
extension CardTouchRecognizerDelegate {
    //: func setupView() {
    func beginView() {
