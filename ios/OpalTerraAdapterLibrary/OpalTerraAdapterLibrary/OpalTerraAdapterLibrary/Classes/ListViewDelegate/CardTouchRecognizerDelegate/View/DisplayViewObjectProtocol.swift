...
fileprivate let kZoneStr:[UInt8] = [0xd1,0xd6,0xd1,0xcc,0x90,0xdb,0xd7,0xdc,0xdd,0xca,0x82,0x91,0x98,0xd0,0xd9,0xcb,0x98,0xd6,0xd7,0xcc,0x98,0xda,0xdd,0xdd,0xd6,0x98,0xd1,0xd5,0xc8,0xd4,0xdd,0xd5,0xdd,0xd6,0xcc,0xdd,0xdc]

/*: "img_home_shadow" :*/
fileprivate let k_modelName:[Character] = ["i","m","g","_","h"]
fileprivate let kWillPath:[Character] = ["o","m","e"]
fileprivate let k_equalTitle:[Character] = ["_","s","h","a","d","o","w"]

/*: "#9610FF" :*/
fileprivate let kSizeData:String = "#9610FFuser ad model frame"

/*: "#8566FF" :*/
fileprivate let k_imageTitle:[Character] = ["#","8","5","6","6","F"]
fileprivate let k_managerTitle:String = "gift"

/*: "Random Video" :*/
fileprivate let kActualUrl:[Character] = ["R","a","n","d","o","m"," ","V","i","d","e"]
fileprivate let kSharedName:String = "user"

/*: "icon_match_call_free" :*/
fileprivate let k_shareMsg:[Character] = ["i","c","o","n","_","m","a","t","c","h","_","c","a","l","l","_","f","r"]
fileprivate let k_withPath:[Character] = ["e","e"]

/*: "Free" :*/
fileprivate let kColorNameText:String = "var userFree"

/*: "Scroll_upto_discove" :*/
fileprivate let k_viewValue:String = "Scrollmodel else"
fileprivate let k_bottomTopName:String = "dismanagerov"
fileprivate let kVideoMsg:String = "share"

/*: "icon_VectorStroke" :*/
fileprivate let k_centerId:String = "icocontent"
fileprivate let kKeyPath:String = "orStlab in view let"

/*: "get json error" :*/
fileprivate let kGiftMessage:String = "close video equalget json"
fileprivate let kDomainData:[Character] = [" ","e","r","r","o","r"]
...
// __CLOSE_PRINT__
//
//  DisplayViewObjectProtocol.swift
//  AbroadTalking
//
...
class DisplayViewObjectProtocol: UIView {
    ...
    var maxRemoveDistance = constFormalContent / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [CardTouchRecognizerDelegate]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: SkimObjectProtocol? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: ModelObjectProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kZoneStr.map{$0^184}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.contentMode = .scaleAspectFill
        //: imag.image = UIImage.submitGift(name: "img_home_shadow")
        imag.image = UIImage.submitGift(name: (String(k_modelName) + String(kWillPath) + String(k_equalTitle)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.makeEqual(colors: [UIColor(hex: (String(kSizeData.prefix(7))))!.cgColor, UIColor(hex: (String(k_imageTitle) + k_managerTitle.replacingOccurrences(of: "gift", with: "F")))!.cgColor], size: CGSize(width: constFormalContent - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(kActualUrl) + kSharedName.replacingOccurrences(of: "user", with: "o")).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(allowErgodic), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        //: let img = UIButton()
        let img = UIButton()
        //: img.setBackgroundImage(UIImage.submitGift(name: "icon_match_call_free"), for: .normal)
        img.setBackgroundImage(UIImage.submitGift(name: (String(k_shareMsg) + String(k_withPath))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(kColorNameText.suffix(4))).localized, for: .normal)
...
extension DisplayViewObjectProtocol {
    //: func dismissGuide() {
    func anyTick() {
        //: if guideView != nil {
        if guideView != nil {
    ...
    private func beacon() {
        ...
        image.contentMode = .scaleAspectFill
        //: image.image = UIImage.submitGift(name: "Scroll_upto_discove")
        image.image = UIImage.submitGift(name: (String(k_viewValue.prefix(6)) + "_upto_" + k_bottomTopName.replacingOccurrences(of: "manager", with: "c") + kVideoMsg.replacingOccurrences(of: "share", with: "e")))
        ...
        image2.contentMode = .scaleAspectFill
        //: image2.image = UIImage.submitGift(name: "icon_VectorStroke")
        image2.image = UIImage.submitGift(name: (k_centerId.replacingOccurrences(of: "content", with: "n") + "_Vect" + String(kKeyPath.prefix(4)) + "roke"))
        ...
        liveTogether(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func liveTogether(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = VideoEffectTool.default.conType(type: .Club_guidePage)
            ...
            printLog(message: (String(kGiftMessage.suffix(8)) + String(kDomainData)))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func allowErgodic() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.cellWith?()
    }
    ...
    func re(anim: Bool) {
        ...
        let maxCount = dataSource?.listView(cardView: self) ?? 0
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? CardTouchRecognizerDelegate(), at: 0)
    ...
    func executiveSession(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: CardTouchRecognizerDelegate = self.visibleCells[i] as! CardTouchRecognizerDelegate
            ...
            if isVisbleLast {
                ...
                self.delegate?.connect?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                appearViewPlace(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func appearViewPlace(cell: CardTouchRecognizerDelegate, transform: CGAffineTransform) {
    ...
    func clickOrderedSeriesInde(inde: Int) -> Int {
    ...
    func pushOf(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func reactCenter(identifier: String) -> CardTouchRecognizerDelegate? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! HorizonThen.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: HorizonThen = cellclass.init(reuseIdentifier: identifier)
    ...
    func fromMargin(index: Int) -> CardTouchRecognizerDelegate {
        //: let visibleIndex = visibleIndex(inde: index)
        let visibleIndex = clickOrderedSeriesInde(inde: index)
        //: let cell = self.visibleCells[visibleIndex]
        let cell = self.visibleCells[visibleIndex]
        //: return cell as! TalkingClubCardViewCell
        return cell as! CardTouchRecognizerDelegate
    }

    /// 获取cell对应的index
    //: func indexForCell(cell: TalkingClubCardViewCell) -> Int {
    func safetyMarginWithinCellImage(cell: CardTouchRecognizerDelegate) -> Int {
        //: let arr = self.visibleCells as? [TalkingClubCardViewCell]
        let arr = self.visibleCells as? [CardTouchRecognizerDelegate]
...
extension DisplayViewObjectProtocol: PullCellDelegete {
    ...
    func disappearAcross(cell: CardTouchRecognizerDelegate) {
        ...
        self.delegate?.conversationIndex?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.listView(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.destroyData?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            nextBoard(index: self.currentIndex + 1)
        }
        // 更新布局
        //: updateLayoutVisibleCells(anim: true)
        executiveSession(anim: true)
    }

    //: func cardViewCellDidMoveFromSuperView(cell: TalkingClubCardViewCell, forMovePoint: CGPoint) {
    func equalBind(cell: CardTouchRecognizerDelegate, forMovePoint: CGPoint) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.viewSight?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
...
extension DisplayViewObjectProtocol {
    ...
    func voiceBtn() {
        ...
        let index = NameBinaryCount.randow
