...
fileprivate let kOfUrl:[UInt8] = [0x7b,0x80,0x7b,0x86,0x3a,0x75,0x81,0x76,0x77,0x84,0x4c,0x3b,0x32,0x7a,0x73,0x85,0x32,0x80,0x81,0x86,0x32,0x74,0x77,0x77,0x80,0x32,0x7b,0x7f,0x82,0x7e,0x77,0x7f,0x77,0x80,0x86,0x77,0x76]

fileprivate func buttonActual(section num: UInt8) -> UInt8 {
...
/*: "gift_bg_dm" :*/
fileprivate let kCenterValueName:String = "gon"
fileprivate let kLittleMsg:String = "count value color colorft_b"

/*: "#FFF4B5" :*/
fileprivate let kMiniId:[Character] = ["#","F","F","F","4","B","5"]

/*: "won " :*/
fileprivate let k_bindMakePath:String = "from message userwon "

/*: " coins in " :*/
fileprivate let k_videoName:String = "to click self coin"
fileprivate let kSendTitle:String = "gender errors in "

/*: " x :*/
fileprivate let kRequestStr:String = "gift will mode x"

/*: "just sent" :*/
fileprivate let kViewId:[Character] = ["j","u","s"]
fileprivate let k_arrayData:[Character] = ["t"," ","s","e","n","t"]

/*: "  :*/
fileprivate let kStatuteKey:[Character] = [" "]

/*: "#4120C1" :*/
fileprivate let kFileText:String = "let self image equal#4120C1"
...
class DataFloatView: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = DisplayHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kOfUrl.map{buttonActual(section: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        //: let v = UIImageView()
        let v = UIImageView()
        //: v.image = UIImage.submitGift(name: "gift_bg_dm")
        v.image = UIImage.submitGift(name: (kCenterValueName.replacingOccurrences(of: "on", with: "i") + String(kLittleMsg.suffix(4)) + "g_dm"))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(kMiniId)))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(kMiniId)))
...
extension DataFloatView {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func actGift(_ model: DisplayHandyJSON) {
        //: giftModel = model
        giftModel = model
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(k_bindMakePath.suffix(4))).localized + "\(giftModel.totalCoin)" + (String(k_videoName.suffix(5)) + String(kSendTitle.suffix(5))).localized + giftModel.gameName
            ...
            let rangeText = (String(kViewId) + String(k_arrayData)).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = scan(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(kFileText.suffix(7))))!, range: nsRange)
            }
    ...
    private func scan(giftmodel: DisplayHandyJSON) -> String {
...
extension DataFloatView {
    ...
    private func resultList() {
        ...
        self.frame = CGRect(x: 0, y: appTurnName, width: constFormalContent, height: 60)
