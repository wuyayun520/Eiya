...
fileprivate let k_modelMsg:[Character] = ["b","t","n","_","s","h","o","w","_","a","l","e","r","t","B","a"]
fileprivate let kAddValueText:[Character] = ["c","k","g","r","o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let kNorStartName:String = "btn_alvar finish in gift var"
fileprivate let kEqualMsg:String = "index topertBac"
fileprivate let k_nameValue:[Character] = ["_","f","a","l","s","e"]
...
// __CLOSE_PRINT__
//
//  FrameHiddenReactiveCompatible.swift
//  AbroadTalking
//
...
public class FrameHiddenReactiveCompatible: NSObject {
    /// 通用回调
...
public extension FrameHiddenReactiveCompatible {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: VideoAlertConfig) -> Void
    ...
    internal class func theory(title: String? = nil,
                               //: message: String?  = nil,
                               message: String? = nil,
                               ...
                               config: VideoAlertConfig? = nil)
    {
        //: customAlert(title: title,
        systemToConfig(title: title,
                       //: message: message,
                       message: message,
    ...
    @objc internal class func belowText(title: String? = nil,
    ...
    class func systemToConfig(title: String? = nil,
                              ...
                              config: VideoAlertConfig? = nil)
    {
        ...
        let alertView = MasterThen(title: title,
        ...
        //: UserTextMacroDefine.getWindow().addSubview(alertView)
        UserTextMacroDefine.clearstoryNo().addSubview(alertView)
    ...
    class func middleText() {
        //: UserTextMacroDefine.getWindow().subviews.forEach { (view) in
        UserTextMacroDefine.clearstoryNo().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: MasterThen.self) {
    ...
    class func rightInfo(title: String, isTouch: Bool) {
        //: UserTextMacroDefine.getWindow().subviews.forEach { (view) in
        UserTextMacroDefine.clearstoryNo().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: MasterThen.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: MasterThen = view as! MasterThen
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.sprechstimme(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class VideoAlertConfig {
    ...
    public var maskType: PlayerMaskType = .color
    /// 阴影
    ...
    /// alert标题字体
    //: public var titleFont: UIFont = .fontChange(type: .Medium, fontSize: 18)
    public var titleFont: UIFont = .fontChange(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.colorStatuteNameApp()
    public var titleColor = UIColor.colorStatuteNameApp()
    /// alert信息字体
    //: public var textFont: UIFont = .fontChange(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .fontChange(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.colorStatuteNameApp()
    public var textColor = UIColor.colorStatuteNameApp()
    //: public var textlineBreakMode = NSLineBreakMode.byWordWrapping
    public var textlineBreakMode = NSLineBreakMode.byWordWrapping

    /// alert按钮字体
    //: public var buttonFont: UIFont = .fontChange(type: .Medium, fontSize: 16)
    public var buttonFont: UIFont = .fontChange(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.increase()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(k_modelMsg) + String(kAddValueText))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(kNorStartName.prefix(6)) + String(kEqualMsg.suffix(6)) + "kground" + String(k_nameValue))
    //: public var rightFont: UIFont = .fontChange(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .fontChange(type: .Medium, fontSize: 16)
