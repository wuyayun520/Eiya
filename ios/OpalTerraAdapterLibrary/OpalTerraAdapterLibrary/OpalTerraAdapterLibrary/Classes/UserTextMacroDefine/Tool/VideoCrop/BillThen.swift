...
fileprivate let kValueTitle:[UInt8] = [0x8a,0x8d,0x8a,0x97,0xcb,0x80,0x8c,0x87,0x86,0x91,0xd9,0xca,0xc3,0x8b,0x82,0x90,0xc3,0x8d,0x8c,0x97,0xc3,0x81,0x86,0x86,0x8d,0xc3,0x8a,0x8e,0x93,0x8f,0x86,0x8e,0x86,0x8d,0x97,0x86,0x87]

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let kRequestMalePath:[Character] = ["b","t","n","J","Q","b","E","A","B","_","e","t","i","h","w","_","k","c","a","b","_","s"]
fileprivate let kPackageData:[Character] = ["h","a","d","o","w"]

/*: "Edit" :*/
fileprivate let k_itemContent:String = "Editif super error window star"

/*: "Finish" :*/
fileprivate let k_liveValue:String = "disappear purpleFinish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let k_shareFormat:String = "component guard topbtnYD"
fileprivate let kMaxKey:String = "select constraintediv_"
fileprivate let k_modelKey:String = "plmanager"

/*: "Cancel" :*/
fileprivate let kTransformId:[Character] = ["C","a","n","c","e","l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let k_dataMaxId:String = "last as make sharebgQk"
fileprivate let k_normalFormat:String = "var text false color varoediv_"
fileprivate let k_suiteStr:String = "trowst"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let k_exceptionText:[UInt8] = [0xca,0xf6,0xfb,0xbe,0xe8,0xf7,0xfa,0xfb,0xf1,0xbe,0xf2,0xfb,0xf0,0xf9,0xea,0xf6,0xbe,0xed,0xf6,0xf1,0xeb,0xf2,0xfa,0xbe,0xfc,0xfb,0xbe,0xbb,0xde,0xb3,0xbb,0xde,0xbe,0xed,0xfb,0xfd,0xf1,0xf0,0xfa,0xed,0xb0,0xbe,0xce,0xf2,0xfb,0xff,0xed,0xfb,0xbe,0xf7,0xf0,0xea,0xfb,0xec,0xfd,0xfb,0xee,0xea,0xbe,0xf7,0xea]

private func withoutTable(status num: UInt8) -> UInt8 {
    return num ^ 158
}
...
typealias VideoEditBlock = (_ videoInfo: WithEqualTransformable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let showModeOpenData: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class BillThen: StartThen {
    ...
    private var slider: WillSliderView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = WithEqualTransformable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = PerformanceEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isSpace == true {
        if ListSocketManager.shared.isSpace == true { // 正在音视频通话，静音播放
    ...
    init(style: PerformanceEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (constFormalContent - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kValueTitle.map{$0^227}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        self.framePopConstraint()
        //: self.bindInteraction()
        self.equalBindCounteraction()
        //: self.func__addNotification()
        self.likeNotification()
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.submitGift(name: "btnJQbEAB_etihw_kcab_shadow"), for: .normal)
        v.setImage(UIImage.submitGift(name: (String(kRequestMalePath) + String(kPackageData))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = MakeLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(collectionSearched), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(k_itemContent.prefix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.increase(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.targetFor(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = MakeLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(k_liveValue.suffix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.tapShared(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.increase()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(socialGroup), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        //: let v = UIButton(type: .custom)
        let v = UIButton(type: .custom)
        //: v.setImage(UIImage.submitGift(name: "btnYDo2AS_oediv_play"), for: .normal)
        v.setImage(UIImage.submitGift(name: (String(k_shareFormat.suffix(5)) + "o2AS_o" + String(kMaxKey.suffix(5)) + k_modelKey.replacingOccurrences(of: "manager", with: "ay"))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(occupationRequest(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(kTransformId)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(recordIn), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        //: let v = UIImageView(image: UIImage.submitGift(name: "bgQkRTbF_oediv_toast"))
        let v = UIImageView(image: UIImage.submitGift(name: (String(k_dataMaxId.suffix(4)) + "RTbF_" + String(k_normalFormat.suffix(6)) + k_suiteStr.replacingOccurrences(of: "row", with: "oa"))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: k_exceptionText.map{withoutTable(status: $0)}, encoding: .utf8)!.edgeDelay(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: ServerThen = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = ServerThen(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension BillThen {
    ...
    private func indexTheme(animated _: Bool = true) {
    ...
    @objc func changeEqualAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: constFormalContent - 2 * 26, height: user_recordMarginValue - 46 - 127 - appButtonData)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = constFormalContent / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: user_recordMarginValue, width: constFormalContent, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = WillSliderView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: showModeOpenData)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - appButtonData)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: EqualSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func socialGroup() {
        ...
        if self.videoEditHandler != nil {
            ...
            StartReactiveCompatible.shared.diagonal(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.indexTheme()
    ...
    @objc func occupationRequest(sender: UIButton) {
...
extension BillThen {
    ...
    @objc private func sizeCollection() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension BillThen {
    ...
    private func styleDraw() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(labelWill), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.supplanting(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
        ...
        if duration > maxduration {
            //: self.editAction()
            self.changeEqualAction()
        }
    }

    /// 添加约束
    //: private func setupSubViewsConstraint() {
    private func framePopConstraint() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + appButtonData)
        }
    ...
    private func equalBindCounteraction() {
        /// 私密视频事件
        //: if self.currStyle == .intimateVideo {
        if self.currStyle == .intimateVideo {
    ...
    private func likeNotification() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(toEditBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sizeCollection),
