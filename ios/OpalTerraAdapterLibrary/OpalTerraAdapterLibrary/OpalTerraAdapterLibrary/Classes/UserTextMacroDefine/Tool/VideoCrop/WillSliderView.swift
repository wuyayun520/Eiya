...
fileprivate let kTheoryText:[UInt8] = [0x68,0x6d,0x68,0x73,0x27,0x62,0x6e,0x63,0x64,0x71,0x39,0x28,0x1f,0x67,0x60,0x72,0x1f,0x6d,0x6e,0x73,0x1f,0x61,0x64,0x64,0x6d,0x1f,0x68,0x6c,0x6f,0x6b,0x64,0x6c,0x64,0x6d,0x73,0x64,0x63]

fileprivate func shirtButton(content num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: EqualSlideDirection) -> Void
...
class WillSliderView: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (noti_recordValue - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTheoryText.map{shirtButton(content: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.makeOrigin(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: MenuReactiveCompatible = {
        ...
        let overlay = MenuReactiveCompatible(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension WillSliderView {
    ...
    private func makeOrigin(point: CGFloat) -> UIImage {
...
extension WillSliderView: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = noti_recordValue - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.levelTo(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.levelTo(isShow: true)
    }
...
extension WillSliderView {
    ...
    private func detailTo() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(noti_recordValue)
