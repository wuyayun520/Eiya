...
fileprivate let k_toModeUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let kGroupKey:[Character] = ["F","F","D","8","5","5"]
...
class MenuReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - user_maxName
                ...
                self.passport(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_toModeUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(kGroupKey)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: noti_recordValue, y: 0, width: user_maxName, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - user_maxName, y: 0, width: user_maxName, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.behindVoice(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: noti_recordValue, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.behindVoice(0, 0, 0, 0.6)
...
extension MenuReactiveCompatible {
    ...
    func tableSearch() {
        ...
        self.leftSilder.frame = CGRect(x: noti_recordValue, y: 0, width: user_maxName, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - user_maxName, y: 0, width: user_maxName, height: self.frame.size.height)
        //: resetIndicator()
        coordinate()
    }

    //: func needShowIndicator(isShow: Bool) {
    func levelTo(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func passport(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - user_maxName,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - user_maxName)
    }
}
...
extension MenuReactiveCompatible {
    ...
    @objc func bubbleGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.passport(startX: videoStartX)
    ...
    @objc func legalRightInput(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension MenuReactiveCompatible {
    ...
    private func denounceSubviews() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(bubbleGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(legalRightInput(gesture:)))
    ...
    func supplanting(_ xposition: CGFloat) {
