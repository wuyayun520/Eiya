...
/*: / :*/
fileprivate let kBottomCellElsePath:[Character] = ["/"]

/*: "\n" :*/
fileprivate let kClearModelId:[Character] = ["\n"]
...
// __CLOSE_PRINT__
//
//  CancelPitchContentView.swift
//  CancelPitchContentView
...
struct CircularFileContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class CancelPitchContentView: UIView {
    // MARK: - 懒加载属性
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        itemAdd(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        itemAdd(placeholder: nil, placeholderColor: nil)
    }
}
...
extension CancelPitchContentView {
    /// placeholder Label Setup
    //: private func setup(placeholder: String?, placeholderColor: UIColor?) {
    private func itemAdd(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(PerformanceLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(PerformanceLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-PerformanceLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: CircularFileContainerInset().top, left: CircularFileContainerInset().left, bottom: CircularFileContainerInset().bottom, right: CircularFileContainerInset().right)
...
extension CancelPitchContentView: UITextViewDelegate {
