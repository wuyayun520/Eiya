...
/*: "Select a date" :*/
fileprivate let kEqualContent:[Character] = ["S","e","l","e","c","t"," ","a"," ","d","a","t","e"]

/*: M" :*/
fileprivate let k_containerContent:String = "model"

/*: D" :*/
fileprivate let k_translationValue:[Character] = ["D"]

/*: Y" :*/
fileprivate let k_popId:[Character] = ["Y"]
...
protocol ManagerObjectProtocol: NSObjectProtocol {
...
class ModeView: PickOutReactiveCompatible, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func appDown() {
        ...
        titleString = (String(kEqualContent)).localized
        ...
        receiveDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func receiveDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func nowAcross(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.addDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.addDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return nameImage(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func nameImage(year: Int, month: Int) -> Int {
