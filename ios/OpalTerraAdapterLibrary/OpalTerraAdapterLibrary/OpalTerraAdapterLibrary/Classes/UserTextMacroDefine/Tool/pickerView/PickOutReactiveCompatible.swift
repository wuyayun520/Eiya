...
fileprivate let k_viewUrl:[UInt8] = [0x18,0x1d,0x18,0x23,0xd7,0x12,0x1e,0x13,0x14,0x21,0xe9,0xd8,0xcf,0x17,0x10,0x22,0xcf,0x1d,0x1e,0x23,0xcf,0x11,0x14,0x14,0x1d,0xcf,0x18,0x1c,0x1f,0x1b,0x14,0x1c,0x14,0x1d,0x23,0x14,0x13]

fileprivate func insiderInformation(table num: UInt8) -> UInt8 {
...
/*: "Cancel" :*/
fileprivate let kPathFormat:String = "Cancelrest false label self at"

/*: "Sure" :*/
fileprivate let k_buttonMsg:String = "Sureclear cancel filter inside"
...
// __CLOSE_PRINT__
//
//  PickOutReactiveCompatible.swift
//  AbroadTalking
//
...
class PickOutReactiveCompatible: UIView {
    // 背景视图
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_viewUrl.map{insiderInformation(table: $0)}, encoding: .utf8)!)
    }

    //: private func setupView() {
    private func lineTap() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(futurismTo))
        ...
        cancelButton.setTitle((String(kPathFormat.prefix(6))).localized, for: .normal)
        //: cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: cancelButton.setTitleColor(UIColor.black, for: .normal)
        cancelButton.setTitleColor(UIColor.black, for: .normal)
        //: cancelButton.addTarget(self, action: #selector(cancelClick(button:)), for: .touchUpInside)
        cancelButton.addTarget(self, action: #selector(resumeScratch(button:)), for: .touchUpInside)
        ...
        confirmButton.setTitle((String(k_buttonMsg.prefix(4))).localized, for: .normal)
        //: confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: confirmButton.setTitleColor(UIColor.black, for: .normal)
        confirmButton.setTitleColor(UIColor.black, for: .normal)
        //: confirmButton.addTarget(self, action: #selector(confirmClick(button:)), for: .touchUpInside)
        confirmButton.addTarget(self, action: #selector(nowAcross(button:)), for: .touchUpInside)
    ...
    @objc func resumeScratch(button _: UIButton) {
        //: dismissView()
        futurismTo()
    }

    //: @objc func confirmClick(button: UIButton) {
    @objc func nowAcross(button _: UIButton) {
        //: dismissView()
        futurismTo()
    }
