...
fileprivate let k_userId:[Character] = ["A","p","p","R","e","s","o","u","r","c"]
fileprivate let kSinceText:[Character] = ["e","s","V","e","r","s","i","o","n","K","e","y"]

/*: ".zip" :*/
fileprivate let k_buttonThumbGuardUrl:String = "log request color true.zip"

/*: "pic" :*/
fileprivate let kBackgroundName:String = "pishare"

/*: "nor" :*/
fileprivate let k_modeMsg:String = "nointimate"

/*: "svga" :*/
fileprivate let k_whiteFormat:[Character] = ["s","v","g","a"]

/*: "mp3" :*/
fileprivate let k_minuteUrl:String = "mpofficial"

/*: "model" :*/
fileprivate let kOfName:[Character] = ["m","o","d","e","l"]

/*: "language" :*/
fileprivate let kAddFormat:[Character] = ["l"]
fileprivate let k_endUrl:[Character] = ["a","n","g","u","a","g","e"]

/*: "@2x.png" :*/
fileprivate let kTimeName:String = "@"
fileprivate let kDeadlineUrl:String = "pic var toward height at2x.png"

/*: "png" :*/
fileprivate let k_angleFormat:String = "pnshared"
...
private let noti_clickValue = (String(k_userId) + String(kSinceText))

//: enum SVGAType: String {
enum PossibilityCollection: String {
...
class VideoEffectTool: NSObject {
    //: static let `default` = SVGAEffectTool()
    static let `default` = VideoEffectTool()
    ...
    private lazy var AppResPath: String = {
        ...
        let res_Dir = (docuPath as NSString).appendingPathComponent(user_failModeData)
        //: let version = Defaults.string(forKey: AppResourcesVersionKey) ?? ""
        let version = show_blockText.string(forKey: noti_clickValue) ?? ""
        //: if version == AppVersion, FileManager.default.fileExists(atPath: res_Dir) {
        if version == userFileContent, FileManager.default.fileExists(atPath: res_Dir) {
            //: return res_Dir
            return res_Dir
        }
        //: let path = PodspecBundle.bundle.path(forResource: PodspecName, ofType: ".zip") ?? ""
        let path = UserPodspecBundle.bundle.path(forResource: user_failModeData, ofType: (String(k_buttonThumbGuardUrl.suffix(4)))) ?? ""
        ...
        let zipSuc = SSZipArchive.unzipFile(atPath: path,
                                            ...
                                            password: user_failModeData,
                                            //: progressHandler: nil)
                                            progressHandler: nil)
        //: if zipSuc {
        if zipSuc {
            //: Defaults.set(AppVersion, forKey: AppResourcesVersionKey)
            show_blockText.set(userFileContent, forKey: noti_clickValue)
            //: return res_Dir
            return res_Dir
        }
        //: return ""
    ...
    private lazy var imageResPath: String = //: return (AppResPath as NSString).appendingPathComponent("pic")
        (AppResPath as NSString).appendingPathComponent((kBackgroundName.replacingOccurrences(of: "share", with: "c")))
    //: }()

    //: private lazy var languageImageResPath: String = {
    private lazy var languageImageResPath: String = {
        //: let language = LanguageManager.shared.currLanguage
        let language = MakeLanguageManager.shared.currLanguage
    ...
    private lazy var norImageResPath: String = {
        ...
        return (imageResPath as NSString).appendingPathComponent((k_modeMsg.replacingOccurrences(of: "intimate", with: "r")))
        //: }()
    }()
    ...
    private lazy var svgaResPath: String = //: return (AppResPath as NSString).appendingPathComponent("svga")
        (AppResPath as NSString).appendingPathComponent((String(k_whiteFormat)))
    //: }()
    ...
    private lazy var mp3ResPath: String = //: return (AppResPath as NSString).appendingPathComponent("mp3")
        (AppResPath as NSString).appendingPathComponent((k_minuteUrl.replacingOccurrences(of: "official", with: "3")))
    //: }()
    ...
    public lazy var modelResPath: String = //: return (AppResPath as NSString).appendingPathComponent("model")
        (AppResPath as NSString).appendingPathComponent((String(kOfName)))
    //: }()
    ...
    public lazy var languagePath: String = //: return (AppResPath as NSString).appendingPathComponent("language")
        (AppResPath as NSString).appendingPathComponent((String(kAddFormat) + String(k_endUrl)))
    //: }()
}

//: extension SVGAEffectTool {
extension VideoEffectTool {
    /// 直接获取图片
    //: func getImage(name: String) -> UIImage? {
    func headKit(name: String) -> UIImage? {
        //: let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending("@2x.png")
        let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending((kTimeName.capitalized + String(kDeadlineUrl.suffix(6))))
        ...
        let path = (norImageResPath as NSString).appendingPathComponent(name).appending((kTimeName.capitalized + String(kDeadlineUrl.suffix(6))))
    ...
    func designationLibrary(iconName: String) -> URL {
        //: var path = (norImageResPath as NSString).appendingPathComponent(iconName)
        var path = (norImageResPath as NSString).appendingPathComponent(iconName)
        //: path = (path as NSString).appendingPathExtension("png")!
        path = (path as NSString).appendingPathExtension((k_angleFormat.replacingOccurrences(of: "shared", with: "g")))!
        //: let url = URL(fileURLWithPath: path)
        let url = URL(fileURLWithPath: path)
        //: return url
        return url
    ...
    func titleType(name: String, type: String = (k_minuteUrl.replacingOccurrences(of: "official", with: "3"))) -> String {
