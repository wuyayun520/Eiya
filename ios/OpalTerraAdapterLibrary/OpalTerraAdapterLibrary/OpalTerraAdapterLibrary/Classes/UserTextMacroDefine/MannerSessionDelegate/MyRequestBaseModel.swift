...
/*: "http://stat. :*/
fileprivate let k_styleKey:[Character] = ["h","t","t","p",":","/","/","s","t","a","t","."]

/*: .com" :*/
fileprivate let k_cellValue:String = ".comvideo black video if path"

/*: "http://" :*/
fileprivate let k_femaleUserData:[Character] = ["h","t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let k_kitUpData:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let kStyleViewKey:String = "hmake"
fileprivate let k_dataMsg:String = "row submit changetp://m."

/*: "http:// :*/
fileprivate let k_selectedId:String = "left intimate time lethttp://"

/*: "-1" :*/
fileprivate let k_toKey:String = "-1"

/*: "3,90" :*/
fileprivate let k_dataStr:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let k_addEqualPath:[Character] = ["1"]
fileprivate let k_statusUrl:String = "layer0-"

/*: "MM-dd-yyyy" :*/
fileprivate let kRenderToBottomPath:[Character] = ["M","M","-","d","d","-","y","y","y","y"]

/*: "id" :*/
fileprivate let k_iconHalfLocationText:[Character] = ["i","d"]
...
class SweepRequestModel: NSObject {
    ...
    override init() {
        ...
        self.requestServer = UserViewToolThen.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func smartObject() -> Bool {
...
struct SimulationModelType: HandyJSON {
    //: var errno: Int!
...
public struct PullErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct PerformanceConfigModel: HandyJSON {
    ...
    var CSConfig = CircumferenceHandyJSON() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(k_styleKey)) + "\(main_keyValue)" + (String(k_cellValue.prefix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(k_femaleUserData))) && !statDomain.hasPrefix((String(k_kitUpData))) {
                //: return "http://".appending(statDomain)
                return (String(k_femaleUserData)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (kStyleViewKey.replacingOccurrences(of: "make", with: "t") + String(k_dataMsg.suffix(7))) + "\(main_keyValue)" + (String(k_cellValue.prefix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(k_selectedId.suffix(7))) + "\(urlDomain)"
        }
    }
...
public class ViewConfigModel: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [ViewHandyJSON] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [ViewHandyJSON] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [ViewHandyJSON] = [] // 语音收费设置
    ...
    var vipSetting = [SettingCancelModelType]() // vip皮肤
    ...
    var tabPageBanner = [AdMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = ValueTransformable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class CoverReactiveCompatible: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [PossibilityEquatable]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [PossibilityEquatable]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (k_dataStr.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.digitizerGroup(self.birthday ?? (String(k_addEqualPath) + k_statusUrl.replacingOccurrences(of: "layer", with: "99") + "01-01"), dateFormat: (String(kRenderToBottomPath)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.blockMake().year else { return "0" }
...
struct SingleModelType: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct PossibilityEquatable: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func studyEqual(compareTo: PossibilityEquatable) -> Bool {
        //: return
        return
...
struct ServerMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [CoequalTransformable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [CoequalTransformable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct CoequalTransformable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [PossibilityEquatable]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct CircumferenceHandyJSON: HandyJSON {
...
struct SettingCancelModelType: HandyJSON {
