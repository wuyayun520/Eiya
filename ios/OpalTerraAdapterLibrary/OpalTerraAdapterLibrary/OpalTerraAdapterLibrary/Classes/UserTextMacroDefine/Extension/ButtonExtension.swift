...
/*: "recharge" :*/
fileprivate let k_ageStr:String = "recmakerg"
fileprivate let kEffectUrl:String = "single"

/*: "recharge_ :*/
fileprivate let kToId:[Character] = ["r","e","c","h","a"]
fileprivate let kNameUrl:[Character] = ["r","g","e","_"]

/*: ".png" :*/
fileprivate let kDataUrl:[Character] = [".","p","n","g"]

/*: "_ :*/
fileprivate let k_femaleMsg:String = "valid"

/*: .png" :*/
fileprivate let kQuoteManagerKey:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let kTableInfoName:String = ".jpgtext text var"

/*: .jpg" :*/
fileprivate let kSucceedStr:[Character] = [".","j","p","g"]
...
enum CustomMagnitude: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class ResumeControl: UIButton {
    ...
    var imageAlignment: CustomMagnitude = .left
...
extension UIButton {
    ...
    func fictionalAnimalTitle(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        product(interval: interval, topOffset: 0)
    }
    ...
    func product(interval: CGFloat, topOffset: CGFloat) {
    ...
    func renderAcross(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func languageUnitState(color: UIColor, forState: UIControl.State) {
    ...
    func successOn(urlStr: String,
                   //: placeImg: UIImage? = UIImage.placeImgSquare(),
                   placeImg: UIImage? = UIImage.rectangle(),
                   ...
                   loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: data_statusText.doingApp(), with: main_keyValue)
    ...
    func templeVisualisationSetFrame(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((k_ageStr.replacingOccurrences(of: "make", with: "ha") + kEffectUrl.replacingOccurrences(of: "single", with: "e"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (k_ageStr.replacingOccurrences(of: "make", with: "ha") + kEffectUrl.replacingOccurrences(of: "single", with: "e")), with: (String(kToId) + String(kNameUrl)) + "\(const_userName)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.successOn(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(kDataUrl))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(kDataUrl)), with: "_\(const_userName)" + (String(kQuoteManagerKey)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(kTableInfoName.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(kTableInfoName.prefix(4))), with: "_\(const_userName)" + (String(kSucceedStr)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.successOn(urlStr: newUrl, placeImg: nil)
    }
