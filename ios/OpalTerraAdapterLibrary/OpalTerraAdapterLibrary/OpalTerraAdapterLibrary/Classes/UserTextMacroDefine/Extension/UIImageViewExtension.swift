...
/*: "add_header_cookies" :*/
fileprivate let k_redId:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func setUrlImage(urlStr: String,
    ...
    internal func setUrlImage(urlStr: String,
                            //: placeImg: UIImage = UIImage.placeImgSquare(),
                            placeImg: UIImage = UIImage.rectangle(),
    ...
    @objc func visualisationMake(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: data_statusText.doingApp(), with: main_keyValue)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.sizeBridePrice())
    }
    ...
    @objc func sizeValueCompletion(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: UserViewToolThen.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(k_redId)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.rectangle(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
