...
/*: "ZUhWdWQzVnNiM1ps" :*/
fileprivate let k_itemData:[Character] = ["Z","U","h","W","d","W","Q","z","V","n","N","i","M","1","p","s"]

/*: "OrKulDHBYyjUHZTy1sS7dw==" :*/
fileprivate let kKeyValue:String = "OrKulstatus handle"
fileprivate let kBackName:String = "error superyjUHZT"
fileprivate let k_removePath:String = "list model keydw=="

/*: "null" :*/
fileprivate let k_boyMsg:String = "nusize"

/*: "(null)" :*/
fileprivate let k_toMsg:String = "(null)if point type fill create"

/*: "nil" :*/
fileprivate let kFrameKey:String = "restl"

/*: "<null>" :*/
fileprivate let k_centerEqualMessage:[Character] = ["<","n","u","l","l",">"]

/*: "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMnopQRSTUVWXYZ0123456789" :*/
fileprivate let kSendData:[UInt8] = [0xab,0xac,0xad,0xae,0xaf,0xb0,0xb1,0xb2,0xb3,0xb4,0xb5,0xb6,0xb7,0xb8,0xb9,0xba,0xbb,0xbc,0xbd,0xbe,0xbf,0xc0,0xc1,0xc2,0xc3,0xc4,0x8b,0x8c,0x8d,0x8e,0x8f,0x90,0x91,0x92,0x93,0x94,0x95,0x96,0x97,0xb8,0xb9,0xba,0x9b,0x9c,0x9d,0x9e,0x9f,0xa0,0xa1,0xa2,0xa3,0xa4,0x7a,0x7b,0x7c,0x7d,0x7e,0x7f,0x80,0x81,0x82,0x83]

fileprivate func makeView(mode num: UInt8) -> UInt8 {
...
/*: "加密参数=  :*/
fileprivate let k_modelId:[Character] = ["加","密","参","数","="," "]

/*: "解密结果=  :*/
fileprivate let kBelowKey:[Character] = ["\u{89e3}"]
fileprivate let kColorFormat:String = "密\u{7ed3}\u{679c}= "

/*: "<.*?>" :*/
fileprivate let kInputKey:String = "<.*?>"
...
let user_videoValue = (String(k_itemData))

//: let UrlDomin = "OrKulDHBYyjUHZTy1sS7dw=="
let data_statusText = (String(kKeyValue.prefix(5)) + "DHBY" + String(kBackName.suffix(6)) + "y1sS7" + String(k_removePath.suffix(4)))
...
extension String {
    ...
    var isEmptyString: Bool {
        ...
        guard self != (k_boyMsg.replacingOccurrences(of: "size", with: "ll")) else { return true }
        //: guard self != "(null)" else { return true }
        guard self != (String(k_toMsg.prefix(6))) else { return true }
        //: guard self != "nil" else { return true }
        guard self != (kFrameKey.replacingOccurrences(of: "rest", with: "ni")) else { return true }
        //: guard self != "<null>" else { return true }
        guard self != (String(k_centerEqualMessage)) else { return true }
    ...
    func manEqualConfines(_ matchStr: String) -> [NSRange] {
...
extension String {
    ...
    static func sessionDown(length: Int) -> String {
        //: let letters: NSString = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMnopQRSTUVWXYZ0123456789"
        let letters: NSString = String(bytes: kSendData.map{makeView(mode: $0)}, encoding: .utf8)! as NSString
    ...
    func adjustInUp(key: String) -> String? {
        //: printLog(message: "加密参数= \(self)")
        printLog(message: (String(k_modelId)) + "\(self)")
    ...
    func belowImageDate(key: String) -> String? {
        ...
        printLog(message: (String(kBelowKey) + kColorFormat) + "\(decryStr)")
        //: return decryStr
        return decryStr
    }

    //: func urlAes256Encrypt() -> String? {
    func listWill() -> String? {
        //: printLog(message: "加密参数= \(self)")
        printLog(message: (String(k_modelId)) + "\(self)")
        //: guard let inputData: Data = data(using: .utf8) else { return nil }
        guard let inputData: Data = data(using: .utf8) else { return nil }
        //: let EncryptData = (inputData as NSData).aes256Encrypt(withKey: UrlKey)
        let EncryptData = (inputData as NSData).aes256Encrypt(withKey: user_videoValue)
        //: return NSData.base64String(from: EncryptData, length: UInt((EncryptData as NSData).length))
        return NSData.base64String(from: EncryptData, length: UInt((EncryptData as NSData).length))
    }

    //: func urlAes256Decrypt() -> String {
    func doingApp() -> String {
        //: let encryData = NSData.base64Data(from: self)
        let encryData = NSData.base64Data(from: self)
        //: let decryData = (encryData as NSData).aes256Decrypt(withKey: UrlKey)
        let decryData = (encryData as NSData).aes256Decrypt(withKey: user_videoValue)
        //: let decryStr = String(data: decryData, encoding: .utf8)
        let decryStr = String(data: decryData, encoding: .utf8)
        //: return decryStr ?? ""
        return decryStr ?? ""
    ...
    static func book(str: String, from: Int, to: Int) -> String {
