...
fileprivate let kNameUrl:[UInt8] = [0xd5,0xda,0xd5,0xe0,0x94,0xcf,0xdb,0xd0,0xd1,0xde,0xa6,0x95,0x8c,0xd4,0xcd,0xdf,0x8c,0xda,0xdb,0xe0,0x8c,0xce,0xd1,0xd1,0xda,0x8c,0xd5,0xd9,0xdc,0xd8,0xd1,0xd9,0xd1,0xda,0xe0,0xd1,0xd0]

fileprivate func quickOrigin(first num: UInt8) -> UInt8 {
...
/*: "Not yet~" :*/
fileprivate let k_searchUserData:String = "Noimage by"
fileprivate let kColorUrl:String = "text video path view sectiont yet~"

/*: "#8C7AFF" :*/
fileprivate let k_blockCenterStr:String = "live unknown m lab count#8C7AFF"
...
class BearDoingDataSource: UIView {
    //: var type = 0
    var type = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kNameUrl.map{quickOrigin(first: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(SizeViewCell.self, forCellReuseIdentifier: SizeViewCell.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(EndViewCell.self, forCellReuseIdentifier: EndViewCell.className())
    }
    ...
    lazy var tableHeaderView = DataHeaderView(frame: CGRect(x: 0, y: 0, width: constFormalContent, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = WeltanschauungReactiveCompatible().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(k_searchUserData.prefix(2)) + String(kColorUrl.suffix(6))).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(k_blockCenterStr.suffix(7))))
...
extension BearDoingDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: SizeViewCell.className(), for: indexPath) as! SizeViewCell
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: EndViewCell.className(), for: indexPath) as! EndViewCell
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.beforeButton(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.fireLast(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: SizeViewCell.className()) as! SizeViewCell
...
extension BearDoingDataSource: JXSegmentedListContainerViewListDelegate {
...
extension BearDoingDataSource {
    //: private func setupSubviews() {
    private func cardEffect() {
