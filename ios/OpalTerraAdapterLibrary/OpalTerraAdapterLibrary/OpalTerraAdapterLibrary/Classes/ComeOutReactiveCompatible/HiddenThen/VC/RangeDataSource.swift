...
/*: "Search" :*/
fileprivate let k_contentKey:[Character] = ["S","e","a","r","c"]
fileprivate let k_fromTitle:String = "H"

/*: "#F5F5F5" :*/
fileprivate let k_cancelFormat:[Character] = ["#","F"]
fileprivate let kHiddenMessage:String = "5F5F5collection info manager"

/*: "Search by UID" :*/
fileprivate let kListLabTitle:[Character] = ["S","e","a","r","c","h"," "]
fileprivate let k_upValue:[Character] = ["b","y"," ","U","I","D"]

/*: "icon_search_sea" :*/
fileprivate let k_frameAfterId:[Character] = ["i","c","o","n","_","s","e","a","r","c","h","_","s"]
fileprivate let k_imageUrl:[Character] = ["e","a"]

/*: "Can’t find~" :*/
fileprivate let k_pathDataMessage:String = "Can’right app let make with"

/*: "icon_kong_kong_default" :*/
fileprivate let k_interruptName:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let k_managerMsg:[Character] = ["P","l","e","a","s","e"," ","e","n","t","e","r"," ","t"]
fileprivate let k_colorMsg:[Character] = ["h","e"," ","q","u","e","r","y"," ","U","I","D"]
...
class RangeDataSource: StartThen {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(k_contentKey) + k_fromTitle.lowercased()).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.makeEqual(colors: UIColor.georgiaHomeBoy(), size: CGSize(width: 72, height: 32)), for: .normal)
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(k_cancelFormat) + String(kHiddenMessage.prefix(5))))
        //: tf.textColor = UIColor.colorStatuteNameApp()
        tf.textColor = UIColor.colorStatuteNameApp()
        //: tf.font = UIFont.pingfangRugularFont(fontSize: 16)
        tf.font = UIFont.targetFor(fontSize: 16)
        ...
        tf.placeholder = (String(kListLabTitle) + String(k_upValue)).localized
    ...
    private lazy var unlessEditingView: UIView = {
        //: let image = UIImage.submitGift(name: "icon_search_sea")
        let image = UIImage.submitGift(name: (String(k_frameAfterId) + String(k_imageUrl)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(MenuCheckPortionCellView.self, forCellReuseIdentifier: MenuCheckPortionCellView.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(k_pathDataMessage.prefix(4)) + "t find~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(k_interruptName))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + showBottomDailyValue, width: constFormalContent, height: user_recordMarginValue - (14 + showBottomDailyValue + appTurnName)), style: style)
    ...
    private lazy var reqManager: HiddenThen = //: return HiddenThen()
        .init()
    //: }()
}
...
extension RangeDataSource {
    ...
    private func writeClick() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.arcInfo(showMsg: (String(k_managerMsg) + String(k_colorMsg)).localized)
            //: return
            return
        }
        ...
        reqManager.safety(uid: searchTF.text ?? "") { [weak self] _, _, _ in
...
extension RangeDataSource {
    ...
    @objc private func toALesserExtent() {
        //: req_searchUid()
        writeClick()
    }
...
extension RangeDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: MenuCheckPortionCellView.className(), for: indexPath) as! MenuCheckPortionCellView
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.unfearingShow(model: model)
...
extension RangeDataSource: UITextFieldDelegate {
...
extension RangeDataSource: JXPagingViewListViewDelegate {
