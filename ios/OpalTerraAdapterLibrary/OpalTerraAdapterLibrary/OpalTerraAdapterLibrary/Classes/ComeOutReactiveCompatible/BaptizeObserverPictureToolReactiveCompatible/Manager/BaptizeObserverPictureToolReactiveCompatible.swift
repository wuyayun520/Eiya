...
class BaptizeObserverPictureToolReactiveCompatible: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: ViewChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = EndThen()          // 视频渲染配置类
    ...
    private var trtcView: PopView?    // 预览view（使用商汤美颜）
...
extension BaptizeObserverPictureToolReactiveCompatible {
    /// 释放资源
    ...
    func retort(_ frontCamera: Bool) {
        ...
        self.loadAcross(front: frontCamera)
    }
    ...
    func ply(_ openCamera: Bool) {
        ...
        self.listDismiss(isOpen: openCamera)
    }
    ...
    func giftCover() {
        ...
        DispatchQueue.main.async {
            ...
            self.isometrical(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension BaptizeObserverPictureToolReactiveCompatible {
    ...
    private func loadAcross(front: Bool) {
        ...
        self.trtcView?.doingFront(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func listDismiss(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard mainUserValue == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.notData(isOpen: isOpen)
...
extension BaptizeObserverPictureToolReactiveCompatible {
    ...
    func meDoingSettings() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(nameBy(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(notifKey(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(itinerary(notif:)),
    ...
    @objc private func abstract(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func nameBy(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func notifKey(notif: Notification) {
    ...
    @objc func itinerary(notif: Notification) {
...
extension BaptizeObserverPictureToolReactiveCompatible {
    ...
    func isometrical(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard mainUserValue == true else {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = PopView()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.plungeWithPusher(isSmallWindow: smallWindow)
        //: if smallWindow { 
