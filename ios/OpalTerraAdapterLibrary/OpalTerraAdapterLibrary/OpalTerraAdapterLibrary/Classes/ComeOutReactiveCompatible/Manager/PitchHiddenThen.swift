...
fileprivate let kTimeText:[UInt8] = [0xee,0xed,0xe1,0xe3,0xf6,0xeb,0xed,0xec,0xa2,0xe3,0xf7,0xf6,0xea,0xed,0xf0,0xeb,0xf8,0xe3,0xf6,0xeb,0xed,0xec,0xd1,0xf6,0xe3,0xf6,0xf7,0xf1,0xa2,0xeb,0xf1,0xa2]

private func countGift(need num: UInt8) -> UInt8 {
    return num ^ 130
}

/*: "get location failed. error: :*/
fileprivate let k_wrapFormat:String = "get lif game data title count"
fileprivate let k_imageName:String = "height let else inputn fail"
fileprivate let kBlockId:[Character] = ["e","d","."," ","e","r","r","o","r",":"]
...
class PitchHiddenThen: NSObject {
    ...
    var locationType: StreamPriorityTarget?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: PitchHiddenThen?
    //: class func shared() -> TalkingLocationManager {
    class func endShared() -> PitchHiddenThen {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = PitchHiddenThen()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func firstText(block: @escaping Locationblock) {
    ...
    func rowBlock(block: @escaping Authblock) {
    ...
    func tagSumCurrent() -> StreamPriorityTarget {
        //: switch locationPermission() {
        switch tiedPermission() {
        //: case .notDetermined:
        case .notDetermined:
            //: return .firstEntry
            return .firstEntry
    ...
    func tiedPermission() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: kTimeText.map{countGift(need: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension PitchHiddenThen: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(k_wrapFormat.prefix(5)) + "ocatio" + String(k_imageName.suffix(6)) + String(kBlockId)) + "\(error.localizedDescription)")
    }
}
