...
@interface MagRawData : NSObject

+ (instancetype)sharedInstance;

@end

@implementation MagRawData

+ (instancetype)sharedInstance {
    static MagRawData *instance = nil;
...
- (Byte *)MagRawDataToCache:(Byte *)data {
    int textFile = data[0];
    int recapture = data[1];
    for (int i = 0; i < textFile / 2; i++) {
        int begin = recapture + i;
        int end = recapture + textFile - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[recapture + textFile] = 0;
    return data + recapture;
}

- (NSString *)StringFromMagRawData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self MagRawDataToCache:data]];
}  

//: camera_focus_red
- (NSString *)mainOnId {
    /* static */ NSString *mainOnId = nil;
    if (!mainOnId) {
        Byte value[] = {16, 5, 247, 77, 36, 100, 101, 114, 95, 115, 117, 99, 111, 102, 95, 97, 114, 101, 109, 97, 99, 133};
        mainOnId = [self StringFromMagRawData:value];
    }
    return mainOnId;
}

//: M_SenseME_Face_Video_5.3.4
- (NSString *)main_intervalId {
    /* static */ NSString *main_intervalId = nil;
    if (!main_intervalId) {
        Byte value[] = {26, 5, 41, 13, 217, 52, 46, 51, 46, 53, 95, 111, 101, 100, 105, 86, 95, 101, 99, 97, 70, 95, 69, 77, 101, 115, 110, 101, 83, 95, 77, 194};
        main_intervalId = [self StringFromMagRawData:value];
    }
    return main_intervalId;
}

//: hasFace
- (NSString *)mainTextStr {
    /* static */ NSString *mainTextStr = nil;
    if (!mainTextStr) {
        Byte value[] = {7, 7, 62, 57, 187, 188, 157, 101, 99, 97, 70, 115, 97, 104, 169};
        mainTextStr = [self StringFromMagRawData:value];
    }
    return mainTextStr;
}

//: model
- (NSString *)main_sizeRangeKey {
    /* static */ NSString *main_sizeRangeKey = nil;
    if (!main_sizeRangeKey) {
        Byte value[] = {5, 4, 26, 65, 108, 101, 100, 111, 109, 24};
        main_sizeRangeKey = [self StringFromMagRawData:value];
    }
    return main_sizeRangeKey;
}

//: FaceActionEvent_Interval_1_second
- (NSString *)showPathMessage {
    /* static */ NSString *showPathMessage = nil;
    if (!showPathMessage) {
        Byte value[] = {33, 3, 239, 100, 110, 111, 99, 101, 115, 95, 49, 95, 108, 97, 118, 114, 101, 116, 110, 73, 95, 116, 110, 101, 118, 69, 110, 111, 105, 116, 99, 65, 101, 99, 97, 70, 235};
        showPathMessage = [self StringFromMagRawData:value];
    }
    return showPathMessage;
}

//: key.Notification.Tap.To.Modify.Exposure.Point
- (NSString *)main_keyMsg {
    /* static */ NSString *main_keyMsg = nil;
    if (!main_keyMsg) {
        Byte value[] = {45, 2, 116, 110, 105, 111, 80, 46, 101, 114, 117, 115, 111, 112, 120, 69, 46, 121, 102, 105, 100, 111, 77, 46, 111, 84, 46, 112, 97, 84, 46, 110, 111, 105, 116, 97, 99, 105, 102, 105, 116, 111, 78, 46, 121, 101, 107, 89};
        main_keyMsg = [self StringFromMagRawData:value];
    }
    return main_keyMsg;
}

//: FaceActionEvent_Interval_12_frame
- (NSString *)showNumberTextPath {
    /* static */ NSString *showNumberTextPath = nil;
    if (!showNumberTextPath) {
        Byte value[] = {33, 6, 28, 54, 33, 178, 101, 109, 97, 114, 102, 95, 50, 49, 95, 108, 97, 118, 114, 101, 116, 110, 73, 95, 116, 110, 101, 118, 69, 110, 111, 105, 116, 99, 65, 101, 99, 97, 70, 253};
        showNumberTextPath = [self StringFromMagRawData:value];
    }
    return showNumberTextPath;
}
...
//
//#import "CostServerReactiveCompatible.h"
//#import "GroupLooksModel.h"
//ST_MOBILE
...
@interface MakeView ()<ShowOf,UIGestureRecognizerDelegate>
{
...
@property (nonatomic, strong) InstalmentLicHelper * lichelper;
//: @property (nonatomic, strong) STParamUtil * ParamUtil;
@property (nonatomic, strong) WillParamUtil * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.lichelper = [InstalmentLicHelper share];
        //: self.ParamUtil = [[STParamUtil alloc]init];
        self.ParamUtil = [[WillParamUtil alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper systemAfter]) {
            ...
            //: [[CostServerReactiveCompatible share] getLocalModel];
            [[CostServerReactiveCompatible share] separate];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(handleTapNoti:)
                                                       ...
                                                       name:[[MagRawData sharedInstance] main_keyMsg]
                                                     //: object:nil];
                                                     object:nil];
...
- (instancetype)initWihtVideoConversation:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        //: } else {
            ...
            [InstalmentLicHelper.share neighbourWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initFilterView:sessionPreset];
...
- (void)setCachFilter {
    ...
    //: if ([CostServerReactiveCompatible share].filterSuite.filterModelPath.length > 0) {
    if ([CostServerReactiveCompatible share].filterSuite.filterModelPath.length > 0) {
        //: GroupLooksModel* model = [[CostServerReactiveCompatible share] getFilterDataModel];
        GroupLooksModel* model = [[CostServerReactiveCompatible share] recoupAtConcentrationEconomicTheory];
        //: if (model) {
        if (model) {
...
- (void)setupCameraAndPreview {
    ...
    _stCamera = [[StarCamera alloc] initWithDevicePosition:AVCaptureDevicePositionFront
...
- (void)resetPreviewWith:(CGFloat)width height:(CGFloat)height {
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[FailureView alloc] initWithFrame:previewRect context:previewContext];
        }
...
- (void)setupHandle {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [UserTextMacroDefine.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [UserTextMacroDefine.imagePick stringByAppendingPathComponent:[[MagRawData sharedInstance] main_intervalId]];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:[[MagRawData sharedInstance] main_sizeRangeKey]];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:[[MagRawData sharedInstance] mainOnId]];
...
- (void)receiveFilterNotice:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[GroupLooksModel class]]) {
    if ([notice.object isKindOfClass:[GroupLooksModel class]]) {
        //: GroupLooksModel* model = (GroupLooksModel*)notice.object;
        GroupLooksModel* model = (GroupLooksModel*)notice.object;
        //: [self handleFilterChanged:model];
        [self handleFilterChanged:model];
    }
...
//: - (void)handleFilterChanged:(GroupLooksModel *)model {
- (void)handleFilterChanged:(GroupLooksModel *)model {
...
- (void)setupBeautyParam {
    ...
    //: RangeReactiveCompatible *beautyValueCache = CostServerReactiveCompatible.share.currentBeautySuite;
    RangeReactiveCompatible *beautyValueCache = CostServerReactiveCompatible.share.awakeStopData;

    // 设置美白参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil secretWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
...
void liquidEcstasy(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void maximumFace(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)callbackDelegateWithSampleBuffer:(CMSampleBufferRef)originalSampleBuffer
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        id<MeMatch> tmpDelegate = self.delegate;

        //
        //: if ([tmpDelegate respondsToSelector:@selector(stVideoBeautyView:didProcessVideoSampleBuffer:)]) {
        if ([tmpDelegate respondsToSelector:@selector(videoBeautyView:didProcessVideoSampleBuffer:)]) {
            //: [tmpDelegate stVideoBeautyView:self didProcessVideoSampleBuffer:copyBuffer];
            [tmpDelegate videoBeautyView:self didProcessVideoSampleBuffer:copyBuffer];
        }

        //
        //: if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(stFilterView:didProcessSampleBuffer:)]) {
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(filterView:didProcessSampleBuffer:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate filterView:self didProcessSampleBuffer:sampleBuffer];
            }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[[MagRawData sharedInstance] showNumberTextPath] object:nil userInfo: @{[[MagRawData sharedInstance] mainTextStr]: @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[[MagRawData sharedInstance] showPathMessage] object:nil userInfo: @{[[MagRawData sharedInstance] mainTextStr]: @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        liquidEcstasy(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (faceCount > 0) {
            ...
            maximumFace(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
