...
//: Height
Byte mainTopCaptureKey[] = {54, 6, 5, 10, 78, 116, 104, 103, 105, 101, 72, 205};

//: Width
Byte showRangeValue[] = {26, 5, 9, 182, 4, 152, 110, 34, 144, 104, 116, 100, 105, 87, 214};

//: 请打开相机权限
Byte mainWithId[] = {68, 21, 7, 165, 111, 55, 214, 144, 153, 233, 131, 157, 230, 186, 156, 230, 184, 155, 231, 128, 188, 229, 147, 137, 230, 183, 175, 232, 34};
...
@interface StarCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:StringFromReadyData(showRangeValue)] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:StringFromReadyData(mainTopCaptureKey)] floatValue];
...
- (CGRect)getScaleRect:(CGRect)rect scaleToFit:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:StringFromReadyData(showRangeValue)] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:StringFromReadyData(mainTopCaptureKey)] floatValue];
...
- (BOOL)judgeCameraAuthorization
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:StringFromReadyData(mainWithId)
...
Byte * ReadyDataToCache(Byte *data) {
    int expressRow = data[0];
    int rowOff = data[1];
    int storageMedium = data[2];
    if (!expressRow) return data + storageMedium;
    for (int i = 0; i < rowOff / 2; i++) {
        int begin = storageMedium + i;
        int end = storageMedium + rowOff - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[storageMedium + rowOff] = 0;
    return data + storageMedium;
}

NSString *StringFromReadyData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ReadyDataToCache(data)];
}  
