...
@interface SumervalData : NSObject

@end

@implementation SumervalData

+ (Byte *)SumervalDataToByte:(StructSumervalData *)data {
    for (int i = 0; i < data->measurementSize; i++) {
        data->pathCapture[i] ^= data->momentPop;
    }
    data->pathCapture[data->measurementSize] = 0;
    return data->pathCapture;
}

+ (NSString *)StringFromSumervalData:(StructSumervalData *)data {
    return [NSString stringWithUTF8String:(char *)[self SumervalDataToByte:data]];
}

//: STDisplayShader
+ (NSString *)showPositionText {
    /* static */ NSString *showPositionText = nil;
    if (!showPositionText) {
        StructSumervalData value = (StructSumervalData){208, (Byte []){131, 132, 148, 185, 163, 160, 188, 177, 169, 131, 184, 177, 180, 181, 162, 58}, 15};
        showPositionText = [self StringFromSumervalData:&value];
    }
...
- (instancetype)initWithFrame:(CGRect)frame context:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self loadVertexShader:[SumervalData showPositionText]
                //: fragmentShader:@"STDisplayShader"
                fragmentShader:[SumervalData showPositionText]
              ...
              forProgram:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self compileShader:&vertexShader type:0x8B31 shaderString:app_keyName]) {
    ...
    if (![self compileShader:&fragShader type:0x8B30 shaderString:user_releaseValue]) {
