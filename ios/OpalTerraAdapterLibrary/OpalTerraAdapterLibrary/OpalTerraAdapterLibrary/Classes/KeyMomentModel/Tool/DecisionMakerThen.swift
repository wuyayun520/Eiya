...
/*: "%@%@" :*/
fileprivate let k_firstViewData:String = "%moment%moment"

/*: "/TXCache" :*/
fileprivate let kInstanceName:String = "/TXCadjust actual"
...
protocol PlayerMakeThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func menu(player: DecisionMakerThen, status: ObjectPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func decisionMaker(player: DecisionMakerThen, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func playerAction(player: DecisionMakerThen, progress: CGFloat)
}
...
class DecisionMakerThen: NSObject {
    ...
    open weak var delegate: PlayerMakeThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: ObjectPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = DecisionMakerThen()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(kInstanceName.prefix(4)) + "ache")) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(group), userInfo: nil, repeats: true)
...
extension DecisionMakerThen {
    ...
    func dataView(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: data_statusText.doingApp(), with: main_keyValue)
    ...
    @objc func group() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.playerAction(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func sharedMode(renderMode: RenderMode) {
    ...
    func setFor(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isSpace == true {
        if ListSocketManager.shared.isSpace == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func imageThe() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.menu(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func equalLabel() -> Bool {
...
extension DecisionMakerThen: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.decisionMaker(player: self, duration: Int(duration), currentTime: Int(currentTime))
