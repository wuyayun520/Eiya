...
fileprivate let kShouldValue:[UInt8] = [0x81,0x86,0x81,0x9c,0xc0,0x8b,0x87,0x8c,0x8d,0x9a,0xd2,0xc1,0xc8,0x80,0x89,0x9b,0xc8,0x86,0x87,0x9c,0xc8,0x8a,0x8d,0x8d,0x86,0xc8,0x81,0x85,0x98,0x84,0x8d,0x85,0x8d,0x86,0x9c,0x8d,0x8c]

private func submitEqual(title num: UInt8) -> UInt8 {
    return num ^ 232
}
...
class KeyViewDelegate: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kShouldValue.map{submitEqual(title: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func dateMake() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(fortifyTap))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(clearTitle))
...
extension KeyViewDelegate: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func fortifyTap(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func clearTitle(tap: UIGestureRecognizer) {
