...
fileprivate let kPicFramePath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#4FAAFF" :*/
fileprivate let kNameManagerPath:[Character] = ["#","4"]
fileprivate let kMakeAddScaleMessage:String = "FstyleFF"

/*: "reply" :*/
fileprivate let k_titleData:String = "reaccounty"

/*: "： :*/
fileprivate let kEndId:String = "："

/*: "F5F5F5" :*/
fileprivate let k_viewTitle:String = "touch5touch5touch5"
...
class MyViewItemCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kPicFramePath.reversed(), encoding: .utf8)!)
    }
...
extension MyViewItemCell {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func submit(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension MyViewItemCell {
    //: func configCell(model: TalkingCommentReplyModel) {
    func enableView(model: ObjectModelType) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(kNameManagerPath) + kMakeAddScaleMessage.replacingOccurrences(of: "style", with: "AA")))
            ...
            let font = UIFont.targetFor(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (k_titleData.replacingOccurrences(of: "account", with: "pl")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.colorStatuteNameApp(), UIColor.appValueDetailColor(), UIColor.colorStatuteNameApp()]
                colorA = [UIColor.childMenu(), UIColor.colorStatuteNameApp(), UIColor.childMenu(), UIColor.colorStatuteNameApp()]
                ...
                strA = [model.nickname, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.colorStatuteNameApp()]
                colorA = [UIColor.childMenu(), UIColor.colorStatuteNameApp()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func topAction(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: constFormalContent - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (k_viewTitle.replacingOccurrences(of: "touch", with: "F")))?.cgColor
...
extension MyViewItemCell {
    ...
    private func numbereraction() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(submit(sender:)))
