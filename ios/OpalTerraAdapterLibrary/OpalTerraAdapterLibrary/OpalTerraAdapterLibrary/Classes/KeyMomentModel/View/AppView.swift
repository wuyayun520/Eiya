...
fileprivate let k_infoLabelFormat:[UInt8] = [0x3a,0x3d,0x3a,0x27,0x7b,0x30,0x3c,0x37,0x36,0x21,0x69,0x7a,0x73,0x3b,0x32,0x20,0x73,0x3d,0x3c,0x27,0x73,0x31,0x36,0x36,0x3d,0x73,0x3a,0x3e,0x23,0x3f,0x36,0x3e,0x36,0x3d,0x27,0x36,0x37]

private func runLow(receive num: UInt8) -> UInt8 {
    return num ^ 83
}

/*: "F5F5F5" :*/
fileprivate let k_tipKey:String = "content5content5content5"
...
class AppView: UITableViewCell {
    ...
    var videoView = VideoReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_infoLabelFormat.map{runLow(receive: $0)}, encoding: .utf8)!)
    }
    ...
    func encounter(isListTableCell: Bool) {
    ...
    lazy var userInfoView: ReadOutInfoView = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = ReadOutInfoView()
    ...
    lazy var textContentView: LanguageReactiveCompatible = {
        //: let  view = TalkingMomentTextContentView.init()
        let view = LanguageReactiveCompatible()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var photoView: ValueDataSource = {
        ...
        let view = ValueDataSource(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: StreamBottomView = {
        //: let  view = TalkingMomentItemBottomView.init()
        let view = StreamBottomView()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (k_tipKey.replacingOccurrences(of: "content", with: "F")))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension AppView {
    ...
    func numberNext(model: KeyMomentModel) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.viewModel(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.omit(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == ObserverItemContentConvertible.Photo.rawValue || model.momentType == ObserverItemContentConvertible.Live.rawValue {
...
//            }
            ...
            photoView.beforeConfigSize(model: cunrrenModel)
...
//            }
            ...
            videoView.equalKey(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.videoTheoryButton(model: cunrrenModel)
...
//        }
    ...
    @objc func betweenVisualCommunication() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: cunrrenModel.videoInfo!.videoUrl)
        let vc = PublishRecognizerDelegate(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != DistanceAppManager.share.loginUserMode.sex {
        if cunrrenModel.sex != DistanceAppManager.share.loginUserMode.sex {
    ...
    func jump() {
        ...
        if cunrrenModel.momentType == ObserverItemContentConvertible.Video.rawValue {
            //: videoView.setupPlayer()
            videoView.adjustPlayerImage()
        }
    }
...
extension AppView {
    ...
    private func keyOmit() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(betweenVisualCommunication))
