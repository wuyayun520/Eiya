...
/*: "id" :*/
fileprivate let k_shareMessage:[Character] = ["i","d"]
...
struct KeyMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var videoInfo: TouchFrameTransformable?
...
enum ObserverItemContentConvertible: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct TouchFrameTransformable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: MatterTextLiteral?
}

///
//: enum MomentVideoStatus: String {
enum MatterTextLiteral: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension KeyMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func titleBy(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            //: textHeight = Int(TalkingMomentTextSizeView.caculateSizeWithText(text: content!, size: CGSize.init(width: ScreenWidth-54, height: CGFloat(MAXFLOAT)), font: UIFont.fontChange(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber==0 ? 20:0))
            textHeight = Int(CancelView.command(text: content!, size: CGSize(width: constFormalContent - 54, height: CGFloat(MAXFLOAT)), font: UIFont.fontChange(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == ObserverItemContentConvertible.Photo.rawValue || momentType == ObserverItemContentConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
