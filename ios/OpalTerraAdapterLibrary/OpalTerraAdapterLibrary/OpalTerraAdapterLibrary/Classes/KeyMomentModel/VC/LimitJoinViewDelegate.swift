...
fileprivate let k_femaleId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Save successfully" :*/
fileprivate let k_partyFormat:[Character] = ["S","a","v","e"," ","s","u"]
fileprivate let k_toolGreenId:String = "normales"

/*: / :*/
fileprivate let k_aMsg:[Character] = ["/"]
...
class LimitJoinViewDelegate: SubmarineViewController {
    ...
    init(momentModel: KeyMomentModel, index: Int, type: MakePhotoType) {
        ...
        if MakeLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(constFormalContent), y: 0)
    ...
    init(images: [UIImage], index: Int, type: MakePhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_femaleId.reversed(), encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: constFormalContent, height: user_recordMarginValue)
        ...
        view.frame = CGRect(x: 0, y: 0, width: constFormalContent, height: user_recordMarginValue)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(constFormalContent), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(constFormalContent), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(constFormalContent), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(KeyViewDelegate.self, forCellWithReuseIdentifier: KeyViewDelegate.className())
...
//    lazy var downloadBtn: ResumeControl = {
//        let btn = ResumeControl.init()
//        btn.setTitle("Save".localized, for: .normal)
//        btn.titleLabel?.font = .fontChange(type: .Regular, fontSize: 16)
...
extension LimitJoinViewDelegate {
...
//        ToolThen.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.rear(showMsg: (String(k_partyFormat) + k_toolGreenId.replacingOccurrences(of: "normal", with: "cc") + "sfully").localized)
                }
                //: } else {
            } else {
...
extension LimitJoinViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func exaggerate() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func viewComment() {
        //: if  currenModel.sex==DistanceAppManager.share.loginUserMode.sex {
        if currenModel.sex == DistanceAppManager.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: KeyViewDelegate = collectionView.dequeueReusableCell(withReuseIdentifier: KeyViewDelegate.className(), for: indexPath) as! KeyViewDelegate
        //: if myType == .normal {
        if myType == .normal {
            ...
            if model.imgUrl?.count ?? 0 > 1 {
                ...
                cell.routeUrl(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.queryRead(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.queryRead(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: KeyViewDelegate = cell as! KeyViewDelegate
        //: tcell.recoverSubviews()
        tcell.getObjectSubviews()
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / constFormalContent)
        //: if myType == .normal {
        if myType == .normal {
