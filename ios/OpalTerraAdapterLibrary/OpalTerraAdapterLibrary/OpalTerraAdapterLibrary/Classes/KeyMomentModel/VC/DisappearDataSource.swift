...
/*: "tab" :*/
fileprivate let kLeadingTitle:String = "TAB"

/*: "page" :*/
fileprivate let kMouthPhotoPath:[Character] = ["p","a","g","e"]

/*: "uid" :*/
fileprivate let kDataFormat:[Character] = ["u","i","d"]

/*: "model" :*/
fileprivate let kOfName:[Character] = ["m","o","d","e","l"]

/*: "There's no feed yet." :*/
fileprivate let k_videoName:[Character] = ["T","h","e","r","e","\'","s"," ","n","o"," ","f","e","e","d"]
fileprivate let kCenterPath:String = " yet.succeed max self file"
...
class DisappearDataSource: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: dataImageItemValue, right: 0)
        //: return table
        return table
...
extension DisappearDataSource {
    //: func reqData() {
    func removeColorData() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(kLeadingTitle.lowercased())] = 1
        //: dict["page"] = pageIndex
        dict[(String(kMouthPhotoPath))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        TvThen.fail(params: dict) { succeed, result, errorModel in
...
extension DisappearDataSource {
    ...
    @objc func alongPage(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(String(kDataFormat))] as! String
    ...
    @objc func modifyWithNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: KeyMomentModel = userinfo[(String(kOfName))] as! KeyMomentModel
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
...
extension DisappearDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = AppView.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? AppView
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = AppView(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == ObserverItemContentConvertible.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: AppView = cell as? AppView else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension DisappearDataSource: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        rate(scrollView: scrollView)
    }
    ...
    func rate(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: AppView = mainTableView.cellForRow(at: middleIndexPath) as! AppView
...
// MARK: - IndependentControllerDelegate

//: extension TalkingPopularMomentVC {
extension DisappearDataSource {
    //: @objc func PostSuccess() {
    @objc func upwardsSuccess() {
        //: mainTableView.mj_header?.beginRefreshing()
        mainTableView.mj_header?.beginRefreshing()
    }
}
...
extension DisappearDataSource {
    ...
    private func formation() {
        ...
        style.TipsTitle = (String(k_videoName) + String(kCenterPath.prefix(5))).localized
    ...
    private func accountCounteraction() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(modifyWithNotification(notification:)), name: dataTabText, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(alongPage(notification:)), name: show_needStatusText, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.notification(notification:)), name: notiScaleName, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(upwardsSuccess), name: userVideoCancelData, object: nil)
    }
}
