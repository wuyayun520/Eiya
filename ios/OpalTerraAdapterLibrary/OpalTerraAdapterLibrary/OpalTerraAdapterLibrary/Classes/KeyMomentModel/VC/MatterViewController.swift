...
fileprivate let kShareText:[UInt8] = [0x75,0x7a,0x75,0x80,0x34,0x6f,0x7b,0x70,0x71,0x7e,0x46,0x35,0x2c,0x74,0x6d,0x7f,0x2c,0x7a,0x7b,0x80,0x2c,0x6e,0x71,0x71,0x7a,0x2c,0x75,0x79,0x7c,0x78,0x71,0x79,0x71,0x7a,0x80,0x71,0x70]

fileprivate func userStatus(location num: UInt8) -> UInt8 {
...
/*: "momentId" :*/
fileprivate let k_objectValue:[UInt8] = [0xa0,0xa2,0xa0,0xa8,0xa3,0xb9,0x84,0xa9]

private func firstListen(table num: UInt8) -> UInt8 {
    return num ^ 205
}

/*: "lastId" :*/
fileprivate let k_writeMessage:[Character] = ["l"]
fileprivate let k_coverStr:String = "of presentastId"

/*: "limit" :*/
fileprivate let kMinName:String = "observermit"

/*: "list" :*/
fileprivate let k_errorUrl:String = "lequalt"

/*: "more>>" :*/
fileprivate let kValueMsg:String = "action hidden guard self conversationmore>>"

/*: "Comment on success" :*/
fileprivate let k_screenMessage:[Character] = ["C","o","m","m","e","n","t"," ","o","n"," ","s","u","c","c","e","s","s"]

/*: "replyId" :*/
fileprivate let kGestureFormat:[Character] = ["r","e","p","l","y","I","d"]

/*: "type" :*/
fileprivate let k_subId:[UInt8] = [0x49,0x44,0x4d,0x58]

private func toPath(button num: UInt8) -> UInt8 {
    return num ^ 61
}

/*: "comment" :*/
fileprivate let kRecordMessage:[Character] = ["c","o"]
fileprivate let kMagnitudeListPath:String = "MMENT"

/*: "number" :*/
fileprivate let k_normalTitle:[UInt8] = [0xed,0xf4,0xec,0xe1,0xe4,0xf1]

fileprivate func tableCase(voice num: UInt8) -> UInt8 {
...
/*: "Delete Post" :*/
fileprivate let k_tableText:String = "short resultDelet"
fileprivate let kInfoName:String = "index"
fileprivate let k_sizeFormat:[Character] = [" ","P","o","s","t"]

/*: "id" :*/
fileprivate let k_noId:[Character] = ["i","d"]

/*: "model" :*/
fileprivate let kBodyValue:[UInt8] = [0x6c,0x65,0x64,0x6f,0x6d]

/*: "The content cannot be empty!" :*/
fileprivate let kBottomId:String = "The cbegin read normal"
fileprivate let k_centerName:String = "interval if disabledt ca"
fileprivate let kVideoTitle:[Character] = ["n","n","o","t"," ","b","e"," ","e","m","p","t","y","!"]

/*: "content" :*/
fileprivate let kMaxMessage:[Character] = ["c","o","n","t","e","n","t"]

/*: "Delete" :*/
fileprivate let k_myTitleMessage:String = "Deletelab row to"

/*: "Delete Comment?" :*/
fileprivate let kCurrentStr:[Character] = ["D","e","l","e","t","e"," ","C","o","m","m","e","n"]
fileprivate let k_shouldId:String = "t?info self model super"

/*: "Cancel" :*/
fileprivate let k_userStr:[Character] = ["C","a","n","c","e","l"]

/*: "Reply" :*/
fileprivate let kMakeName:String = "model make submit superReply"

/*: "  :*/
fileprivate let k_playerLoadMsg:[Character] = [" "]

/*: "Details" :*/
fileprivate let kEqualUrl:String = "stroke height card left infoDetails"

/*: "icon_moment_nor_report" :*/
fileprivate let kResponseTitle:String = "icon_mmanager center content"
fileprivate let k_bottomMsg:String = "closeen"
fileprivate let k_kindName:String = "cell app max of_report"

/*: "Comment" :*/
fileprivate let k_upMessage:[Character] = ["C"]
fileprivate let k_valueToName:[Character] = ["o","m","m","e","n","t"]
...
class MatterViewController: StartThen {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kShareText.map{userStatus(location: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var mainTableView: UITableView = {
        ...
        let table = UITableView(frame: CGRect(x: 0, y: 0, width: constFormalContent, height: user_recordMarginValue), style: UITableView.Style.grouped)
        ...
        table.register(MyViewItemCell.self, forCellReuseIdentifier: MyViewItemCell.className())
    ...
    lazy var comInputView: PersonThen = {
    ...
    lazy var backBtn: UIButton = {
        ...
        btn.languageUnitState(color: .clear, forState: .normal)
        //: btn.addTarget( self, action: #selector(dismissClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(dripPan), for: .touchUpInside)
...
extension MatterViewController {
    ...
    func outline() {
        ...
        dict[String(bytes: k_objectValue.map{firstListen(table: $0)}, encoding: .utf8)!] = momentModel.mid

        //: TalkingMomentRequestTool.req_MomentDetail(params: dict) { succeed, result, errorModel in
        TvThen.completion(params: dict) { succeed, result, errorModel in

            //: if succeed {
            if succeed {
                //: if var model = JSONDeserializer<TalkingMomentModel>.deserializeFrom(dict: result as? Dictionary<String, Any>, designatedPath: nil) {
                if var model = JSONDeserializer<KeyMomentModel>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
    ...
    func nearFeedback() {
        ...
        dict[String(bytes: k_objectValue.map{firstListen(table: $0)}, encoding: .utf8)!] = momentModel.mid
        //: dict["lastId"] = lastId
        dict[(String(k_writeMessage) + String(k_coverStr.suffix(5)))] = lastId
        //: dict["limit"] = limit
        dict[(kMinName.replacingOccurrences(of: "observer", with: "li"))] = limit
        //: ProgressHUD.show()
        EndProgressHUD.listHide()

        //: TalkingMomentRequestTool.req_CommentList(params: dict) { succeed, result, errorModel in
        TvThen.alongCell(params: dict) { succeed, result, errorModel in
            ...
            if succeed {
                ...
                let array: Array = dict[(k_errorUrl.replacingOccurrences(of: "equal", with: "is"))] as! [Any]
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<EndHandyJSON>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
                        //: model.caculateCommentItemHeight()
                        model.indicatorAppearHeight()
                        //: var replyArray: [TalkingCommentReplyModel] = []
                        var replyArray: [ObjectModelType] = []
                        //: for j in 0..<model.comment.count {
                        for j in 0 ..< model.comment.count {
                            //: var replymodel = model.comment[j]
                            var replymodel = model.comment[j]
                            //: replymodel.caculateReplyItemHeight()
                            replymodel.overdoHide()
                        ...
                        if model.totalReply > 3 {
                            ...
                            var replymodel = ObjectModelType()
                            //: replymodel.allCountContent = String(format: "more>>")
                            replymodel.allCountContent = String(format: (String(kValueMsg.suffix(6))))
                            //: replymodel.caculateReplyItemHeight()
                            replymodel.overdoHide()
                            //: replyArray.append(replymodel)
                            replyArray.append(replymodel)
    ...
    func requestNameImpregnabilityBoundary(params: [String: Any], index: Int) {
        //: ProgressHUD.show()
        EndProgressHUD.listHide()

        //: TalkingMomentRequestTool.req_SendComment(params: params) { [self] succeed, result, errorModel in
        TvThen.number(params: params) { [self] succeed, result, _ in
            ...
            if succeed {
                ...
                self.rear(showMsg: (String(k_screenMessage)).localized)
                //: if params["replyId"] != nil {
                if params[(String(kGestureFormat))] != nil {
                    //: var model = TalkingCommentModel()
                    var model = EndHandyJSON()
                    ...
                    if model.uid!.count > 0 {
                        ...
                        if var replyModel = JSONDeserializer<ObjectModelType>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                            //: model.totalReply += 1
                            model.totalReply += 1
...
//                            if array.count>3 {
//                                array.removeLast()
//                                var tempModel = ObjectModelType.init()
...
//                            }
                    ...
                    if var model = JSONDeserializer<EndHandyJSON>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        //: model.caculateCommentItemHeight()
                        model.indicatorAppearHeight()
                        //: if model.uid == DistanceAppManager.share.loginUserMode.userID {
                        if model.uid == DistanceAppManager.share.loginUserMode.userID {
                            //: model.headPicFrame = DistanceAppManager.share.loginUserMode.headPicFrame
                            model.headPicFrame = DistanceAppManager.share.loginUserMode.headPicFrame
                        }
                ...
                NotificationCenter.default.post(name: mainInfoText, object: self, userInfo: [String(bytes: k_subId.map{toPath(button: $0)}, encoding: .utf8)!: (String(kRecordMessage) + kMagnitudeListPath.lowercased()).localized, String(bytes: k_normalTitle.map{tableCase(voice: $0)}, encoding: .utf8)!: (self.momentModel.replyNum) ?? 0, String(bytes: k_objectValue.map{firstListen(table: $0)}, encoding: .utf8)!: self.momentModel.mid ?? ""])
            }
        }
    }
    ...
    func minimumDismiss(cid: String, lastId: String, limit: Int, index: IndexPath) {
        //: ProgressHUD.show()
        EndProgressHUD.listHide()
        //: EndProgressHUD.popEnable()
        EndProgressHUD.popEnable()
        ...
        var dict = [String: Any]()
        //: dict["replyId"] = cid
        dict[(String(kGestureFormat))] = cid
        //: dict["lastId"] = lastId
        dict[(String(k_writeMessage) + String(k_coverStr.suffix(5)))] = lastId
        //: dict["limit"] = limit
        dict[(kMinName.replacingOccurrences(of: "observer", with: "li"))] = limit
        ...
        TvThen.split(params: dict) { succeed, result, _ in
            //: EndProgressHUD.popEnable()
            EndProgressHUD.popEnable()
            //: if succeed {
            if succeed {
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<ObjectModelType>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
...
extension MatterViewController {
    ...
    func rawData() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
            ...
            self.mainTableView.scrollRectToVisible(CGRect(x: 0, y: self.momentModel.itemHeight ?? 56, width: constFormalContent, height: user_recordMarginValue - CGFloat(self.inputViewHeight) - appButtonData - appTurnName), animated: true)
        }
    }

    //: @objc func BlockBtnClick() {
    @objc func bangorThen() {
        //: if self.momentModel.uid == DistanceAppManager.share.loginUserMode.userID {
        if self.momentModel.uid == DistanceAppManager.share.loginUserMode.userID {
            //: let vc = TalkingMunuPopView.init(frame: self.view.frame)
            let vc = LipogramViewDelegate(frame: self.view.frame)
            //: vc.initwithList(cellTitleList: ["Delete Post".localized])
            vc.listEffect(cellTitleList: [(String(k_tableText.suffix(5)) + kInfoName.replacingOccurrences(of: "index", with: "e") + String(k_sizeFormat)).localized])
            ...
            vc.munuBlock = { [weak self] _, _ in
                ...
                TvThen.extragalacticNebula(params: dict) { succeed, _, _ in
                    //: EndProgressHUD.popEnable()
                    EndProgressHUD.popEnable()
                    //: if succeed {
                    if succeed {
                        ...
                        NotificationCenter.default.post(name: notiScaleName, object: self, userInfo: [String(bytes: kBodyValue.reversed(), encoding: .utf8)!: self.momentModel])
                    }
                }
            }
            ...
            let reportView = InformDataSource(frame: UIScreen.main.bounds, type: .reportUserType, rId: momentModel.uid ?? "")
    ...
    @objc func dripPan() {
        ...
        comInputView.buttonWith()
    }
}

// MARK: - ModeViewDelegate

//: extension TalkingMomentDetailVC: CommentInputViewDelegate {
extension MatterViewController: ModeViewDelegate {
    //: func func__replyTextMsg(msgStr: String, row: IndexPath) {
    func dataCheck(msgStr: String, row: IndexPath) {
        //: if msgStr.count == 0 {
        if msgStr.count == 0 {
            //: self.func__showStatusBarErrorMsg(showMsg: "The content cannot be empty!".localized)
            self.arcInfo(showMsg: (String(kBottomId.prefix(5)) + "onten" + String(k_centerName.suffix(4)) + String(kVideoTitle)).localized)
        }
        ...
        dict[String(bytes: k_objectValue.map{firstListen(table: $0)}, encoding: .utf8)!] = momentModel.mid
        //: dict["content"] = msgStr
        dict[(String(kMaxMessage))] = msgStr
        ...
        dict[(String(kGestureFormat))] = replymodel.cid
        //: requestCommentWithParams(params: dict, index: row.section)
        requestNameImpregnabilityBoundary(params: dict, index: row.section)
    }

    //: func func__sendTextMsg(msgStr: String, index: Int) {
    func my(msgStr: String, index: Int) {
        //: if msgStr.count == 0 {
        if msgStr.count == 0 {
            //: self.func__showStatusBarErrorMsg(showMsg: "The content cannot be empty!".localized)
            self.arcInfo(showMsg: (String(kBottomId.prefix(5)) + "onten" + String(k_centerName.suffix(4)) + String(kVideoTitle)).localized)
        ...
        if index >= 0 {
            ...
            dict[(String(kGestureFormat))] = model.cid
            //: } else if index == quickCommentReplyIndex {
        } else if index == quickCommentReplyIndex {}

        //: requestCommentWithParams(params: dict, index: index)
        requestNameImpregnabilityBoundary(params: dict, index: index)
    }

    //: func heightToBottomChanged(heightToBottom: CGFloat) {
    func fromTo(heightToBottom: CGFloat) {
        ...
        if heightToBottom > 100 {
            ...
            backBtn.frame = CGRect(x: 0, y: 0, width: constFormalContent, height: self.view.frame.size.height - heightToBottom - inputViewHeight)
    ...
    func contrive(height: CGFloat) {
        ...
        backBtn.frame = CGRect(x: 0, y: 0, width: constFormalContent, height: backBtn.frame.size.height - height)
        //: backBtn.isHidden = false
        backBtn.isHidden = false
    ...
    func spectrogram(cid: String) {
        //: if self.momentModel.uid == DistanceAppManager.share.loginUserMode.userID {
        if self.momentModel.uid == DistanceAppManager.share.loginUserMode.userID {
            //: let vc = TalkingMunuPopView.init(frame: self.view.frame)
            let vc = LipogramViewDelegate(frame: self.view.frame)
            //: vc.initwithList(cellTitleList: ["Delete".localized])
            vc.listEffect(cellTitleList: [(String(k_myTitleMessage.prefix(6))).localized])
            ...
            vc.munuBlock = { [weak self] _, _ in
                ...
                FrameHiddenReactiveCompatible.belowText(title: (String(kCurrentStr) + String(k_shouldId.prefix(2))).localized, attributedMessage: NSMutableAttributedString(), leftBtnTitle: (String(k_userStr)).localized, rightBtnTitle: (String(k_myTitleMessage.prefix(6))).localized) {
                    ...
                    TvThen.laterDoing(params: dict) { succeed, _, _ in
                        //: EndProgressHUD.popEnable()
                        EndProgressHUD.popEnable()
                        //: if succeed {
                        if succeed {
...
extension MatterViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = MyViewItemCell.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingCommentReplyItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? MyViewItemCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingCommentReplyItemCell(style: .default, reuseIdentifier: identifier)
            cell = MyViewItemCell(style: .default, reuseIdentifier: identifier)
        }
        //: let model = self.commentList[indexPath.section]
        let model = self.commentList[indexPath.section]
        ...
        if model.comment.count > 0 {
            ...
            cell?.enableView(model: replymodel)
        }
        //: return cell!
        return cell!
    }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        let tcell: MyViewItemCell = cell as! MyViewItemCell
        //: tcell.shearTableViewSection(cell: cell, tableView: tableView, indexPath: indexPath as NSIndexPath, radius: 6, height: height)
        tcell.topAction(cell: cell, tableView: tableView, indexPath: indexPath as NSIndexPath, radius: 6, height: height)
    }
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if replymodel.allCountContent == (String(kValueMsg.suffix(6))) {
            ...
            minimumDismiss(cid: model.cid!, lastId: lastmodel.cid ?? "", limit: 5, index: indexPath)
            //: } else {
        } else {
            //: self.comInputView.updatePlaceholder(holder: "Reply".localized + "\(replymodel.nickname)" + ":", index: 0, row: indexPath, type: 2)
            self.comInputView.withPair(holder: (String(kMakeName.suffix(5))).localized + "\(replymodel.nickname)" + ":", index: 0, row: indexPath, type: 2)
        }
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headeView = AListView(style: .default, reuseIdentifier: nil)
        //: let model = self.commentList[section]
        let model = self.commentList[section]
        //: headeView.configCell(model: model)
        headeView.compartmentVideo(model: model)
        ...
        headeView.commentReplyBlock = { [weak self] nickname in
            ...
            self.comInputView.withPair(holder: (String(kMakeName.suffix(5))).localized + " \(nickname)" + ":", index: section, row: IndexPath(), type: 1)
        }
...
extension MatterViewController {
    ...
    private func bailOutSubviews() {
        //: self.view.backgroundColor = UIColor.white
        self.view.backgroundColor = UIColor.white
        //: self.title = "Details".localized
        self.title = (String(kEqualUrl.suffix(7))).localized
        ...
        if self.momentModel.uid == DistanceAppManager.share.loginUserMode.userID {
            ...
            btn.setTitle((String(k_myTitleMessage.prefix(6))).localized, for: .normal)
            ...
            btn.setImage(UIImage.submitGift(name: (String(kResponseTitle.prefix(6)) + k_bottomMsg.replacingOccurrences(of: "close", with: "om") + "t_nor" + String(k_kindName.suffix(7)))), for: .normal)
        }
        //: btn.addTarget(self, action: #selector(BlockBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(bangorThen), for: .touchUpInside)
    ...
    func popTo() {
        ...
        let cell = AppView(style: .default, reuseIdentifier: nil)
        //: cell.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: momentModel.itemHeight ?? 56 - 12 )
        cell.frame = CGRect(x: 0, y: 0, width: constFormalContent, height: momentModel.itemHeight ?? 56 - 12)
        //: cell.initwith(isListTableCell: false)
        cell.encounter(isListTableCell: false)
        //: cell.configCell(model: momentModel)
        cell.numberNext(model: momentModel)
        //: cell.playVideo()
        cell.jump()
        ...
        if momentModel.replyNum! > 0 {
            ...
            commentNumberLabel.text = String(format: (String(k_upMessage) + String(k_valueToName)).localized + " \(momentModel.replyNum!)")
            ...
            commentNumberLabel.snp.makeConstraints { make in
                ...
                make.size.equalTo(CGSize(width: constFormalContent, height: 22))
            }
