...
fileprivate let k_grayTitle:[UInt8] = [0x55,0x5a,0x55,0x60,0x14,0x4f,0x5b,0x50,0x51,0x5e,0x26,0x15,0xc,0x54,0x4d,0x5f,0xc,0x5a,0x5b,0x60,0xc,0x4e,0x51,0x51,0x5a,0xc,0x55,0x59,0x5c,0x58,0x51,0x59,0x51,0x5a,0x60,0x51,0x50]

fileprivate func spreadHead(leading num: UInt8) -> UInt8 {
...
/*: "%@.mp4" :*/
fileprivate let kDataDateKey:[Character] = ["%","@",".","m","p","4"]

/*: "http" :*/
fileprivate let kRawStr:[Character] = ["h","t","t","p"]
...
class PopItemView: AtView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_grayTitle.map{spreadHead(leading: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(trainColor))
    ...
    override func toAnimating() {
        ...
        let path = FinancialAidThen.shared.rent(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? DisappearMsgModel())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(kDataDateKey)), path)
            ...
            self.delegate?.sectionEnd(effectItemView: self)
        }
    }
    ...
    @objc func trainColor() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension PopItemView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((String(kRawStr))) {
