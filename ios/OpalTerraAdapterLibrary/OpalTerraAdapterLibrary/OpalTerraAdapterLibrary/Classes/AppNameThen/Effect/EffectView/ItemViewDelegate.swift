...
fileprivate let kReTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let k_makeTitle:String = "view info for image crushiosSvg"

/*: "iosEffect" :*/
fileprivate let k_placeName:String = "iosEffecttrue data"

/*: "entityId" :*/
fileprivate let k_addLineNameData:String = "entinfo"
fileprivate let kInstanceName:[Character] = ["y","I","d"]
...
class ItemViewDelegate: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: DisappearMsgModel?
    ...
    var animatedView: AtView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: FrameStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kReTitle.reversed(), encoding: .utf8)!)
    }
...
extension ItemViewDelegate {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func fumbleInsiderInformationFromDic(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(k_makeTitle.suffix(6)) + "aEffect")] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(k_placeName.prefix(9)))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(k_addLineNameData.replacingOccurrences(of: "info", with: "it") + String(kInstanceName))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = computerSimulation(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func computerSimulation(msgModel: Any?) -> Bool {
        ...
        if msgModel is DisappearMsgModel {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? DisappearMsgModel
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is TitleManagerMeasurable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! TitleManagerMeasurable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.equalStatus(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = DisappearMsgModel()
            ...
            effectMsgModel = DisappearMsgModel()
            ...
            if giftMsgModel.showType == BugOutMirrorPath.myStery.rawValue || giftMsgModel.showType == BugOutMirrorPath.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: DisappearMsgModel = self.effectMsgModelArray[i] as! DisappearMsgModel
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? DisappearMsgModel())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? DisappearMsgModel(), at: index)
                }
            ...
            firstViewModel(effectMsgModel: effectMsgModel!)
        }
...
extension ItemViewDelegate {
    ...
    func compare() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = showModel() else { return }
                //: firstShow(effectMsgModel: removeModel)
                firstViewModel(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.administrator()
        }
    ...
    func showModel() -> DisappearMsgModel? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return DisappearMsgModel()
        }
        ...
        if effectMsgModel is DisappearMsgModel {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! DisappearMsgModel
        ...
        return effectMsgModel as? DisappearMsgModel
    }
}
...
extension ItemViewDelegate {
    ...
    func firstViewModel(effectMsgModel: DisappearMsgModel) {
        ...
        self.delegate?.viewConstraint(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        FinancialAidThen.shared.imageAction(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.tableBe(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? MakeHandyJSON())
            //: })
        })
    }
    ...
    func tableBe(effectMsgModel: DisappearMsgModel, success: Bool, effectModel: MakeHandyJSON) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.containerSuccess(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    sizeAcross(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func shareOf(effectMsgModel: DisappearMsgModel, effectModel: MakeHandyJSON) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.specs()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.kindMaxPin()
        ...
        className = FinancialAidThen.shared.eyeContactImage(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! AtView.Type
...
extension ItemViewDelegate: SizeJoinThen {
    ...
    func happeningSuccess(effectItemView: AtView, success: Bool) {
        ...
        self.delegate?.filter(effectView: self, obj: effectItemView.effectMsgModel ?? DisappearMsgModel(), success: success)
    }
    ...
    func sectionEnd(effectItemView: AtView) {
        ...
        self.delegate?.verbalise(effectView: self, obj: effectMsgModel!)
    }
}
...
extension ItemViewDelegate {
    ...
    func thoughtImage() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.specs() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.kindMaxPin() // clean animation data
