...
/*: "id" :*/
fileprivate let k_centerId:String = "ileading"

/*: "num" :*/
fileprivate let k_bottomName:[Character] = ["n","u","m"]
...
@objc public enum BugOutMirrorPath: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct TitleManagerMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// BugOutMirrorPath
    ...
    mutating func equalStatus(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct WithCloseTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(k_bottomName))
    }
}
