...
/*: "serialQueue" :*/
fileprivate let k_listId:[Character] = ["s","e","r","i","a"]
fileprivate let k_delayId:[Character] = ["l","Q","u","e","u","e"]

/*: "fromUids" :*/
fileprivate let k_sharedValue:[UInt8] = [0x5d,0x49,0x54,0x56,0x6e,0x52,0x5f,0x48]

private func chemistryLaboratoryVideo(range num: UInt8) -> UInt8 {
    return num ^ 59
}
...
let show_beautyData = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class AppNameThen: NSObject {
    ...
    private static var _instance: AppNameThen?
    //: class func shared() -> DBUserInfoManager {
    class func pushShared() -> AppNameThen {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = AppNameThen()
            // 添加退出账号通知
...
extension AppNameThen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([HumankindThen]?, Error?) -> Void) {
    class func secretFrom(with userIdsArr: [String], completion: @escaping ([HumankindThen]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if AppNameThen.pushShared().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(AppNameThen.pushShared().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            AppNameThen.pushShared().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            getAdd(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func getBy(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(k_listId) + String(k_delayId)))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: PlayerRequestTool.req_queryStatus(toUids: requestUidsStr) { _, list in
                    PlayerRequestTool.doOrigin(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func starExecute(message: V2TIMMessage) -> BearEnableMsgTable {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = IsoclinalThen.tabEnable(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = BearEnableMsgTable.sharedMini(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = IsoclinalThen.dict(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = BearEnableMsgTable.placeDb(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension AppNameThen {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> HumankindThen? {
    class func imageBar(targetId: String) -> HumankindThen? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return ConcealedInfoTable.purchaseSubWith(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: HumankindThen?) {
    class func toMedia(with userInfo: HumankindThen?) {
        ...
        let model = AppNameThen.imageBar(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        ConcealedInfoTable.compartment(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: app_messageTopData, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func equalBlock(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = AppNameThen.imageBar(targetId: targetId) else { return }
    ...
    class func server(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = AppNameThen.imageBar(targetId: targetId) else { return }
    ...
    class func contentWith(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        ConcealedInfoTable.messageGift(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        AppNameThen.pushShared().reqedUidsSet.remove(targetUid)
    }
}
...
extension AppNameThen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([HumankindThen]?, Error?) -> Void) {
    private class func getAdd(with userIdsArr: [String], completion: @escaping ([HumankindThen]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = ConcealedInfoTable.enableEqual(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if AppNameThen.pushShared().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            AppNameThen.dataFormatting(with: noDBUidArr)
        }
    }
    ...
    private class func dataFormatting(with userIdsArr: [String]) {
        ...
        AppNameThen.pushShared().reqedUidsSet = AppNameThen.pushShared().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: PlayerRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        PlayerRequestTool.usageConversation(param: [String(bytes: k_sharedValue.map{chemistryLaboratoryVideo(range: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                AppNameThen.pushShared().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            AppNameThen.furtherance(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: HumankindThen] = [:]
            var retInfoDict: [String: HumankindThen] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: app_messageTopData, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [HumankindThen]) {
    private class func furtherance(userInfoArr: [HumankindThen]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            ConcealedInfoTable.compartment(aWrap)
        }
    }
}
