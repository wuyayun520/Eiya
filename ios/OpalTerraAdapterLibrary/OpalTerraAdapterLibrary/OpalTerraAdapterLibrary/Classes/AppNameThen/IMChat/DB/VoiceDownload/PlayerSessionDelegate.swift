...
fileprivate let kLeadingPath:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","v"]
fileprivate let k_fromMsg:[Character] = ["o","i","c","e","/"]

/*: "Documents/User/record/" :*/
fileprivate let k_startPath:[Character] = ["D","o","c","u","m","e","n","t","s","/"]
fileprivate let kMakeName:String = "User/rinfo care var height"

/*: "cancelled" :*/
fileprivate let k_numberName:String = "CANC"

/*: .wav" :*/
fileprivate let kLabTitle:String = "temp try extension m.wav"
...
typealias OneKeyGreetDownloadBlock = (_ model: ViewStreamThen) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol StackObjectProtocol: NSObjectProtocol {
    /// 下载完成
    ...
    func overContent(model: ViewStreamThen)
    /// 过期语音消息，服务器缓存时间为7天
...
extension PlayerSessionDelegate {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func tasks(_ tasksArray: [ViewStreamThen]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.cornerRePath()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(kLeadingPath) + String(k_fromMsg))) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(CancelCacheDefine.needPath())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(k_startPath) + String(kMakeName.prefix(6)) + "ecord/")) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(CancelCacheDefine.tableDown())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            BearEnableMsgTable.doingModify(msgId: model.taskId, status: "\(AppComparable.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func keyDetail(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = ViewStreamThen.isolateMedium(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? ViewStreamThen()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(kLeadingPath) + String(k_fromMsg))) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(CancelCacheDefine.needPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(k_startPath) + String(kMakeName.prefix(6)) + "ecord/")) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(CancelCacheDefine.tableDown())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension PlayerSessionDelegate: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = ViewStreamThen.popular(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.overContent(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (k_numberName.lowercased() + "elled") {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    hangConversation(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.voiceNeedColor(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                hangConversation(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.netRow(url: task.taskDescription ?? "")
        }
...
extension PlayerSessionDelegate {
    ...
    func hangConversation(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(AppComparable.pause.rawValue)" : "\(AppComparable.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            BearEnableMsgTable.doingModify(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.usufructuaryModel(model: taskModel)
            }
    ...
    func voiceNeedColor(url: String) {
        //: guard let taskModel = self.downloadingModelDic[url] else { return }
        guard let taskModel = self.downloadingModelDic[url] else { return }
        //: self.downloadingModelDic.removeValue(forKey: url)
        self.downloadingModelDic.removeValue(forKey: url)
        ...
        case .chatRecord:
            ...
            BearEnableMsgTable.timePosition(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.exaggerateMessage(model: taskModel)
    ...
    func netRow(url: String) {
        ...
        let size = ViewStreamThen.popular(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                BearEnableMsgTable.doingModify(msgId: taskModel.taskId, status: "\(AppComparable.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(kLabTitle.suffix(4)))
            ...
            self.alongBindComplete(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.render(model: taskModel)
            ...
            case .chatRecord:
                ...
                BearEnableMsgTable.doingModify(msgId: taskModel.taskId, status: "\(AppComparable.expired.rawValue)")
    ...
    func alongBindComplete(at path: String, withKeyId keyId: String, withTypeId typeId: ManagerPropertyProtocol, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(kLabTitle.suffix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.dataObject()
                    ...
                    if typeId == .chatRecord {
                        ...
                        BearEnableMsgTable.doingModify(msgId: keyId, filePath: newPath)
                    }
