...
fileprivate let kShouldData:String = "TSystelse return date"
fileprivate let kBlockUrl:[Character] = ["e","m","M","e","s","s","a"]
fileprivate let k_labKey:[Character] = ["g","e","C","e","l","l","_","R","e","u","s","e","I","d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let kModelText:[UInt8] = [0x62,0x65,0x62,0x7f,0x23,0x68,0x64,0x6f,0x6e,0x79,0x31,0x22,0x2b,0x63,0x6a,0x78,0x2b,0x65,0x64,0x7f,0x2b,0x69,0x6e,0x6e,0x65,0x2b,0x62,0x66,0x7b,0x67,0x6e,0x66,0x6e,0x65,0x7f,0x6e,0x6f]

private func blockAdd(male num: UInt8) -> UInt8 {
    return num ^ 11
}

/*: "icon_talk_notic" :*/
fileprivate let kTagText:String = "icon_ttable type"
fileprivate let k_selectUrl:String = "arcic"
...
@objc public protocol PhoneMessageCellDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: MakeMessageCell)
    func hourlong(cell: MakeMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: MakeMessageCell)
    func tapCell(cell: MakeMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: MakeMessageCell)
    func equilateral(cell: MakeMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: MakeMessageCell)
    func statuteNameCell(cell: MakeMessageCell)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: MakeMessageCell)
    func radiogram(cell: MakeMessageCell)
}

//: @objcMembers public class MakeMessageCell: TCommonTableViewCell {
@objcMembers public class MakeMessageCell: MakeNameReactiveCompatible {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\InscriptionThen.h
     */
    //: public var messageData: InscriptionThen?
    public var messageData: InscriptionThen?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(kShouldData.prefix(5)) + String(kBlockUrl) + String(k_labKey))

    /**
     *  协议委托
     *  负责实现 PhoneMessageCellDelegate 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: PhoneMessageCellDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kModelText.map{blockAdd(male: $0)}, encoding: .utf8)!)
    }
    ...
    override public func afterMid(with data: DisappearReactiveCompatible) {
        //: super.fill(with: data)
        super.afterMid(with: data)
        //: let newData: InscriptionThen = data as! InscriptionThen
        let newData: InscriptionThen = data as! InscriptionThen
    ...
    override public func updateConstraints() {
        ...
        //: let csize = self.messageData?.voiceOver()
        let csize = self.messageData?.voiceOver()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(atService(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.tapShared(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(nameRecognizer(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(listForIndex(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        let imagV = UIImageView()
        //: imagV.image = UIImage.submitGift(name: "icon_talk_notic")
        imagV.image = UIImage.submitGift(name: (String(kTagText.prefix(6)) + "alk_n" + k_selectUrl.replacingOccurrences(of: "arc", with: "ot")))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(perimeter(recognizer:)))
...
//: extension MakeMessageCell {
extension MakeMessageCell {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func atService(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.statuteNameCell(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func listForIndex(recognizer: UIGestureRecognizer) {
    ...
    @objc func perimeter(recognizer _: UIGestureRecognizer) {
        //: if self.messageData?.status == .Msg_Status_Fail {
        if self.messageData?.status == .Msg_Status_Fail {
            //: self.delegate?.onRetryMessage(cell: self)
            self.delegate?.tapCell(cell: self)
        }
    }

    //: @objc func onSelectMessage(recognizer: UIGestureRecognizer) {
    @objc func nameRecognizer(recognizer _: UIGestureRecognizer) {
