...
fileprivate let kTurnMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chats_banner_subtract" :*/
fileprivate let kTableId:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let kResultInfoFormat:String = "next makets_bann"
fileprivate let k_hiddenKey:String = "er_suself make var"

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let kNameMsg:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u","b"]
fileprivate let k_angleUrl:String = "timea"
fileprivate let kJoinNameTitleKey:String = "ct_premake view"

/*: "bannerList" :*/
fileprivate let k_theFormat:[Character] = ["b","a","n","n","e","r","L","i"]
fileprivate let k_makeValue:String = "establisht"

/*: "JXBannerCellID" :*/
fileprivate let k_timeIndexStr:[Character] = ["J","X","B","a","n","n","e","r","C","e","l"]
fileprivate let kRootPathText:[Character] = ["l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let kInfoData:[Character] = ["#","F","F","F","F","F","F"]
...
protocol InfiltrateReactiveCompatible: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func since(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class WithDataSource: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: InfiltrateReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTurnMsg.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [AdMeasurable] = //: return Array<AdMeasurable>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.submitGift(name: "icon_chats_banner_subtract"), for: .normal)
        btn.setImage(UIImage.submitGift(name: (String(kTableId) + String(kResultInfoFormat.suffix(7)) + String(k_hiddenKey.prefix(5)) + "btract")), for: .normal)
        //: btn.setImage(UIImage.submitGift(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.submitGift(name: (String(kNameMsg) + k_angleUrl.replacingOccurrences(of: "time", with: "tr") + String(kJoinNameTitleKey.prefix(6)))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(doingEdit), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension WithDataSource {
    ...
    func vantageBanner() {
        ...
        SumRequestManager().prolusion(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(k_theFormat) + k_makeValue.replacingOccurrences(of: "establish", with: "s"))] ?? []).arrayValue
...
extension WithDataSource {
    ...
    @objc private func doingEdit() {
        ...
        self.delegate?.since(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func imageView(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension WithDataSource: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(k_timeIndexStr) + String(kRootPathText)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.setUrlImage(urlStr: url, placeImg: UIImage.rectangle())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(kInfoData)))!
