...
/*: "bg_chat_honey_left" :*/
fileprivate let kBehindKey:[Character] = ["b","g"]
fileprivate let kPointPath:String = "_chatguard extension title"
fileprivate let k_succeedStr:String = "y_leftsection out to"

/*: "bg_chat_honey_right" :*/
fileprivate let k_coverKey:[Character] = ["b","g","_","c","h","a","t","_"]
fileprivate let kMessageName:String = "honey_view start size"
fileprivate let kModelMinimumStr:String = "rinorht"

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_followToValue:[UInt8] = [0x62,0x67,0x62,0x6d,0x21,0x5c,0x68,0x5d,0x5e,0x6b,0x33,0x22,0x19,0x61,0x5a,0x6c,0x19,0x67,0x68,0x6d,0x19,0x5b,0x5e,0x5e,0x67,0x19,0x62,0x66,0x69,0x65,0x5e,0x66,0x5e,0x67,0x6d,0x5e,0x5d]

fileprivate func reportSection(content num: UInt8) -> UInt8 {
    let value = Int(num) + 7
    if value > 255 {
...
/*: \n" :*/
fileprivate let kTextPath:[Character] = ["\n"]

/*: "#FA74B7" :*/
fileprivate let k_drownToAddValue:[Character] = ["#"]
fileprivate let k_borderId:[Character] = ["F","A","7","4","B","7"]

/*: "icon_coin" :*/
fileprivate let kLeadingValue:[Character] = ["i","c","o","n","_"]
fileprivate let k_lockMessage:[Character] = ["c","o","i","n"]
...
//: @objcMembers public class ModeReactiveCompatible: TalkingChatMsgBaseCellData {
@objcMembers public class ModeReactiveCompatible: GroupReactiveCompatible {
    ...
    override public init(direction: UserAppMeasurable) {
        ...
        if direction == .MsgDirectionIncoming {
            ...
            self.bubbleImg = UIImage.beforeImaginationImage(name: (String(kBehindKey) + String(kPointPath.prefix(5)) + "_hone" + String(k_succeedStr.prefix(6))))
            ...
            self.bubbleImg = UIImage.beforeImaginationImage(name: (String(k_coverKey) + String(kMessageName.prefix(6)) + kModelMinimumStr.replacingOccurrences(of: "nor", with: "g")))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_followToValue.map{reportSection(content: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        let attr = reason(giftModel: self.msgModel)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func reason(giftModel: ContentModel) -> NSMutableAttributedString {
        ...
        let giftname = heritageChannel(giftmodel: giftModel.msgInfo.wantGift)
        //: let str = "\(giftModel.msgInfo.wantGift.content)"
        let str = "\(giftModel.msgInfo.wantGift.content)"
        //: let attr1 = NSMutableAttributedString.init(string: str, attributes: [.font: font, .foregroundColor: UIColor.colorStatuteNameApp()])
        let attr1 = NSMutableAttributedString(string: str, attributes: [.font: font, .foregroundColor: UIColor.colorStatuteNameApp()])
        //: let attr2 = NSMutableAttributedString.init(string: "\(giftname)\n", attributes: [.font: font, .foregroundColor: UIColor.init(hex: "#FA74B7")!])
        let attr2 = NSMutableAttributedString(string: "\(giftname)\n", attributes: [.font: font, .foregroundColor: UIColor(hex: (String(k_drownToAddValue) + String(k_borderId)))!])
        ...
        let attr3 = NSMutableAttributedString(string: "\(giftModel.msgInfo.wantGift.giftPrice)", attributes: [.font: font, .foregroundColor: UIColor(hex: (String(k_drownToAddValue) + String(k_borderId)))!])
        ...
        giftImgV.setUrlImage(urlStr: giftModel.msgInfo.wantGift.giftIcon)
        ...
        giftImgV2.contentMode = .scaleAspectFit
        //: giftImgV2.image = UIImage.submitGift(name: "icon_coin")
        giftImgV2.image = UIImage.submitGift(name: (String(kLeadingValue) + String(k_lockMessage)))
    ...
    func heritageChannel(giftmodel: SizeModel) -> String {
        ...
        if UserViewToolThen.share.interfaceLang == PlayerSubqueryConvertible.en.rawValue {
            ...
            gameName = giftmodel.giftNameI18n[PlayerSubqueryConvertible.en.rawValue] as? NSString ?? ""
            ...
            gameName = giftmodel.giftNameI18n[PlayerSubqueryConvertible.ar.rawValue] as? NSString ?? ""
            ...
            gameName = giftmodel.giftNameI18n[PlayerSubqueryConvertible.es.rawValue] as? NSString ?? ""
            ...
            gameName = giftmodel.giftNameI18n[PlayerSubqueryConvertible.pt.rawValue] as? NSString ?? ""
        }
