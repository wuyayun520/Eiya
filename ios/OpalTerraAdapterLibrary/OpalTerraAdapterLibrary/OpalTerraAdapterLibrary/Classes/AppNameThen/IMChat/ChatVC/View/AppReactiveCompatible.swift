...
fileprivate let k_touchData:[UInt8] = [0x34,0x33,0x34,0x29,0x75,0x3e,0x32,0x39,0x38,0x2f,0x67,0x74,0x7d,0x35,0x3c,0x2e,0x7d,0x33,0x32,0x29,0x7d,0x3f,0x38,0x38,0x33,0x7d,0x34,0x30,0x2d,0x31,0x38,0x30,0x38,0x33,0x29,0x38,0x39]

private func bookColor(gift num: UInt8) -> UInt8 {
    return num ^ 93
}

/*: "0/ :*/
fileprivate let kTextData:String = "0/"

/*: "#AAAAAA" :*/
fileprivate let k_centerTitle:String = "#"
fileprivate let kModelMainMenuName:[Character] = ["A","A","A","A","A","A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let kPullId:String = "btn_cbutton info"
fileprivate let kEqualMessage:String = "layerin"
fileprivate let kTableClickId:[Character] = ["g","s","_","u","n","c","h","o","i","c","e"]

/*: "btn_chatsettings_choiced" :*/
fileprivate let k_sharedUrl:[Character] = ["b","t","n","_","c","h","a","t","s"]
fileprivate let k_centerFormat:[Character] = ["e","t","t","i","n","g","s","_","c","h"]
fileprivate let k_frameUrl:String = "olayced"

/*: "Top" :*/
fileprivate let kModeStr:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let kViewEqualValue:[Character] = ["S","a","v","e"]

/*: "Cancel" :*/
fileprivate let kFormalFormat:String = "balance"
fileprivate let kPhotoFormat:[Character] = ["a","n","c","e","l"]

/*: / :*/
fileprivate let kPathButtonId:String = "record"
...
class AppReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_touchData.map{bookColor(gift: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + appButtonData
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: user_recordMarginValue - height, width: constFormalContent, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.imageUser(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        //: let v = UIView()
        let v = UIView()
        //: v.backgroundColor = UIColor.postColor()
        v.backgroundColor = UIColor.postColor()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (k_centerTitle.capitalized + String(kModelMainMenuName)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: ResumeControl = {
        ...
        //: btn.setImage(UIImage.submitGift(name: "btn_chatsettings_unchoice"), for: .normal)
        btn.setImage(UIImage.submitGift(name: (String(kPullId.prefix(5)) + "hatse" + kEqualMessage.replacingOccurrences(of: "layer", with: "tt") + String(kTableClickId))), for: .normal)
        //: btn.setImage(UIImage.submitGift(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.submitGift(name: (String(k_sharedUrl) + String(k_centerFormat) + k_frameUrl.replacingOccurrences(of: "lay", with: "i"))), for: .selected)
        ...
        btn.setTitle((String(kModeStr)).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(rite), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(kViewEqualValue)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(teamTo), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((kFormalFormat.replacingOccurrences(of: "balance", with: "C") + String(kPhotoFormat)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(wellLinedGenerate), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension AppReactiveCompatible: YYTextViewDelegate {
...
extension AppReactiveCompatible {
    ...
    @objc private func teamTo() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: PlayerRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        PlayerRequestTool.afterCompletion(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    @objc private func wellLinedGenerate() {
        //: dismiss()
        statusDismiss()
    }
    ...
    func replyTo(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
