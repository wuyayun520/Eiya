...
/*: "#ffffff" :*/
fileprivate let k_positionMessage:String = "#fffffftotal false visual else"

/*: "earned" :*/
fileprivate let k_nameTouchPath:[Character] = ["e","a","r","n","e","d"]

/*: "Coins in" :*/
fileprivate let k_toPath:String = "Coinsmain quote view error user"
fileprivate let k_giftName:String = " inmodel voice path"

/*: "Error:  :*/
fileprivate let k_bottomId:String = "task"
fileprivate let kModelText:[Character] = ["r","r","o","r",":"," "]

/*: "#C946D2" :*/
fileprivate let kOfMsg:String = "let of#C946D2"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let kNameKey:String = "full"
fileprivate let k_brightTitle:String = "n_chatsize name return in"
fileprivate let k_ofData:String = "S_userecord user view after"
...
class RepresentView: UIView {
    ...
    private let TextColor = (String(k_positionMessage.prefix(7)))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: NameScreenModel?) {
        //: self.init()
        self.init()
    ...
    private func row() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            upgradeScreen(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (String(k_nameTouchPath))) {
                ...
                if let range = attrStr.string.range(of: (String(k_toPath.prefix(5)) + String(k_giftName.prefix(3)))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.fontChange(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.fontChange(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (k_bottomId.replacingOccurrences(of: "task", with: "E") + String(kModelText)) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func upgradeScreen(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.setUrlImage(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.streetSmartEqual(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: NameScreenModel? {
        //: didSet {
        didSet {
            //: configModel()
            row()
    ...
    var totalTime: CGFloat {
        ...
        return (constFormalContent + widthValue) * SpendTime / constFormalContent
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / constFormalContent
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / constFormalContent
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(kOfMsg.suffix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.contentMode = .scaleAspectFill
        //: h.image = UIImage.submitGift(name: "btn_chat_floatS_userBK_nor")
        h.image = UIImage.submitGift(name: (kNameKey.replacingOccurrences(of: "full", with: "bt") + String(k_brightTitle.prefix(6)) + "_float" + String(k_ofData.prefix(5)) + "rBK_nor"))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(talkEqual(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.tapShared(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension RepresentView {
    ...
    @objc func talkEqual(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = DistanceAppManager.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = DistanceAppManager.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: AddReactiveCompatible.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        AddReactiveCompatible.share.pathAcross(urlStr: httpsUrl, webConfig: config)
    }
}
