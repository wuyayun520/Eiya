...
fileprivate let kColorFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let k_priceMessage:String = "infove"
...
class MakeRecognizerDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kColorFormat.reversed(), encoding: .utf8)!)
    }
    ...
    func tableBar(_ danumuMsgModel: NameScreenModel?) {
    ...
    @objc func cutCustomerAction(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(cutCustomerAction(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension MakeRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    func vieFirst(for danumuMsgModel: NameScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = RepresentView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: constFormalContent, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if MakeLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = constFormalContent
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (k_priceMessage.replacingOccurrences(of: "info", with: "mo")))
...
//    }
    ...
    func failureSize(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                vieFirst(for: model, isFirst: isFirst)
