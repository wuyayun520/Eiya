...
fileprivate let kLogModelData:[UInt8] = [0xd6,0xd1,0xd6,0xcb,0x97,0xdc,0xd0,0xdb,0xda,0xcd,0x85,0x96,0x9f,0xd7,0xde,0xcc,0x9f,0xd1,0xd0,0xcb,0x9f,0xdd,0xda,0xda,0xd1,0x9f,0xd6,0xd2,0xcf,0xd3,0xda,0xd2,0xda,0xd1,0xcb,0xda,0xdb]

private func faceLabView(evaluate num: UInt8) -> UInt8 {
    return num ^ 191
}

/*: "nav_back_black_nor" :*/
fileprivate let kMaxData:String = "nsize"
fileprivate let kMakeStr:[Character] = ["_","b","a","c","k","_","b","l","a","c","k","_","n","o","r"]

/*: "btn_detail_more" :*/
fileprivate let kErrorTopText:String = "self info height heightbtn_d"
fileprivate let kNameMessage:[Character] = ["o","r","e"]

/*: "btn_chat_videocall" :*/
fileprivate let kReloadId:[Character] = ["b","t","n","_","c","h","a","t","_","v"]
fileprivate let kCellStr:String = "idecountcall"

/*: "icon_lounge" :*/
fileprivate let kYearFormat:[Character] = ["i","c","o","n","_"]
fileprivate let kColorPath:String = "lmemberunge"

/*: "Official" :*/
fileprivate let kCenterContent:[Character] = ["O","f","f","i","c","i","a","l"]

/*: "Online List" :*/
fileprivate let kCellMessage:[Character] = ["O","n","l","i","n","e"," ","L","i"]
fileprivate let k_twoStr:String = "interaction"

/*: "icon_privateChat_intimate" :*/
fileprivate let k_toName:String = "item image containericon_p"
fileprivate let k_videoName:String = "none leading leftChat"

/*: "#FF60B9" :*/
fileprivate let kAllText:[Character] = ["#","F","F","6","0","B","9"]

/*: "Customer Care Center" :*/
fileprivate let kBlockUrl:[Character] = ["C","u","s","t","o","m","e","r"," ","C","a","r","e"," ","C","e","n","t","e","r"]

/*: "#48FF1E" :*/
fileprivate let k_keyTitle:[Character] = ["#","4","8","F","F","1"]
fileprivate let kShareId:[Character] = ["E"]

/*: "Online" :*/
fileprivate let kViewToFormat:[Character] = ["O","n","l","i","n","e"]

/*: "#FF4B7F" :*/
fileprivate let kObjectPath:String = "#FF4B7Fbackground self add ok"

/*: "busy" :*/
fileprivate let k_shapeText:String = "sharesy"

/*: "+ :*/
fileprivate let kTopTitle:String = "model"

/*: "get json error" :*/
fileprivate let k_countSexStr:String = "get make from type"
fileprivate let kEqualMsg:String = " errview size element view"
fileprivate let k_frameMessage:String = "OR"

/*: "transform.rotation" :*/
fileprivate let k_errorUserData:String = "tranmodelf"
fileprivate let kHalfId:String = "interactionainteractionio"
fileprivate let kLabelData:String = "value"

/*: "transform.scale" :*/
fileprivate let k_sTimeUrl:[Character] = ["t","r","a","n","s","f"]
fileprivate let k_ofId:[Character] = ["o","r","m",".","s","c","a","l","e"]

/*: "zoom&shake" :*/
fileprivate let kGenderPath:[Character] = ["z","o","o","m","&","s","h","a","k"]
fileprivate let k_portPath:[Character] = ["e"]
...
class CoverTitleView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kLogModelData.map{faceLabView(evaluate: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var backButton: UIButton = {
        ...
        let img = UIImage.submitGift(name: (kMaxData.replacingOccurrences(of: "size", with: "av") + String(kMakeStr))).withRenderingMode(.alwaysTemplate)
    ...
    lazy var moreButton: UIButton = {
        ...
        let img = UIImage.submitGift(name: (String(kErrorTopText.suffix(5)) + "etail_m" + String(kNameMessage))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var videoCallBtn: UIButton = {
        //: let btn = UIButton(type: .custom)
        let btn = UIButton(type: .custom)
        //: let img = UIImage.submitGift(name: "btn_chat_videocall").withRenderingMode(.alwaysTemplate)
        let img = UIImage.submitGift(name: (String(kReloadId) + kCellStr.replacingOccurrences(of: "count", with: "o"))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
    ...
    lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.submitGift(name: (String(kYearFormat) + kColorPath.replacingOccurrences(of: "member", with: "o")))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(kCenterContent)).localized, for: .normal)
    ...
    lazy var onlineListBtn: UIButton = {
        ...
        btn.setTitle((String(kCellMessage) + k_twoStr.replacingOccurrences(of: "interaction", with: "st")).localized, for: .normal)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineStatusV: ResumeControl = {
    ...
    private lazy var myBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(clickColorComponent), for: .touchUpInside)
    ...
    private lazy var intimateBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.submitGift(name: "icon_privateChat_intimate"), for: .normal)
        btn.setImage(UIImage.submitGift(name: (String(k_toName.suffix(6)) + "rivate" + String(k_videoName.suffix(4)) + "_intimate")), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(sessionData), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var intimateLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(kAllText)))
    ...
    private lazy var addIntimateLB: UILabel = {
        ...
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.font = UIFont.fontChange(type: .Semibold, fontSize: 12)
        lb.font = UIFont.fontChange(type: .Semibold, fontSize: 12)
        //: lb.isHidden = true
        lb.isHidden = true
...
extension CoverTitleView {
    ...
    func equalViews() {
        ...
        if self.chatType == .service {
            ...
            titleLabel.text = (String(kBlockUrl)).localized
            //: titleLabel.font = .fontChange(type: .Medium, fontSize: 18)
            titleLabel.font = .fontChange(type: .Medium, fontSize: 18)
            //: titleLabel.textColor = .colorStatuteNameApp()
            titleLabel.textColor = .colorStatuteNameApp()
    ...
    func viewSmart() {
        ...
        if self.chatType == .service { // 官方客服
            ...
            let offset = MakeLanguageManager.shared.direction == .leftToRight ? 22 : -22
    ...
    ///   - userModel: 用户信息
    ///   - isFrom: 列表控制器入口
    //: func private_updateNavView(userModel: HumankindThen, isFrom: PrivateChatIsFromEnum) {
    func contain(userModel: HumankindThen, isFrom: NameDataFromEnum) {
        ...
        if userModel.onlineStatus == 1 {
            ...
            let image = UIImage.activeOf(color: UIColor(hex: (String(k_keyTitle) + String(kShareId)))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.circle(), for: .normal)
            onlineStatusV.setImage(image.circle(), for: .normal)
            //: onlineStatusV.setTitle("Online".localized, for: .normal)
            onlineStatusV.setTitle((String(kViewToFormat)).localized, for: .normal)
            ...
            let image = UIImage.activeOf(color: UIColor(hex: (String(kObjectPath.prefix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineStatusV.setImage(image.circle(), for: .normal)
            onlineStatusV.setImage(image.circle(), for: .normal)
            //: onlineStatusV.setTitle("busy".localized, for: .normal)
            onlineStatusV.setTitle((k_shapeText.replacingOccurrences(of: "share", with: "bu")).localized, for: .normal)
            //: } else {
        } else {
        ...
        titleLabel.textColor = userModel.loungePlus ? .usufructuaryHeadSkinColour() : .white
        ...
        var videoCall = userModel.enableVideoCall
        //: if isFrom == .LiveRoom || DistanceAppManager.share.appStatus == AppSkinStatus.special.rawValue {
        if isFrom == .LiveRoom || DistanceAppManager.share.appStatus == NameQuickLookable.special.rawValue {
            //: videoCall = false
            videoCall = false
        }
        //: if videoCall {
    ...
    /// - Parameters:
    ///   - userModel: 用户信息
    //: func private_updateIntimateNavView(userModel: HumankindThen) {
    func awakened(userModel: HumankindThen) {
        ...
        userBtn.isHidden = false
        //: myBtn.setUrlImage(urlStr: DistanceAppManager.share.loginUserMode.headPic ?? "")
        myBtn.successOn(urlStr: DistanceAppManager.share.loginUserMode.headPic ?? "")
        //: userBtn.setUrlImage(urlStr: userModel.headPic)
        userBtn.successOn(urlStr: userModel.headPic)
    ...
    func save(intimate: Int) {
        //: let str  = "+\(intimate)"
        let str = "+\(intimate)"
        //: let attrString = NSMutableAttributedString(string: str)
        let attrString = NSMutableAttributedString(string: str)
        //: let attributes = [.foregroundColor: UIColor.white, .font: UIFont.fontChange(type: .Semibold, fontSize: 12)] as [NSAttributedString.Key: Any]
        let attributes = [.foregroundColor: UIColor.white, .font: UIFont.fontChange(type: .Semibold, fontSize: 12)] as [NSAttributedString.Key: Any]
        ...
        let url = VideoEffectTool.default.conType(type: .Private_Chat_intimate)
        //: do {
        do {
            ...
            printLog(message: (String(k_countSexStr.prefix(4)) + "json" + String(kEqualMsg.prefix(4)) + k_frameMessage.lowercased()))
        }
    }
    ...
    func requestRecent(intimate: Int) {
        //: intimateLB.text = "\(intimate)"
        intimateLB.text = "\(intimate)"
    }

    //: @objc func myBtnClick() {
    @objc func clickColorComponent() {
        //: AddReactiveCompatible.share.func__pushToUserDetailVC(uid: DistanceAppManager.share.loginUserMode.userID)
        AddReactiveCompatible.share.afterTo(uid: DistanceAppManager.share.loginUserMode.userID)
    }

    //: @objc func userIconClick() {
    @objc func visible() {
        //: AddReactiveCompatible.share.func__pushToUserDetailVC(uid: userId)
        AddReactiveCompatible.share.afterTo(uid: userId)
    }

    //: @objc func intimateBtnClick() {
    @objc func sessionData() {
        //: let view = TalkingIntimateInstructionsPopUpView.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight))
        let view = SingleReactiveCompatible(frame: CGRect(x: 0, y: 0, width: constFormalContent, height: user_recordMarginValue))
        //: view.show()
        view.communicationTag()
    }

    //: func videoCallBtnAnimat() {
    func videoThroughSquareBlock() {
        ...
        let roate = CAKeyframeAnimation(keyPath: (k_errorUserData.replacingOccurrences(of: "model", with: "s") + "orm.ro" + kHalfId.replacingOccurrences(of: "interaction", with: "t") + kLabelData.replacingOccurrences(of: "value", with: "n")))
        ...
        let zoom = CAKeyframeAnimation(keyPath: (String(k_sTimeUrl) + String(k_ofId)))
        ...
        videoCallBtn.layer.add(groupAnnimation, forKey: (String(kGenderPath) + String(k_portPath)))
    }
}
