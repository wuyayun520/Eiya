...
fileprivate let k_addMsg:[UInt8] = [0x9e,0xa3,0x9e,0xa9,0x5d,0x98,0xa4,0x99,0x9a,0xa7,0x6f,0x5e,0x55,0x9d,0x96,0xa8,0x55,0xa3,0xa4,0xa9,0x55,0x97,0x9a,0x9a,0xa3,0x55,0x9e,0xa2,0xa5,0xa1,0x9a,0xa2,0x9a,0xa3,0xa9,0x9a,0x99]

fileprivate func viewShared(post num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let k_valueName:String = "location leticon_"
fileprivate let k_lengthName:String = "block cover let normal make_lock"

/*: "icon_chat_burned" :*/
fileprivate let kMakeId:String = "icon_chatto label name type up"
fileprivate let k_sizeUrl:String = "_burnedlayer no"

/*: "icon_chat_burned_shan" :*/
fileprivate let kRangeMessage:String = "icon_ctype key string incident"
fileprivate let kPostTitle:String = "RNED"

/*: "Already burned" :*/
fileprivate let k_styleFieldMessage:String = "time button extension kit selfAlr"
fileprivate let kFileId:[Character] = ["e","a","d","y"," ","b","u","r","n","e","d"]
...
class RemoteModelViewCell: ObjectStreamMsgCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_addMsg.map{viewShared(post: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        //: imag.alpha = 0.9
        imag.alpha = 0.9
        //: imag.image = UIImage.submitGift(name: "icon_chat_lock")
        imag.image = UIImage.submitGift(name: (String(k_valueName.suffix(5)) + "chat" + String(k_lengthName.suffix(5))))
    ...
    lazy var burnedImagV: ResumeControl = {
        ...
        imgV.setBackgroundImage(UIImage.submitGift(name: (String(kMakeId.prefix(9)) + String(k_sizeUrl.prefix(7)))), for: .normal)
        //: imgV.setImage(UIImage.submitGift(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.submitGift(name: (String(kRangeMessage.prefix(6)) + "hat_bu" + kPostTitle.lowercased() + "_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(k_styleFieldMessage.suffix(3)) + String(kFileId)).localized, for: .normal)
...
extension RemoteModelViewCell {
    ...
    override func afterMid(with data: DisappearReactiveCompatible) {
        ...
        let intimatePhoto: TimeExposureModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            viewEnable(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            viewEnable(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: ChatCellData, urlStr: String?, isMosaic: Bool) {
    func viewEnable(data: ChatCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            equalRemove(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
    ...
    func equalRemove(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.sizeValueCompletion(urlStr: urlStr) { [weak self] image in
