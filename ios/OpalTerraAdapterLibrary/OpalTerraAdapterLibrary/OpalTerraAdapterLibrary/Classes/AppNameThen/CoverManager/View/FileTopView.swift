...
fileprivate let k_withMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bth_live_follow_pre" :*/
fileprivate let kResultTitle:[Character] = ["b","t"]
fileprivate let kTextMessage:String = "h_livekey corner image app"
fileprivate let k_addData:String = "add if record return actionow_pre"

/*: "btn_live_closed_nor" :*/
fileprivate let kViewMessage:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e"]
fileprivate let k_progressKey:String = "d_norcell else i"

/*: "removeAttentionUid" :*/
fileprivate let kValueMsg:[Character] = ["r","e","m","o","v","e","A","t","t","e","n","t","i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let kIconTitle:[Character] = ["a","t","t","e","n"]
fileprivate let k_viewTitle:String = "allow buttontionUid"

/*: "source" :*/
fileprivate let kSourceText:String = "soheadrc"
fileprivate let k_actualIconFormat:String = "key"

/*: "num" :*/
fileprivate let kPokeImageMsg:String = "nuremove"

/*: "99+" :*/
fileprivate let kLabelKey:[UInt8] = [0xd0,0xd0,0xc2]

private func cancelBottom(color num: UInt8) -> UInt8 {
    return num ^ 233
}

/*: "UID: :*/
fileprivate let k_selectedTitle:[Character] = ["U","I","D",":"]
...
class FileTopView: UIView {
    ...
    private var partyModel = ResumeCancelMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_withMessage.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(valueMove), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.submitGift(name: (String(kResultTitle) + String(kTextMessage.prefix(6)) + "_foll" + String(k_addData.suffix(6)))), for: .normal)
    ...
    private lazy var miniBtn: UIButton = {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setImage(UIImage.submitGift(name: "btn_live_closed_nor"), for: .normal)
        btn.setImage(UIImage.submitGift(name: (String(kViewMessage) + String(k_progressKey.prefix(5)))), for: .normal)
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(networkStop), for: .touchUpInside)
    ...
    private lazy var onlineListView: LanguageView = {
...
extension FileTopView {
    ...
    @objc func broadcastAt(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(kValueMsg))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(kIconTitle) + String(k_viewTitle.suffix(7)))] = uid
            //: dict["source"] = "2"
            dict[(kSourceText.replacingOccurrences(of: "head", with: "u") + k_actualIconFormat.replacingOccurrences(of: "key", with: "e"))] = "2"
        }
        //: PlayerRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
...
extension FileTopView {
    ...
    @objc private func possibility() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        EnableReqTool.party(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(kPokeImageMsg.replacingOccurrences(of: "remove", with: "m"))].intValue
    ...
    private func tapTimer() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(possibility), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension FileTopView {
    ...
    func viewCurrent() {
        ...
        partyModel = CoverManager.byRest().partyModel
        //: nickNameLab.text = partyModel.streamerInfo.nickname
        nickNameLab.text = partyModel.streamerInfo.nickname
        //: uidLab.text = "UID:\(partyModel.streamerInfo.uid)"
        uidLab.text = (String(k_selectedTitle)) + "\(partyModel.streamerInfo.uid)"
        ...
        icon.setUrlImage(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.delay(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == DistanceAppManager.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == DistanceAppManager.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            shareQueryed(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            shareQueryed(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        tapTimer()
    ...
    @objc private func detailClick() {
        ...
        broadcastAt(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.shareQueryed(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }
    ...
    func shareQueryed(isAttention: Bool) {
...
extension FileTopView {
    ...
    private func betweenColorBubble() {
    ...
    private func views() {
        ...
        frame = CGRect(x: 0, y: 0, width: constFormalContent, height: main_netValue + actualWidth(w: 43))
