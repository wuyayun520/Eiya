...
fileprivate let kSendMsg:[UInt8] = [0x8c,0x8b,0x8c,0x91,0xcd,0x86,0x8a,0x81,0x80,0x97,0xdf,0xcc,0xc5,0x8d,0x84,0x96,0xc5,0x8b,0x8a,0x91,0xc5,0x87,0x80,0x80,0x8b,0xc5,0x8c,0x88,0x95,0x89,0x80,0x88,0x80,0x8b,0x91,0x80,0x81]

private func giftMain(array num: UInt8) -> UInt8 {
    return num ^ 229
}
...
@objc protocol SizeViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func doingNow()

    //: @objc optional func endScroll()
    @objc optional func majorApp()
}

//: class TalkingDanmuMsgListTableView: UIView {
class DisplayModelTableView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kSendMsg.map{giftMain(array: $0)}, encoding: .utf8)!)
    }
...
extension DisplayModelTableView {
    //: func startTimer() {
    func actionTitle() {
        //: stopTimer()
        rein()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(littleEvent), userInfo: nil, repeats: true)
    }
...
extension DisplayModelTableView {
    ...
    func makeModel(msgModel: MakeDanmuModel?) {
    ...
    func viewBottom() {
        ...
        if AllHeight > dataNeedBackHeightName {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < dataNeedBackHeightName {
                ...
                self.msgTableView.height = dataNeedBackHeightName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = dataNeedBackHeightName - AllHeight
...
extension DisplayModelTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func kitHandle(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.majorApp?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(const_marginValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ValueView.rowProgress(tableView: tableView, msg: msgModel, indexPath: indexPath)
