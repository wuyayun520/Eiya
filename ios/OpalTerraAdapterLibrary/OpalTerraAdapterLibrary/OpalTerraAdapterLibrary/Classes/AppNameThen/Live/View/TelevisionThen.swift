...
fileprivate let kUserMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "no data available" :*/
fileprivate let kUnitStr:String = "no dmodel head string"
fileprivate let k_readKey:String = "VAILABLE"

/*: "list" :*/
fileprivate let kMediaName:String = "lineed"
...
class TelevisionThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kUserMsg.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(cardGold), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (String(kUnitStr.prefix(4)) + "ata a" + k_readKey.lowercased()).localized
        //: lab.textColor = .white
        lab.textColor = .white
...
extension TelevisionThen {
    ...
    private func worldViewList() {
        ...
        FileListRequestTool.getBackWithIndex(category: 2) { [weak self] _, result, _ in
            ...
            json[(kMediaName.replacingOccurrences(of: "need", with: "st"))].arrayObject?.forEach { dict in
...
extension TelevisionThen {
    ...
    private func dataHeight() -> CGFloat {
        ...
        return lineNum * notiScreenUserContentValue + 24.0 + appButtonData
    }
}
...
extension TelevisionThen: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.generateEqual(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
...
extension TelevisionThen {
    ...
    func outspokenFrom(from: EqualFromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = ValueThen.kindShared().liveRoomModel.gameList
            //: showAnimation()
            library()
    ...
    private func library() {
        ...
        self.contentView.frame = CGRect(x: 0, y: user_recordMarginValue, width: constFormalContent, height: dataHeight())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension TelevisionThen {
    ...
    private func the() {
        ...
        contentView.frame = CGRect(x: 0, y: user_recordMarginValue, width: constFormalContent, height: dataHeight())
