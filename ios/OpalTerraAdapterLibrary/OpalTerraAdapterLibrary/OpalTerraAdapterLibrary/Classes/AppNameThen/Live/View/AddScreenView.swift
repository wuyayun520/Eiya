...
fileprivate let kCurrentStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let kDataIndexStr:String = "moto"
...
class AddScreenView: UIView {
    ...
    var danumuMsgArray: [ToTransformable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [AddCoverItemView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ToTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kCurrentStr.reversed(), encoding: .utf8)!)
    }
    ...
    func choose(_ danumuMsgModel: ToTransformable?) {
    ...
    @objc func equalTeam(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(equalTeam(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension AddScreenView: UIGestureRecognizerDelegate {
    ...
    func advise(for danumuMsgModel: ToTransformable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = AddCoverItemView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (kDataIndexStr.replacingOccurrences(of: "to", with: "ve")))
    ...
    func opposition(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                advise(for: model, isFirst: isFirst)
