...
/*: "#ffffff" :*/
fileprivate let k_byName:String = "#ffffffred data scale to let"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let kAddMsg:String = "/dist/lreturn data var self"
fileprivate let kCurrentName:[Character] = ["o","u","n","g","e","P","l","u","s","/","i","n","d","e","x",".","h","t","m","l"]

/*: "https://" :*/
fileprivate let kLiveName:String = "model model if maskhttps:/"
fileprivate let k_errorData:[Character] = ["/"]

/*: "http://" :*/
fileprivate let k_dataKey:String = "http://smart manager succeed"
...
class AddCoverItemView: UIView {
    ...
    let Broadcast_TextColor = (String(k_byName.prefix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .tapShared(fontSize: 12)
    ...
    convenience init(model: ToTransformable?) {
        //: self.init()
        self.init()
    ...
    func arrayUrl(withUrl url: String) {
        ...
        self.backgroundImageView.setUrlImage(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.streetSmartEqual(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: ToTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            needUp()
    ...
    var totalTime: CGFloat {
        ...
        return (constFormalContent + widthValue) * Broadcast_SpendTime / constFormalContent
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / constFormalContent
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / constFormalContent
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(dower(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension AddCoverItemView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func dower(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: AddReactiveCompatible.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            AddReactiveCompatible.share.afterTo(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: AddReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                AddReactiveCompatible.share.magnitudeeractionPinType(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(kAddMsg.prefix(7)) + String(kCurrentName))) {
                    //: AddReactiveCompatible.share.func__pushToSubscribePageWebVC()
                    AddReactiveCompatible.share.belowWith()
                    //: return
                    return
                }
                //: let httpsUrl = DistanceAppManager.share.appConfigMode.urlH5Domain+url
                let httpsUrl = DistanceAppManager.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(kLiveName.suffix(7)) + String(k_errorData)), with: (String(k_dataKey.prefix(7))))
                //: AddReactiveCompatible.share.func__pushToWebVC(urlStr: httpUrl)
                AddReactiveCompatible.share.pathAcross(urlStr: httpUrl)
            }
