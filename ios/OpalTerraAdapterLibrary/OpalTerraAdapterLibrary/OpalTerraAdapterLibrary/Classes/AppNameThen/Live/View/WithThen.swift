...
/*: "99+" :*/
fileprivate let kCornerUrl:[Character] = ["9","9","+"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_collectionPath:[UInt8] = [0xa3,0xa4,0xa3,0xbe,0xe2,0xa9,0xa5,0xae,0xaf,0xb8,0xf0,0xe3,0xea,0xa2,0xab,0xb9,0xea,0xa4,0xa5,0xbe,0xea,0xa8,0xaf,0xaf,0xa4,0xea,0xa3,0xa7,0xba,0xa6,0xaf,0xa7,0xaf,0xa4,0xbe,0xaf,0xae]

/*: "#2DF2FF" :*/
fileprivate let k_whiteMessage:[Character] = ["#","2","D","F","2","F"]
fileprivate let kFrameId:String = "with"

/*: "Live" :*/
fileprivate let k_viewDataText:String = "Livetrue false self top if"

/*: "img_home_shadow_icon" :*/
fileprivate let kListValue:[Character] = ["i","m","g","_","h","o","m","e"]
fileprivate let k_imageWithData:String = "shared render_shado"
fileprivate let kEqualName:String = "list make addw_icon"

/*: "get json error" :*/
fileprivate let kPicId:String = "gshow"
fileprivate let k_titleName:[Character] = [" ","j","s","o","n"," ","e","r","r","o","r"]

/*: "#FF2348" :*/
fileprivate let k_dateStr:String = "along image view#FF234"
fileprivate let k_languageFormat:String = "video"

/*: "#35C759" :*/
fileprivate let k_errorMakePath:[Character] = ["#","3","5","C","7","5","9"]

/*: "mfBean" :*/
fileprivate let k_menuPath:String = "image frame let rowmfBean"

/*: "%.1f" :*/
fileprivate let kBackCellStr:String = "model phone all cover%.1f"

/*: "points\n≈ $" :*/
fileprivate let k_textName:[Character] = ["p","o","i","n"]
fileprivate let k_kindMsg:String = "ts\n≈ $"

/*:   :*/
fileprivate let kShowStr:[Character] = [" "]
...
public var show_rateValue: Int {
    //: return DistanceAppManager.share.appConfigMode.countdown1 + DistanceAppManager.share.appConfigMode.countdown2
    return DistanceAppManager.share.appConfigMode.countdown1 + DistanceAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class WithThen: ObjectMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= DistanceAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(show_rateValue - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                afterSession()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(DistanceAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(DistanceAppManager.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_collectionPath.map{$0^202}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: ResumeControl = {
        ...
        let image = UIImage.activeOf(color: UIColor(hex: (String(k_whiteMessage) + kFrameId.replacingOccurrences(of: "with", with: "F")))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.circle(), for: .normal)
        img.setImage(image.circle(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(k_viewDataText.prefix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.submitGift(name: (String(kListValue) + String(k_imageWithData.suffix(6)) + String(kEqualName.suffix(6))))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (kPicId.replacingOccurrences(of: "show", with: "et") + String(k_titleName)))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.font = UIFont.tapShared(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(k_dateStr.suffix(6)) + k_languageFormat.replacingOccurrences(of: "video", with: "8")))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(k_errorMakePath)))
...
extension WithThen {
    ...
    static func titleView() -> WithThen {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = WithThen(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = ListSocketManager.shared.liveMiniPosition
    ...
    func afterSession() {
        ...
        liveStatusImgV.isHidden = !ValueThen.kindShared().isLive
...
extension WithThen {
    ...
    private func betweenCrop() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(morePointchange(notification:)),
    ...
    @objc private func morePointchange(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(k_menuPath.suffix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        toTitle(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func toTitle(mfBean: Double) {
        //: let nowBean = DistanceAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = DistanceAppManager.share.loginUserMode.mf_bean + mfBean
        //: DistanceAppManager.share.loginUserMode.mf_bean = nowBean
        DistanceAppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((String(k_textName) + k_kindMsg).localized) \(divisor)"
